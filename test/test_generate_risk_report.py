...
def test_numeric_and_string_positions():
    ...
    # numeric, string-numeric, and bad-string
    ...
    # numeric tickers should be present
    assert "AAPL" in cw and "SPY" in cw
    # weights should sum to ~1 (allow for float rounding)
...
def test_quantity_price_position():
    ...
    cw = rpt["current_weights"]
    # both tickers should exist
    assert "TSLA" in cw and "MSFT" in cw
    # TSLA weight approx 2000/3800
...
def test_zero_or_nan_portfolio_value_guard():
    ...
    # should still return a report and not raise
    assert "current_weights" in rpt
    # weight normalization still returns a dict
