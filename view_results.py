...
with tabs[0]:
    ...
    # Load sources
    tl = load_csv(
        "trade_log.csv", RESULTS_DIR
    )  # expected: date, ticker, action/side, price, quantity, profit
    sig = load_csv("signals_with_rationale.csv", RESULTS_DIR)  # preferred
    if sig.empty:
        sig = load_csv("signals.csv", RESULTS_DIR)  # fallback
    ns = load_csv(
        "news_sentiment.csv", RESULTS_DIR
    )  # expected: date/publishedAt, ticker, title/url/sentiment
    ...
    if not tickers:
        ...
        with c3:
            show_candles = st.selectbox(
                "Price View", ["Line", "Candlestick"], index=0, key="t0_price_view"
            )

        # Slice window — make cutoff tz-naive UTC to match parsed data
        cutoff_ts = pd.Timestamp.now(tz="UTC").normalize().tz_localize(None) - pd.Timedelta(
            days=lookback_days
        )
        ...
        if not ns.empty:
            ...
            if "date" in ns_t.columns:
                ns_t["date"] = pd.to_datetime(
                    ns_t["date"], errors="coerce", utc=True
                ).dt.tz_localize(None)
            elif "publishedAt" in ns_t.columns:
                ns_t["date"] = pd.to_datetime(
                    ns_t["publishedAt"], errors="coerce", utc=True
                ).dt.tz_localize(None)
            else:
                ns_t["date"] = pd.NaT
        ...
        wins = (
            int((tl_t.get("profit", pd.Series(dtype=float)) > 0).sum())
            if not tl_t.empty and "profit" in tl_t.columns
            else 0
        )
        win_rate = (wins / total_trades) if total_trades > 0 else 0.0
        avg_pnl = (
            float(tl_t["profit"].mean()) if not tl_t.empty and "profit" in tl_t.columns else 0.0
        )
        cum_pnl = (
            float(tl_t["profit"].sum()) if not tl_t.empty and "profit" in tl_t.columns else 0.0
        )
        ...
        if not sig_t.empty and "date" in sig_t.columns and sig_t["date"].notna().any():
            ...
            last_conf = (
                float(row.get("confidence", np.nan)) if "confidence" in sig_t.columns else np.nan
            )
        ...
        with k4:
            st.metric("Cum P&L", f"{cum_pnl:,.2f}")
        with k5:
                        ...
                        go.Scatter(
                            ...
                            customdata=np.stack(
                                [
                                    dfg.get(
                                        "predicted_close",
                                        pd.Series(np.nan, index=dfg.index),
                                    )
                                    .fillna(0)
                                    .values,
        ...
        with st.expander("Signals & Rationale (latest 200 rows)"):
            if not sig_t.empty:
                ...
                st.dataframe(
                    sig_t.sort_values("date", ascending=True).tail(200)[cols_show],
        ...
        with st.expander("Related News"):
            if not ns_t.empty:
                show_cols = [
                    c
                    for c in ["date", "ticker", "sentiment", "news", "description"]
...
with tabs[3]:
    ...
    if df.empty:
        ...
        if not has_returns and not has_cum:
            st.warning(
                "Expected either daily returns ('strategy_return','market_return') "
            ...
            if not tickers:
                ...
                with c2:
                    normalize = st.checkbox(
                        "Normalize curves to 1.0 at start", value=True, key="t3_norm"
                    )
                with c3:
                ...
                if show_kpis and cs.notna().any() and cm.notna().any():
                    ...
                    with k4:
                        st.metric(
                            "Mkt Total",
                            (
                                f"{m_stats['total_return']:.1%}"
                                if np.isfinite(m_stats["total_return"])
                                else "—"
                            ),
                        )
                    with k5:
                        st.metric(
                            "Mkt CAGR",
                            (f"{m_stats['cagr']:.1%}" if np.isfinite(m_stats["cagr"]) else "—"),
                        )
                    with k6:
                        st.metric(
                            "Mkt MaxDD",
                            (f"{m_stats['max_dd']:.1%}" if np.isfinite(m_stats["max_dd"]) else "—"),
                        )
                ...
                with st.expander("📋 Leaderboard across tickers"):
                    ...
                    for t in tickers:
                        ...
                        if has_returns and not has_cum:
                            sr = pd.to_numeric(
                                sub_t.get("strategy_return"), errors="coerce"
                            ).fillna(0)
                            mr = pd.to_numeric(sub_t.get("market_return"), errors="coerce").fillna(
                                0
                            )
                            sub_t["cumulative_strategy"] = (1 + sr).cumprod()
                            sub_t["cumulative_market"] = (1 + mr).cumprod()
                        cs_t = pd.to_numeric(
                            sub_t.get("cumulative_strategy"), errors="coerce"
                        ).dropna()
                        cm_t = pd.to_numeric(
                            sub_t.get("cumulative_market"), errors="coerce"
                        ).dropna()
                        ...
                        agg.append(
                            {
                                ...
                                "alpha_total": (
                                    (s_stats["total_return"] - m_stats["total_return"])
                                    if np.isfinite(s_stats["total_return"])
                                    and np.isfinite(m_stats["total_return"])
                                    else np.nan
                                ),
        ...
        if f.empty:
            ...
            if not PLOTLY_OK:
                ...
                if chart_type == "Candlestick" and has_base_dates:
                    ...
                    ohlc = load_parquet(ohlc_path)
                    if not ohlc.empty and {
                        "date",
                        "open",
                        "high",
                        "low",
                        "close",
                    }.issubset(ohlc.columns):
                        parse_dates_inplace(ohlc, ("date",))
                        ohlc = ohlc.dropna(subset=["date"]).sort_values("date")
                        fig.add_trace(
                            go.Candlestick(
    ...
    if df.empty:
        ...
        label_col = (
            ...
            if "signal" in df.columns
                ...
                "action" if "action" in df.columns else ("side" if "side" in df.columns else None)
            )
        )
...
with tabs[9]:
    ...
    if df.empty:
        ...
        if not required.issubset(df.columns):
            ...
            latest = (
                df[df["action"].astype(str).str.upper() == "BUY"]
...
with tabs[10]:
    ...
    if df.empty:
        ...
        if "ticker" not in df.columns:
            ...
            st.dataframe(
                trades[cols] if set(cols).issubset(trades.columns) else trades,
                use_container_width=True,
            )
...
with tabs[12]:
    ...
    if df.empty:
        ...
        if score_col:
            ...
            st.warning(
                "No score column found (expected 'total_score' or 'score'). Showing raw data."
            )
            st.dataframe(df, use_container_width=True)
...
with tabs[13]:
    ...
    if df.empty:
        ...
        if score_col:
            ...
            st.warning(
                "No score column found (expected 'total_score' or 'score'). Showing first 10 rows."
            )
...
with tabs[14]:
    ...
    if df.empty:
        ...
        with c3:
            smin = float(
                np.nanmin(pd.to_numeric(df.get("sentiment", pd.Series([np.nan])), errors="coerce"))
            )
            smax = float(
                np.nanmax(pd.to_numeric(df.get("sentiment", pd.Series([np.nan])), errors="coerce"))
            )
        ...
        with cM:
            if "sentiment" in f.columns:
                st.metric(
                    "Avg Sentiment",
                    f"{pd.to_numeric(f['sentiment'], errors='coerce').mean():.2f}",
                )
        with cR:
...
with tabs[15]:
    ...
    if df.empty:
        ...
        if sort_cols:
            f = f.sort_values(sort_cols, ascending=[False, False, False][: len(sort_cols)])
        ...
        with c1:
            st.metric("Alerts shown", len(f))
        with c2:
            if "priority" in f.columns:
                st.metric("HIGH priority", int((f["priority"] == "HIGH").sum()))
        with c3:
            if "ticker" in f.columns:
                st.metric("Unique tickers", f["ticker"].nunique())
...
with tabs[17]:
    ...
    if df.empty:
        ...
        if not {"ticker", "feature", "importance"}.issubset(df.columns):
            ...
            ticker = st.selectbox(
                "Select a ticker", sorted(df["ticker"].unique()), key="t17_ticker"
            )
            ...
            if PLOTLY_OK:
                fig = px.bar(
                    ...
                    x="feature",
                    y="importance",
                    title=f"Feature Importance: {ticker}",
                )
                st.plotly_chart(fig, use_container_width=True)
    ...
    if sig.empty or sns.empty:
        ...
        if {"title", "url"}.issubset(merged.columns):
            merged["news"] = merged.apply(
                lambda r: make_clickable(r.get("title", ""), r.get("url", "")), axis=1
            )
...
with tabs[20]:
    ...
    if mc.empty:
        st.info(
            "No model_comparison.csv yet. Expected columns: ['ticker','date','model','close','predicted_close']."
        )
    else:
        ...
        if missing:
            ...
            sel_models = st.multiselect(
                "Select models to compare", models, default=models, key="t20_models"
            )
    ...
    if uploaded_file:
        try:
            ...
            if not {"date", "close"}.issubset(df.columns):
                ...
                df["strategy_return"] = df["close"].pct_change().fillna(0) * df["signal"].shift(
                    1
                ).fillna(0)
...
with tabs[23]:
    ...
    if con.empty:
        ...
        tw_sum = (
            float(con["target_weight"].fillna(0).sum())
            if "target_weight" in con.columns
            else np.nan
        )
        c1, c2 = st.columns(2)
        with c1:
...
try:
    ...
    if _cb_col1.button(
        "↻ Rescan data / clear cache", use_container_width=True, key="global_rescan"
    ):
        try:
            st.cache_data.clear()
        except Exception:
    ...
    if df.empty:
        ...
        with c1:
            bench_choice = st.selectbox(
                "Benchmark",
                ["Avg Market (across tickers)", "SPY (ticker)"],
                key="t25_bench",
            )
        with c2:
        ...
        if show_kpis:
            ...
            with k5:
                st.metric(
                    "Bench CAGR",
                    f"{b_stats['cagr']:.1%}" if np.isfinite(b_stats["cagr"]) else "—",
                )
            with k6:
                st.metric(
                    "Bench MaxDD",
        ...
        if raw_wide.shape[1] == 0:
            ...
            if mkt.empty or not {"date", "ticker"}.issubset(mkt.columns):
                ...
                if not common_cols:
                    ...
                    W, port_lvl_g, port_lvl_n, bench_lvl, tvr, ps_g, ps_n = build_portfolio(
                        max_pct, alpha
                    )
                    ...
                    with st.expander("Average weights by ticker (over period)"):
                        ...
                        st.dataframe(
                            avg_w.rename("avg_weight").to_frame(),
                            use_container_width=True,
                        )
                    ...
                    with st.expander("🔎 Quick grid search (max cap × α)"):
                        do_grid = st.checkbox(
                            "Run grid search near current params",
                            value=False,
                            key="t26_grid",
                        )
                        if do_grid:
                            ...
                            alphas = sorted(
                                set(
                                    [
                                        max(0.0, min(1.0, round(x, 2)))
                                        for x in [alpha, 0.20, 0.30, 0.40, 0.50]
                                    ]
                                )
                            )
                            rows, best = [], None
                                        ...
                                        rows.append(
                                            {
                                                "max_cap": c,
                                                "alpha": a,
                                                "total_return_net": stats_n.get(
                                                    "total_return", np.nan
                                                ),
                                                "cagr_net": stats_n.get("cagr", np.nan),
                                                "sharpe_net": sharpe_n,
                                            }
                                        )
                                        if best is None or (
                                            np.isfinite(sharpe_n) and sharpe_n > best["sharpe_net"]
                                        ):
                                            best = {
                                                "max_cap": c,
                                                "alpha": a,
                            ...
                            if rows:
                                ...
                                if not PLOTLY_OK:
                                    st.info(
                                        f"Best combo: cap={best['max_cap']:.2f}, α={best['alpha']:.2f}, Net Sharpe={best['stats_n']['sharpe']:.2f}"
                                    )
                                else:
                                    st.markdown(
                                        f"**Best combo (net):** max cap **{best['max_cap']:.2f}**, α **{best['alpha']:.2f}** — Sharpe **{best['stats_n']['sharpe']:.2f}**, CAGR **{best['stats_n']['cagr']:.1%}**"
                                    )
                                    bf = go.Figure()
                                    bf.add_trace(
                                        go.Scatter(
                                            x=best["curve_n"].index,
                                            y=best["curve_n"].values,
                                            name="Best Smart-Weight (Net)",
                                            mode="lines",
                                        )
                                    )
                                    bf.add_trace(
                                    ...
                                    bf.update_layout(
                                        title="Best grid result vs Benchmark"
                                        + (" (normalized)" if normalize_curves else ""),
                                        xaxis_title="Date",
                                        yaxis_title="Cumulative Level",
                                    )
...
with tabs[27]:
    ...
    if sig.empty:
        st.info(
            "Need signals_with_rationale.csv (or signals.csv) with at least [date, ticker, confidence]."
        )
        st.stop()
    ...
    with c1:
        target = st.selectbox(
            "Calibration target (what confidence tries to predict)",
    ...
    with c3:
        show_scatter = st.checkbox(
            "Show scatter (confidence vs next-day return)", True, key="t27_scatter"
        )

    target_series = (
        panel["fwd_mkt_ret"] if target.startswith("Next-day Market") else panel["fwd_strat_ret"]
    )

    merged = sig_idxed.join(target_series.rename("target_ret"), how="left").dropna(
...
with tabs[28]:
    ...
    if svs.empty:
        ...
        has_cum = {"cumulative_strategy", "cumulative_market"}.issubset(svs.columns)
        if has_ret and not has_cum:
            svs["strategy_return"] = pd.to_numeric(svs["strategy_return"], errors="coerce").fillna(
                0
            )
            svs["market_return"] = pd.to_numeric(svs["market_return"], errors="coerce").fillna(0)
            svs["cumulative_strategy"] = svs.groupby("ticker")["strategy_return"].apply(
        ...
        if sig.empty:
            ...
            with c3:
                cost_bps = (
                    st.slider("Trading cost (bps per $ traded)", 0, 50, 5, 1, key="t28_cost") / 1e4
                )
            with c4:
            ...
            if w_mat is None or w_mat.empty:
                ...
                port_ret_gross = (
                    (w_long * r.reindex(w_long.index).fillna(0.0)).groupby(level=0).sum()
                )
            ...
            if bench_choice.startswith("SPY") and ("SPY" in tickers_unique):
                ...
                if has_ret and not has_cum:
                    spy["bench_ret"] = pd.to_numeric(spy["market_return"], errors="coerce").fillna(
                        0
                    )
                else:
                    to_numeric(spy, ["cumulative_market"])
            ...
            if show_kpis:
                ...
                def _ps(levels: pd.Series) -> Dict[str, float]:
                    ...
                    sharpe = (
                        (avg / vol) * np.sqrt(252)
                        if (np.isfinite(avg) and vol and vol > 0)
                        else np.nan
                    )
                    ...
                    cagr = (
                        (end / start) ** (1 / years) - 1
                        if (np.isfinite(years) and years > 0 and start > 0)
                        else np.nan
                    )
                    dd = (
                        (levels / levels.cummax() - 1.0)
                        if not levels.empty
            ...
            with st.expander("⬇️ Export data"):
                ...
                daily = pd.DataFrame(
                    {
                        "date": (
                            port_ret_gross.index if "port_ret_gross" in locals() else pd.Index([])
                        ),
                        "gross_ret": (
                            port_ret_gross.values if "port_ret_gross" in locals() else []
                        ),
                        "net_ret": (
                            port_ret_net.reindex(port_ret_gross.index).values
                            if "port_ret_gross" in locals()
                            else []
                        ),
                        "bench_ret": (
                            bench_ret.reindex(port_ret_gross.index).values
                            if "port_ret_gross" in locals()
                            else []
                        ),
                        "turnover": (
                            tvr.reindex(port_ret_gross.index).values
                            if "port_ret_gross" in locals()
                            else []
                        ),
                        "cost_applied": (
                            (port_ret_net - port_ret_gross).reindex(port_ret_gross.index).values
                            if "port_ret_gross" in locals()
                            else []
                        ),
                    }
...
with tabs[29]:
    ...
    if svs.empty:
        ...
        with c3:
            cost_bps = (
                st.slider("Trading cost (bps per $ traded)", 0, 50, 5, 1, key="t29_cost") / 1e4
            )
        ...
        if (~pos_rows).any():
            ...
            def _eq_row(r):
                n = r.notna().sum()
                return (
                    pd.Series(np.where(r.notna(), 1.0 / n, 0.0), index=r.index)
                    if n and n > 0
        ...
        if show_kpis:
            ...
            def _ps(levels: pd.Series) -> Dict[str, float]:
                ...
                rets = levels.pct_change().dropna()
                avg, vol = (
                    (float(rets.mean()), float(rets.std())) if not rets.empty else (np.nan, np.nan)
                )
                sharpe = (
                    (avg / vol) * np.sqrt(252) if (np.isfinite(avg) and vol and vol > 0) else np.nan
                )
                start, end = float(levels.iloc[0]), float(levels.iloc[-1])
                total = (end / start - 1.0) if (np.isfinite(start) and start > 0) else np.nan
                days = (levels.index[-1] - levels.index[0]).days if len(levels.index) > 1 else 0
                ...
                dd = (
                    (levels / levels.cummax() - 1.0) if not levels.empty else pd.Series(dtype=float)
                )
                maxdd = float(dd.min()) if not dd.empty else np.nan
                return {
                    "total_return": total,
        ...
        with st.expander("📎 Average weights by ticker (over period)"):
            ...
            st.dataframe(
                w_avg.reset_index().rename(columns={0: "avg_weight"}),
                use_container_width=True,
            )
...
# footer
st.caption(
    "Tip: If new files aren’t appearing, use the Diagnostics expander or click ↻ in Raw CSV to clear cache and rescan."
)
