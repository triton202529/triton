...
from scipy.linalg import toeplitz, hankel, solve, LinAlgError, LinAlgWarning, lstsq
...
def kaiserord(ripple, width):
    ...
    if A < 8:
        ...
        raise ValueError(
            "Requested maximum ripple attenuation "
...
def firwin(
    ...
    *,
    ...
    if pass_zero in ("bandstop", "lowpass"):
        ...
        raise ValueError(
            f"Parameter {pass_zero=} not in (True, False, 'bandpass', "
...
def firwin2(
    numtaps, freq, gain, *, nfreqs=None, window="hamming", antisymmetric=False, fs=None
):
    """
    FIR filter design using the window method.
    ...
    shift = xp.exp(-(numtaps - 1) / 2.0 * 1j * xp.pi * x / nyq)
    if ftype > 2:
        shift *= 1j
...
def remez(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(
            "Type must be 'bandpass', 'differentiator', " "or 'hilbert'"
        ) from e
    ...
    result = _sigtools._remez(
        numtaps, bands, desired, weight, tnum, fs, maxiter, grid_density
    )
...
def firls(numtaps, bands, desired, *, weight=None, fs=None):
    ...
    if (np.diff(bands) <= 0).any() or (np.diff(bands[:, 0]) < 0).any():
        raise ValueError(
            "bands must be monotonically nondecreasing and have " "width > 0."
        )
    ...
    try:  # try the fast way
        ...
        for ww in w:
            if ww.category == LinAlgWarning and str(ww.message).startswith(
...
def minimum_phase(
    ...
    *,
    ...
    if method == "hilbert":
        ...
        dp = max(H) - 1
        ds = 0 - min(H)
        S = 4.0 / (xp.sqrt(1 + dp + ds) + xp.sqrt(1 - dp + ds)) ** 2
        H += ds
        H *= S
        H = xp.sqrt(H)
...
def firwin_2d(
    hsize, window, *, fc=None, fs=2, circular=False, pass_zero=True, scale=True
):
    """
    ...
    if circular:
        if fc is None:
            raise ValueError(
                "Cutoff frequency `fc` must be " "provided when `circular` is True"
            )
