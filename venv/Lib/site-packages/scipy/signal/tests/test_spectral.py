...
class TestPeriodogram:
    ...
    def test_unk_scaling(self):
        assert_raises(
            ValueError, periodogram, np.zeros(4, np.complex128), scaling="foo"
        )
    ...
    def test_window_external(self):
        ...
        assert_raises(ValueError, periodogram, x, 10, win_err)  # win longer than signal
    ...
    def test_shorter_window_error(self):
        ...
        expected_msg = (
            "the size of the window must be the same size "
...
class TestWelch:
    ...
    def test_unk_scaling(self):
        assert_raises(
            ValueError, welch, np.zeros(4, np.complex128), scaling="foo", nperseg=4
        )
    ...
    def test_detrend_external(self):
        x = np.arange(10, dtype=np.float64) + 0.04
        f, p = welch(x, nperseg=10, detrend=lambda seg: signal.detrend(seg, type="l"))
        assert_allclose(p, np.zeros_like(p), atol=1e-15)
    ...
    def test_window_external(self):
        ...
        assert_raises(
            ValueError, welch, x, 10, win, nperseg=4
        )  # because nperseg != win.shape[-1]
        win_err = signal.get_window("hann", 32)
        assert_raises(
            ValueError, welch, x, 10, win_err, nperseg=None
        )  # win longer than signal
    ...
    def test_window_long_or_nd(self):
        ...
        assert_raises(ValueError, welch, np.zeros(4), 1, np.arange(6).reshape((2, 3)))
    ...
    def test_window_correction(self):
        ...
        for window in ["hann", "bartlett", ("tukey", 0.1), "flattop"]:
            ...
            assert_allclose(
                np.sqrt(trapezoid(p_dens, freq)), A * np.sqrt(2) / 2, rtol=1e-3
            )
    ...
    def test_average(self):
        ...
        assert_allclose(f, np.linspace(0, 0.5, 5))
        q = np.array([0.1, 0.05, 0.0, 1.54074396e-33, 0.0])
        assert_allclose(p, q, atol=1e-7, rtol=1e-7)

        assert_raises(ValueError, welch, x, nperseg=8, average="unrecognised-average")

    def test_ratio_scale_to(self):
        """Verify the factor of ``sum(abs(window)**2)*fs / abs(sum(window))**2``
        used in the `welch`  and `csd` docstrs."""
        x, win, fs = np.array([1.0, 0, 0, 0]), np.ones(4), 12
...
class TestCSD:
    ...
    def test_window_external(self):
        ...
        assert_array_equal(pe.shape, (5,))
        assert_raises(
            ValueError, csd, x, x, 10, win, nperseg=256
        )  # because nperseg != win.shape[-1]
        win_err = signal.get_window("hann", 32)
        assert_raises(
            ValueError, csd, x, x, 10, win_err, nperseg=None
        )  # because win longer than signal
    ...
    def test_window_long_or_nd(self):
        assert_raises(
            ValueError, csd, np.zeros(4), np.ones(4), 1, np.array([1, 1, 1, 1, 1])
        )
        assert_raises(
            ValueError, csd, np.zeros(4), np.ones(4), 1, np.arange(6).reshape((2, 3))
        )
...
class TestSpectrogram:
    ...
    def test_window_external(self):
        ...
        assert_raises(
            ValueError, spectrogram, x, fs, win, nperseg=8
        )  # because nperseg != win.shape[-1]
        win_err = signal.get_window(("tukey", 0.25), 2048)
        assert_raises(
            ValueError, spectrogram, x, fs, win_err, nperseg=None
        )  # win longer than signal
...
class TestLombscargle:
    def test_frequency(self):
        ...
        delta = f[1] - f[0]
        assert w - f[np.argmax(P)] < (delta / 2.0)
    ...
    def test_floating_mean_false(self):
        ...
        pgram_offset = lombscargle(
            t, y + offset, f, normalize=True, floating_mean=False
        )
...
class TestSTFT:
    ...
    def test_input_validation(self):
        ...
        with pytest.warns(
            UserWarning, match="NOLA condition failed, " + "STFT may not be invertible"
        ):
    ...
    def test_roundtrip_real(self, scaling):
        ...
        for window, N, nperseg, noverlap in settings:
            ...
            tr, xr = istft(
                zz, nperseg=nperseg, noverlap=noverlap, window=window, scaling=scaling
            )
    ...
    def test_roundtrip_not_nola(self):
        ...
        for window, N, nperseg, noverlap in settings:
            ...
            with pytest.warns(UserWarning, match="NOLA"):
                tr, xr = istft(
                    zz, nperseg=nperseg, noverlap=noverlap, window=window, boundary=True
                )
    ...
    def test_roundtrip_scaling(self):
        ...
        s2 = np.sum(Zp0.real**2 + Zp0.imag**2, axis=0) + np.sum(
            Zp1.real**2 + Zp1.imag**2, axis=0
        )
...
class TestSampledSpectralRepresentations:
    ...
    def test_windowed_DFT(self):
        ...
        assert_allclose(
            welch(self.x_ref, scaling="spectrum", **kw)[1],
            P_mag,
            atol=float_res * max(P_mag),
        )
        assert_allclose(
            welch(self.x_ref, scaling="density", **kw)[1],
            P_psd,
