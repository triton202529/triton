...
def _compute_symiirorder2_bwd_hs(k, cs, rsq, omega):
    ...
    c0 = (
        cssq * (1.0 + rsq) / (1.0 - rsq) / (1 - 2 * rsq * np.cos(2 * omega) + rsq * rsq)
    )
...
class TestSymIIR:
    ...
    def test_symiir1_ic(self, dtype, precision, xp):
        ...
        xp_assert_close(
            symiirorder1_ic(x, b, precision), expected, atol=2e-6, rtol=2e-7
        )
    ...
    @skip_xp_backends(
        np_only=True, reason="_initial_fwd functions are private and numpy-only"
    )
    @pytest.mark.parametrize("dtype", ["float32", "float64"])
    @pytest.mark.parametrize("precision", [-1.0, 0.7, 0.5, 0.25, 0.0075])
    def test_symiir2_initial_fwd(self, dtype, precision, xp):
        ...
        fwd_initial_2 = (
            ...
            + (
                ...
                + r ** (n_exp + 4) * xp.sin(omega * (n_exp + 3))
            )
            / xp.sin(omega)
        )
    ...
    @skip_xp_backends(
        np_only=True, reason="_initial_bwd functions are private and numpy-only"
    )
    @pytest.mark.parametrize("dtype", ["float32", "float64"])
    @pytest.mark.parametrize("precision", [-1.0, 0.7, 0.5, 0.25, 0.0075])
    def test_symiir2_initial_bwd(self, dtype, precision, xp):
        ...
        diff = _compute_symiirorder2_bwd_hs(
            idx, cs, r * r, omega
