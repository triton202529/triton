...
from scipy.signal._short_time_fft import (
    FFT_MODE_TYPE,
...
def test_closest_STFT_dual_window_exceptions():
    ...
    with pytest.raises(ValueError, match="Parameter hop=2.0 is not an integer.*"):
        ...
        closest_STFT_dual_window(np.ones(4), 2.0, np.ones(4))
...
def test_closest_STFT_dual_window_roundtrip(win_name, m, hop, sym_win, scaled):
    ...
    xp_assert_close(s1 * s3, 1.0, atol=res, err_msg="Invalid Scale factors")
...
@pytest.mark.parametrize(
    "win_name, nperseg, noverlap, scale_fac",
    (
        [
            ("boxcar", 16, 8, 1 / 2),
            ("boxcar", 18, 12, 1 / 3),
...
def test_closest_STFT_dual_window_cola(win_name, nperseg, noverlap, scale_fac):
    ...
    d_s, s = closest_STFT_dual_window(
        win, nperseg - noverlap, desired_dual, scaled=True
    )

    res = np.finfo(desired_dual.dtype).resolution
    rel_tol_d = max(abs(d_s)) * res * 3
    xp_assert_close(
        s, scale_fac, atol=res * 10, err_msg=f"Scale factor off by {s/scale_fac}"
    )
    xp_assert_close(
        d_s,
        desired_dual * scale_fac,
        atol=res * 10,
        rtol=rel_tol_d,
    ...
    d_u, u = closest_STFT_dual_window(
        win * scale_fac, nperseg - noverlap, desired_dual, scaled=False
    )

    # this should be hard-coded not computed, so no need for allclose
    assert u == 1.0, "Scaling factor not 1 for parameter `scaled=True`!"
    xp_assert_close(
        d_u,
        desired_dual,
        atol=res * 10,
...
def test_invalid_initializer_parameters():
    ...
    with pytest.raises(
        ValueError, match=r"Parameter win must be 1d, " + r"but win.shape=\(2, 2\)!"
    ):
        ShortTimeFFT(np.ones((2, 2)), hop=4, fs=1)
    with pytest.raises(ValueError, match="Parameter win must have " + "finite entries"):
        ShortTimeFFT(np.array([1, np.inf, 2, 3]), hop=4, fs=1)
    with pytest.raises(
        ValueError, match="Parameter hop=0 is not " + "an integer >= 1!"
    ):
        ShortTimeFFT(np.ones(4), hop=0, fs=1)
    with pytest.raises(
        ValueError, match="Parameter hop=2.0 is not " + "an integer >= 1!"
    ):
        # noinspection PyTypeChecker
        ShortTimeFFT(np.ones(4), hop=2.0, fs=1)
    with pytest.raises(
        ValueError, match=r"dual_win.shape=\(5,\) must equal " + r"win.shape=\(4,\)!"
    ):
        ShortTimeFFT(np.ones(4), hop=2, fs=1, dual_win=np.ones(5))
    with pytest.raises(
        ValueError, match="Parameter dual_win must be " + "a finite array!"
    ):
...
def test_exceptions_properties_methods():
    ...
    with pytest.raises(
        ValueError, match="Sampling interval T=-1 must be " + "positive!"
    ):
        SFT.T = -1
    with pytest.raises(
        ValueError, match="Sampling frequency fs=-1 must be " + "positive!"
    ):
        SFT.fs = -1
    with pytest.raises(
        ValueError,
        match="fft_mode='invalid_typ' not in "
        + r"\('twosided', 'centered', "
        + r"'onesided', 'onesided2X'\)!",
    ):
        SFT.fft_mode = "invalid_typ"
    with pytest.raises(
        ValueError, match="For scaling is None, " + "fft_mode='onesided2X' is invalid.*"
    ):
        SFT.fft_mode = "onesided2X"
    with pytest.raises(
        ValueError,
        match="Attribute mfft=7 needs to be " + "at least the window length.*",
    ):
        SFT.mfft = 7
    ...
    with pytest.raises(ValueError, match="phase_shift=3.0 has the unit .*"):
        SFT.phase_shift = 3.0
    with pytest.raises(
        ValueError, match="-mfft < phase_shift < mfft " + "does not hold.*"
    ):
    ...
    with pytest.raises(
        ValueError, match="Trend type must be 'linear' " + "or 'constant'"
    ):
        # noinspection PyTypeChecker
        SFT.stft_detrend(np.zeros(16), detr="invalid")
    with pytest.raises(
        ValueError, match="Parameter detr=nan is not a str, " + "function or None!"
    ):
        # noinspection PyTypeChecker
    ...
    with pytest.raises(
        ValueError, match=r"S.shape\[f_axis\]=2 must be equal" + " to self.f_pts=5.*"
    ):
        SFT.istft(np.zeros((2, 2)))
    with pytest.raises(
        ValueError, match=r"S.shape\[t_axis\]=1 needs to have" + " at least 2 slices.*"
    ):
        SFT.istft(np.zeros((SFT.f_pts, 1)))
    with pytest.raises(
        ValueError, match=r".*\(k1=100\) <= \(k_max=12\) " + "is false!$"
    ):
        SFT.istft(np.zeros((SFT.f_pts, 3)), k1=100)
    with pytest.raises(
        ValueError, match=r"\(k1=1\) - \(k0=0\) = 1 has to " + "be at least.* length 4!"
    ):
        SFT.istft(np.zeros((SFT.f_pts, 3)), k0=0, k1=1)

    with pytest.raises(
        ValueError, match=r"Parameter axes_seq='invalid' " + r"not in \['tf', 'ft'\]!"
    ):
        # noinspection PyTypeChecker
...
@pytest.mark.parametrize(
    "win_params, Nx",
    [
        (("gaussian", 2.0), 9),  # in docstr
        ("triang", 7),
        (("kaiser", 4.0), 9),
        (("exponential", None, 1.0), 9),
        (4.0, 9),
    ],
)
def test_from_window(win_params, Nx: int):
    ...
    SFT0 = ShortTimeFFT(
        w_sym, hop=3, fs=fs, fft_mode="twosided", scale_to="psd", phase_shift=1
    )
    nperseg = len(w_sym)
    noverlap = nperseg - SFT0.hop
    SFT1 = ShortTimeFFT.from_window(
        win_params,
        fs,
        nperseg,
        noverlap,
...
def test_from_win_equals_dual_unitary():
    ...
    SFT = ShortTimeFFT.from_win_equals_dual(
        des_win, hop, 1, fft_mode="twosided", scale_to="unitary"
    )
    # Orthogonal signals:
    ...
    xp_assert_close(
        np.sum(Sxx),
        np.sum(x**2, dtype=Sxx.dtype),
        atol=atol,
        err_msg="Energies do not match!",
    )
    xp_assert_close(
        np.sum(Sxy), 0.0j, atol=atol, err_msg="STFT scalar product of Sx and Sy not 0!"
    )
    xp_assert_close(
        SFT.dual_win,
        SFT.win * SFT.m_num,
...
@pytest.mark.parametrize(
    "scale_to, fac_psd, fac_mag",
    [(None, 0.25, 0.125), ("magnitude", 2.0, 1), ("psd", 1, 0.5)],
)
...
@pytest.mark.parametrize(
    "fft_mode, f",
    [
        ("onesided", [0.0, 1.0, 2.0]),
        ("onesided2X", [0.0, 1.0, 2.0]),
        ("twosided", [0.0, 1.0, 2.0, -2.0, -1.0]),
...
def test_impulse_roundtrip(i):
    ...
    xp_assert_close(Sx1[:, q1_le:], Sx[:, q1_le - Sx1.shape[1] :], err_msg=f"{i=}")
...
def test_asymmetric_window_roundtrip(hop: int):
    ...
    xp_assert_close(
        x1, x1, err_msg="Roundtrip for asymmetric window with " + f" {hop=} failed!"
    )
...
def test_minimal_length_signal(m_num):
    ...
    xp_assert_close(
        x1,
        x,
        err_msg=f"Roundtrip minimal length signal ({n=})"
    ...
    with pytest.raises(
        ValueError,
        match=rf"S.shape\[t_axis\]={Sx.shape[1]-1}"
...
def test_tutorial_stft_legacy_stft():
    ...
    # Legacy STFT:
    f0_u, t0, Sz0_u = stft(
        z, fs, win, nperseg, noverlap, return_onesided=False, scaling="spectrum"
    )
    ...
    t0_r, z0_r = istft(
        Sz0_u, fs, win, nperseg, noverlap, input_onesided=False, scaling="spectrum"
    )
...
@pytest.mark.parametrize("fft_mode", ("twosided", "centered", "onesided", "onesided2X"))
def test_roundtrip_multidimensional(fft_mode: FFT_MODE_TYPE):
    ...
    SFT = ShortTimeFFT(
        get_window("hann", 4), hop=2, fs=1, scale_to="magnitude", fft_mode=fft_mode
    )
...
def test_roundtrip_two_dimensional(phase_shift: int | None):
    ...
    n = 21
    SFT = ShortTimeFFT.from_window(
        "hann", fs=1, nperseg=13, noverlap=7, mfft=16, phase_shift=phase_shift
    )
...
def test_roundtrip_windows(window, n: int, nperseg: int, noverlap: int):
    ...
    SFT = ShortTimeFFT(
        w, nperseg - noverlap, fs=1, fft_mode="twosided", phase_shift=None
    )
    ...
    xp_assert_close(
        x.astype(np.complex128), x1, err_msg="Roundtrip for float values failed"
    )
...
@pytest.mark.parametrize(
    "window, N, nperseg, noverlap, mfft",
...
def test_stft_padding_roundtrip(
    window, N: int, nperseg: int, noverlap: int, mfft: int, padding
):
    ...
    SFT = ShortTimeFFT.from_window(
        window, 1, nperseg, noverlap, fft_mode="twosided", mfft=mfft
    )
    ...
    xp_assert_close(
        z1, z, err_msg="Failed complex roundtrip with " + f" '{padding}' padding"
    )
...
@pytest.mark.parametrize("f_c", (7.0, 23.0))  # frequency of input sine
