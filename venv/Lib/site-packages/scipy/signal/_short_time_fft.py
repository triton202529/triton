...
def closest_STFT_dual_window(
    ...
    *,
    ...
    if not (win.ndim == 1 and win.shape == desired_dual.shape):
        raise ValueError(
            "Parameters `win` and `desired_dual` are not 1d arrays of "
    ...
    if not (1 <= hop <= len(win) and isinstance(hop, int | np.integer)):
        raise ValueError(
            f"Parameter {hop=} is not an integer between 1 and " + f"{len(win)=}!"
        )
    ...
    if not scaled:
        return w_d + desired_dual - q_d, 1.0
    ...
    def from_win_equals_dual(
        ...
        *,
        ...
        if not (desired_win.ndim == 1 and desired_win.size > 0):
            raise ValueError(
                f"Parameter desired_win is not 1d, but " f"{desired_win.shape=}!"
            )
        if issubclass(desired_win.dtype.type, np.integer):
            raise ValueError(
                "Parameter desired_win cannot be of integer type, "
        ...
        if not (1 <= hop <= len(desired_win) and isinstance(hop, int | np.integer)):
            raise ValueError(
                f"Parameter {hop=} is not an integer between 1 and "
                + f"{len(desired_win)=}!"
            )
        if scale_to not in ["magnitude", "psd", "unitary", None]:
            raise ValueError(
                f"Parameter {scale_to=} not in "
        ...
        for m in range(hop):
            ...
            if not (a > relative_resolution):
                raise ValueError(
                    "Parameter desired_win does not have valid STFT dual "
                    f"window for {hop=}!"
                )
            win[m::hop] /= a
