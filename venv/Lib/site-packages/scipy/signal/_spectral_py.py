...
def lombscargle(
    ...
    *,
    ...
    if not (x.ndim == 1 and x.size > 0 and x.shape == y.shape == weights.shape):
        raise ValueError(
            "Parameters x, y, weights must be 1-D arrays of " "equal non-zero length!"
        )
    ...
    if not (np.all(weights >= 0) and np.sum(weights) > 0):
        raise ValueError(
            "Parameter weights must have only non-negative entries "
    ...
    * The `csd` function calculates ``np.conj(Sx[q,p]) * Sy[q,p]``, whereas
    ...
    if isinstance(window, str) or isinstance(window, tuple):
        ...
        if nperseg < 1:
            ...
            warnings.warn(
                f"{nperseg=} is greater than signal length max(len(x), "
                + f"len(y)) = {n}, using nperseg = {n}",
                stacklevel=3,
            )
            nperseg = n
