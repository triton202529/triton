...
def parzen(M, sym=True, *, xp=None, device=None):
    ...
    n = xp.arange(
        -(M - 1) / 2.0, (M - 1) / 2.0 + 0.5, 1.0, dtype=xp.float64, device=device
    )
    w = xp.where(
        abs(n) <= (M - 1) / 4.0,
        (1 - 6 * (abs(n) / (M / 2.0)) ** 2.0 + 6 * (abs(n) / (M / 2.0)) ** 3.0),
        2 * (1 - abs(n) / (M / 2.0)) ** 3.0,
    )
    return _truncate(w, needs_trunc)
...
def bartlett(M, sym=True, *, xp=None, device=None):
    ...
    w = xp.where(n <= (M - 1) / 2.0, 2.0 * n / (M - 1), 2.0 - 2.0 * n / (M - 1))
...
def general_hamming(M, alpha, sym=True, *, xp=None, device=None):
    ...
    """
    xp = _namespace(xp)
    a = xp.asarray([alpha, 1.0 - alpha], dtype=xp.float64, device=device)
    device = xp_device(a)
    return _general_cosine_impl(M, a, xp, device, sym=sym)
...
def kaiser(M, beta, sym=True, *, xp=None, device=None):
    ...
    n = xp.arange(0, M, dtype=xp.float64, device=device)
    alpha = (M - 1) / 2.0
    w = special.i0(beta * xp.sqrt(1 - ((n - alpha) / alpha) ** 2.0)) / special.i0(
        xp.asarray(beta, dtype=xp.float64)
    )
...
def chebwin(M, at, sym=True, *, xp=None, device=None):
    ...
    order = M - 1.0
    _val = xp.asarray(10 ** (abs(at) / 20.0), dtype=xp.float64, device=device)
...
def cosine(M, sym=True, *, xp=None, device=None):
    ...
    w = xp.sin(xp.pi / M * (xp.arange(M, dtype=xp.float64, device=device) + 0.5))
...
def taylor(M, nbar=4, sll=30, norm=True, sym=True, *, xp=None, device=None):
    ...
    def W(n):
        return 1 + 2 * xp.matmul(
            Fm, xp.cos(2 * xp.pi * ma[:, xp.newaxis] * (n - M / 2.0 + 0.5) / M)
        )
...
def dpss(
    M, NW, Kmax=None, sym=True, norm=None, return_ratios=False, *, xp=None, device=None
):
    """
    ...
    if _len_guards(M):
        if not return_ratios:
            ...
            return xp.ones(M, dtype=xp.float64), 1.0
        else:
    ...
    d = ((M - 1 - 2 * nidx) / 2.0) ** 2 * xp.cos(xp.asarray(2 * xp.pi * W))
    ...
    if norm != 2:
        ...
        if M % 2 == 0:
            if norm == "approximate":
                ...
                shift = -(1 - 1.0 / M) * xp.arange(1, M // 2 + 1, dtype=xp.float64)
...
def lanczos(M, *, sym=True, xp=None, device=None):
    ...
    def _calc_right_side_lanczos(n, m):
        return xpx.sinc(2.0 * xp.arange(n, m, dtype=xp.float64) / (m - 1) - 1.0, xp=xp)
...
def get_window(window, Nx, fftbins=True, *, xp=None, device=None):
    ...
    try:
        ...
        if isinstance(window, tuple):
            ...
            raise ValueError(
                f"{str(type(window))} as window type is not supported."
            ) from e
