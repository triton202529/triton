...
directed_2SP_0_to_3 = [[-9999, 0, -9999, 1, -9999], [-9999, 0, -9999, 4, 1]]

undirected_SP = np.array(
    [
        [0, 3, 3, 1, 2],
        [3, 0, 6, 2, 4],
        [3, 6, 0, 4, 5],
...
undirected_SP_limit_2 = np.array(
    [
        [0, np.inf, np.inf, 1, 2],
...
undirected_sparse_zero_G = scipy.sparse.csr_array(
    ([0, 0, 1, 1, 2, 2, 1, 1], ([0, 1, 1, 2, 2, 0, 3, 4], [1, 0, 2, 1, 0, 2, 4, 3])),
    shape=(5, 5),
)
...
directed_negative_weighted_G = np.array(
    [[0, 0, 0], [-1, 0, 0], [0, -1, 0]], dtype=float
)

directed_negative_weighted_SP = np.array(
    [[0, np.inf, np.inf], [-1, 0, np.inf], [-2, -1, 0]], dtype=float
)
...
def test_undirected():
    def check(method, directed_in):
        if directed_in:
            SP1 = shortest_path(
                directed_G, method=method, directed=False, overwrite=False
            )
            assert_array_almost_equal(SP1, undirected_SP)
        else:
            SP2 = shortest_path(
                undirected_G, method=method, directed=True, overwrite=False
            )
...
def test_directed_sparse_zero():
    ...
    def check(method):
        SP = shortest_path(
            directed_sparse_zero_G, method=method, directed=True, overwrite=False
        )
...
def test_undirected_sparse_zero():
    def check(method, directed_in):
        if directed_in:
            SP1 = shortest_path(
                directed_sparse_zero_G, method=method, directed=False, overwrite=False
            )
            assert_array_almost_equal(SP1, undirected_sparse_zero_SP)
        else:
            SP2 = shortest_path(
                undirected_sparse_zero_G, method=method, directed=True, overwrite=False
            )
...
@pytest.mark.parametrize(
    "directed, SP_ans", ((True, directed_SP), (False, undirected_SP))
)
...
def test_dijkstra_min_only_random(n):
    ...
    data = scipy.sparse.random_array(
        (n, n), density=0.5, format="lil", rng=rng, dtype=np.float64
    )
    ...
    rng.shuffle(v)
    indices = v[: int(n * 0.1)]
    ds, pred, sources = dijkstra(
        data, directed=True, indices=indices, min_only=True, return_predecessors=True
    )
    for k in range(n):
        p = pred[k]
        s = sources[k]
...
def test_star_graph(n, method, directed):
    ...
    assert_allclose(SP_solution, SP)
...
def test_predecessors():
    SP_res = {True: directed_SP, False: undirected_SP}
    pred_res = {True: directed_pred, False: undirected_pred}

    def check(method, directed):
        SP, pred = shortest_path(
            directed_G,
            method,
            directed=directed,
            overwrite=False,
...
def test_construct_shortest_path():
    def check(method, directed):
        SP1, pred = shortest_path(
            directed_G, directed=directed, overwrite=False, return_predecessors=True
        )
...
def test_construct_dist_matrix_predecessors_error(directed):
    ...
    assert_raises(
        TypeError, construct_dist_matrix, directed_G, pred.astype(np.int64), directed
    )
...
def test_unweighted_path():
    def check(method, directed):
        SP1 = shortest_path(
            directed_G, directed=directed, overwrite=False, unweighted=True
        )
        SP2 = shortest_path(
            unweighted_G, directed=directed, overwrite=False, unweighted=False
        )
        assert_array_almost_equal(SP1, SP2)
...
def test_negative_cycles():
    ...
    for directed in (True, False):
        ...
        assert_raises(NegativeCycleError, yen, graph, 0, 1, 1, directed=directed)
...
def test_yen_directed():
    distances, predecessors = yen(
        directed_G, source=0, sink=3, K=2, return_predecessors=True
    )
    assert_allclose(distances, [5.0, 9.0])
...
def test_yen_dense():
    ...
    assert_allclose(distances, [2.0, 3.0, 8.0, 9.0, 11.0])
...
def test_yen_undirected():
    ...
    assert_allclose(distances, [1.0, 4.0, 5.0, 8.0])
...
def test_yen_unweighted():
    ...
    assert_allclose(distances, [2.0, 3.0])
    assert_allclose(predecessors, directed_2SP_0_to_3)
