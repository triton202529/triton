...
from scipy.linalg import eigh, toeplitz, cholesky_banded, cho_solve_banded
...
def test_b_orthonormalize(n, m, Vdtype, Bdtype, BVdtype):
    ...
    is_all_complex = np.issubdtype(Vdtype, np.complexfloating) and np.issubdtype(
        BVdtype, np.complexfloating
    )
    is_all_notcomplex = (
        not np.issubdtype(Vdtype, np.complexfloating)
        and not np.issubdtype(Bdtype, np.complexfloating)
        and not np.issubdtype(BVdtype, np.complexfloating)
    )
    ...
    atol = (
        m
        * n
        * max(np.finfo(Vdtype).eps, np.finfo(BVdtype).eps, np.finfo(np.float64).eps)
    )
...
def test_diagonal(n, m, m_excluded):
    ...
    M_s = dia_array(([1.0 / vals], [0]), shape=(n, n))
    M_a = M_s.toarray()
...
def test_sakurai():
    ...
    assert_allclose(accuracy, 0.0, atol=tol)
    a_l = LinearOperator((n, n), matvec=A, matmat=A, dtype="float64")
    ea, _ = eigsh(
        a_l, k=m, which="SA", tol=1e-9, maxiter=15000, v0=rng.normal(size=(n, 1))
    )
...
def test_MikotaPair(n):
    ...
    el, _ = lobpcg(Ac, X, Bc, M=a, tol=1e-4, maxiter=40, largest=False)
    accuracy = max(abs(ee - el) / ee)
    assert_allclose(accuracy, 0.0, atol=tol)
...
def test_diagonal_data_types(n, m):
    ...
    for s_f_i, s_f in enumerate(list_sparse_format):
        ...
        Ms64 = dia_array(([1.0 / vals], [0]), shape=(n, n)).asformat(s_f)
