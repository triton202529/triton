...
class TestLinsolve:
    ...
    def test_twodiags(self, format: str, idx_dtype: np.dtype):
        A = dia_array(
            ([[1, 2, 3, 4, 5], [6, 5, 8, 9, 10]], [0, 1]), shape=(5, 5)
        ).asformat(format)
        b = array([1, 2, 3, 4, 5])
    ...
    def test_shape_compatibility(self):
        ...
        bs = [
            ...
            dok_array([[1], [6]]),
            bsr_array([[1], [6]]),
            array([[1.0, 2.0, 3.0], [6.0, 8.0, 10.0]]),
            csc_array([[1.0, 2.0, 3.0], [6.0, 8.0, 10.0]]),
            csr_array([[1.0, 2.0, 3.0], [6.0, 8.0, 10.0]]),
            dok_array([[1.0, 2.0, 3.0], [6.0, 8.0, 10.0]]),
            bsr_array([[1.0, 2.0, 3.0], [6.0, 8.0, 10.0]]),
        ]
        ...
        for b in bs:
            ...
            for spmattype in [csc_array, csr_array, dok_array, lil_array]:
                ...
                assert_array_almost_equal(
                    toarray(x1), x, err_msg=repr((b, spmattype, 1))
                )
                assert_array_almost_equal(
                    toarray(x2), x, err_msg=repr((b, spmattype, 2))
                )
    ...
    def test_sparsity_preservation(self):
        ident = csc_array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
        b = csc_array([[0, 1], [1, 0], [0, 0]])
        x = spsolve(ident, b)
    ...
    def test_dtype_cast(self):
        A_real = scipy.sparse.csr_array([[1, 2, 0], [0, 0, 3], [4, 0, 5]])
        A_complex = scipy.sparse.csr_array([[1, 2, 0], [0, 0, 3], [4, 0, 5 + 1j]])
...
class TestSpsolveTriangular:
    ...
    def test_singular(self, fmt):
        ...
        for lower in (True, False):
            assert_raises(
                scipy.linalg.LinAlgError, spsolve_triangular, A, b, lower=lower
            )
    ...
    def test_random(self, n, m, lower, format, unit_diagonal, choice_of_A, choice_of_b):
        ...
        if choice_of_b == "floats":
            ...
            raise ValueError("choice_of_b must be 'floats', 'ints', or 'complexints'.")
