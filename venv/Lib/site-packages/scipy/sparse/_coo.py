...
class _coo_base(_data_matrix, _minmax_mixin):
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False, *, maxprint=None):
        ...
        if isinstance(arg1, tuple):
            if isshape(arg1, allow_nd=self._allow_nd):
                ...
                self.coords = tuple(
                    np.array([], dtype=idx_dtype) for _ in range(len(self._shape))
                )
                ...
                idx_dtype = self._get_index_dtype(
                    coords, maxval=max(self.shape), check_contents=True
                )
                self.coords = tuple(
                    np.array(idx, copy=copy, dtype=idx_dtype) for idx in coords
                )
            ...
            if issparse(arg1):
                ...
                coords = M.nonzero()
                self.coords = tuple(
                    idx.astype(index_dtype, copy=False) for idx in coords
                )
    ...
    def _getnnz(self, axis=None):
        ...
        return np.bincount(
            downcast_intp_index(self.coords[1 - axis]), minlength=self.shape[1 - axis]
        )
    ...
    def _check(self):
        ...
        if self.ndim != len(self.coords):
            raise ValueError(
                "mismatching number of index arrays for shape; "
        ...
        for i, idx in enumerate(self.coords):
            if idx.dtype.kind != "i":
                warn(
                    f"index array {i} has non-integer dtype ({idx.dtype.name})",
                    stacklevel=3,
                )
        ...
        if self.nnz > 0:
            for i, idx in enumerate(self.coords):
                if idx.max() >= self.shape[i]:
                    raise ValueError(
                        f"axis {i} index {idx.max()} exceeds "
    ...
    def transpose(self, axes=None, copy=False):
        if axes is None:
            ...
            raise ValueError(
                "Sparse matrices do not support an 'axes' "
        ...
        return self.__class__(
            (self.data, permuted_coords), shape=permuted_shape, copy=copy
        )
    ...
    def toarray(self, order=None, out=None):
        ...
        if self.ndim == 1:
            ...
            coo_todense_nd(
                strides, self.nnz, self.ndim, coords, self.data, B.ravel("A"), fortran
            )
    ...
    def todia(self, copy=False):
        ...
        if len(diags) > 100:
            ...
            warn(
                f"Constructing a DIA matrix with {len(diags)} diagonals "
                "is inefficient",
    ...
    def _setdiag(self, values, k):
        ...
        self.coords = (
            np.concatenate((self.row[keep], new_row)),
    ...
    def _sum_duplicates(self, coords, data):
        ...
        data = data[order]
        unique_mask = np.logical_or.reduce([idx[1:] != idx[:-1] for idx in coords])
    ...
    def _matmul_vector(self, other):
        if self.ndim > 2:
            ...
            coords = np.concatenate(self.coords)
            coo_matvec_nd(
                self.nnz, len(self.shape), strides, coords, self.data, other, result
            )
        ...
        result = np.zeros(
            result_shape, dtype=upcast_char(self.dtype.char, other.dtype.char)
        )
        if self.ndim == 2:
            col = self.col
            row = self.row
        ...
        if issparse(other):
            ...
            if self_is_1d:
                ...
                result = result.reshape(
                    tuple(result.shape[:-2]) + tuple(result.shape[-1:])
                )
            if other_is_1d:
    ...
    def _matmul_multivector(self, other):
        ...
        if self.ndim >= 3 or other.ndim >= 3:
            ...
            coo_matmat_dense_nd(
                ...
                len(self.shape),
                other.shape[-1],
                np.array(other_shape),
                np.array(result_shape),
        ...
        result = np.zeros(result_shape, dtype=result_dtype)
        coo_matmat_dense(
            self.nnz, other.shape[-1], row, col, self.data, other.ravel("C"), result
        )
    ...
    def dot(self, other):
        ...
        if self.shape[-1] != other.shape[-2:][0]:
            raise ValueError(
                f"shapes {self.shape} and {other.shape}" " are not aligned for n-D dot"
            )
    ...
    def tensordot(self, other, axes=2):
        ...
        if any(
            self.shape[ax] != other.shape[bx] for ax, bx in zip(axes_self, axes_other)
        ):
    ...
    def _broadcast_to(self, new_shape, copy=False):
        ...
        if len(new_shape) < len(old_shape):
            raise ValueError(
                "New shape must have at least as many dimensions"
        ...
        if any((o != 1 and o != n) for o, n in zip(shape, new_shape)):
            raise ValueError(
                f"current shape {old_shape} cannot be "
...
def _process_axes(ndim_a, ndim_b, axes):
    if isinstance(axes, int):
        ...
        if any(ax >= ndim_a or ax < -ndim_a for ax in axes_a) or any(
            bx >= ndim_b or bx < -ndim_b for bx in axes_b
        ):
