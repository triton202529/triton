...
class _cs_matrix(_data_matrix, _minmax_mixin, IndexMixin):
        ...
        if issparse(arg1):
            ...
            if isshape(arg1, allow_nd=self._allow_nd):
                ...
                if len(arg1) == 2:
                    ...
                    raise ValueError(
                        f"unrecognized {self.__class__.__name__} "
    ...
    def _multiply_2d_with_broadcasting(self, other):
        ...
        if self.shape[0] == 1 or self.ndim == 1:
            if other2d.shape[1] == 1:  # Dense column vector.
                ...
                raise ValueError(f"inconsistent shapes {self.shape} and {other.shape}")
            idx_dtype = self._get_index_dtype(
                ret.col, maxval=ret.nnz * other2d.shape[0]
            )
        ...
        if self.shape[1] == 1:
            if other2d.shape[0] == 1:  # Dense row vector.
                ...
                raise ValueError(f"inconsistent shapes {self.shape} and {other.shape}")
            idx_dtype = self._get_index_dtype(
                ret.row, maxval=ret.nnz * other2d.shape[1]
            )
    ...
    def _matmul_multivector(self, other):
        ...
        result = np.zeros(
            (M, n_vecs), dtype=upcast_char(self.dtype.char, other.dtype.char)
        )
    ...
    def sum(self, axis=None, dtype=None, out=None):
        ...
        if (
            self.ndim == 2
            and not hasattr(self, "blocksize")
            and axis in self._swap(((1, -1), (0, -2)))[0]
        ):
    ...
    def _major_index_fancy(self, idx):
        ...
        return self.__class__(
            (res_data, res_indices, res_indptr), shape=new_shape, copy=False
        )
    ...
    def _major_slice(self, idx, copy=False):
        ...
        row_nnz = self.indptr[start1:stop1:step] - self.indptr[start0:stop0:step]
        idx_dtype = self.indices.dtype
        res_indptr = np.zeros(M + 1, dtype=idx_dtype)
        np.cumsum(row_nnz, out=res_indptr[1:])
    ...
    def _set_many(self, i, j, x):
        ...
        if -1 not in offsets:
            ...
            warn(
                f"Changing the sparsity structure of a {self.__class__.__name__} is"
    ...
    def _insert_many(self, i, j, x):
        ...
        idx_dtype = self._get_index_dtype(
            (self.indices, self.indptr), maxval=(self.indptr[-1] + x.size)
        )
        self.indptr = np.asarray(self.indptr, dtype=idx_dtype)
        self.indices = np.asarray(self.indices, dtype=idx_dtype)
    ...
    def resize(self, *shape):
        ...
        if hasattr(self, "blocksize"):
            ...
            if rm or rn:
                raise ValueError(
                    f"shape must be divisible into {self.blocksize}"
                    f" blocks. Got {shape}"
                )
    ...
    def _binopt(self, other, op):
        ...
        idx_dtype = self._get_index_dtype(
            (self.indptr, self.indices, other.indptr, other.indices), maxval=maxnnz
        )
