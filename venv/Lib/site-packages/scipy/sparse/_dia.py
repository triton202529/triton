...
class _dia_base(_data_matrix):
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False, *, maxprint=None):
        ...
        if issparse(arg1):
            ...
            try:
                ...
                raise ValueError(
                    "unrecognized form for " f"{self.format}_matrix constructor"
                ) from e
    ...
    def _getnnz(self, axis=None):
        if axis is not None:
            raise NotImplementedError(
                "_getnnz over an axis is not implemented " "for DIA format"
            )
        M, N = self.shape
        nnz = 0
    ...
    def _add_sparse(self, other, sub=False):
        ...
        if np.array_equal(self.offsets, other.offsets):
            return self._with_data(
                self.data - other.data if sub else self.data + other.data
            )
    ...
    def multiply(self, other):
        ...
        offsets, self_idx, other_idx = np.intersect1d(
    ...
    def _matmul_vector(self, other):
        ...
        y = np.zeros(self.shape[0], dtype=upcast_char(self.dtype.char, x.dtype.char))
        ...
        dia_matvec(
            M, N, len(self.offsets), L, self.offsets, self.data, x.ravel(), y.ravel()
        )
    ...
    def _matmul_multivector(self, other):
        res = np.zeros(
            (self.shape[0], other.shape[1]), dtype=np.result_type(self.data, other)
        )
        dia_matvecs(
            *self.shape,
            *self.data.shape,
    ...
    def _matmul_sparse(self, other):
        ...
        return self._dia_container(
            (data.reshape(len(offsets), -1), offsets), (self.shape[0], other.shape[1])
        )
    ...
    def transpose(self, axes=None, copy=False):
        ...
        data = np.hstack(
            (
                self.data,
                np.zeros((self.data.shape[0], pad_amount), dtype=self.data.dtype),
            )
        )
        data = data[r, c]
        return self._dia_container(
            (data, offsets), shape=(num_cols, num_rows), copy=copy
        )
    ...
    def resize(self, *shape):
        ...
        if M > self.shape[0] and np.any(
            self.offsets + self.shape[0] < self.data.shape[1]
        ):
            # explicitly clear values that were previously hidden
            mask = self.offsets[:, None] + self.shape[0] <= np.arange(
                self.data.shape[1]
            )
            self.data[mask] = 0
