...
def check_tolerance(ftol, xtol, gtol, method):
    ...
    if method == "lm" and (ftol < EPS or xtol < EPS or gtol < EPS):
        raise ValueError(
            "All tolerances must be higher than machine epsilon "
...
IMPLEMENTED_LOSSES = dict(
    linear=None, huber=huber, soft_l1=soft_l1, cauchy=cauchy, arctan=arctan
)
...
def least_squares(
    ...
    if f0.ndim != 1:
        raise ValueError(
            "`fun` must return at most 1-d array_like. " f"f0.shape: {f0.shape}"
        )
