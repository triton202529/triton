    ...
    *,
    ...
    if method == "bvls":
        ...
        if not isinstance(A, np.ndarray):
            raise ValueError(
                "method='bvls' can't be used with `A` being "
                "sparse or LinearOperator."
            )
    ...
    if not (
        (isinstance(lsmr_tol, float) and lsmr_tol > 0) or lsmr_tol in ("auto", None)
    ):
    ...
    if lsq_solver == "exact":
        ...
        unbd_lsq = lsmr(
            A, b, maxiter=lsmr_maxiter, atol=first_lsmr_tol, btol=first_lsmr_tol
        )
