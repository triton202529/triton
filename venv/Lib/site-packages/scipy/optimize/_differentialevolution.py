...
class DifferentialEvolutionSolver:
    ...
    def __init__(
        ...
        *,
        ...
        if np.size(self.limits, 0) != 2 or not np.all(np.isfinite(self.limits)):
            raise ValueError(
                "bounds should be a sequence containing finite "
        ...
        self.num_population_members = max(
            5, popsize * max(1, self.parameter_count - eb_count)
        )
    ...
    def solve(self):
        ...
        for nit in range(1, self.maxiter + 1):
            ...
            if self.disp:
                print(
                    f"differential_evolution step {nit}: f(x)="
    ...
    def __next__(self):
        ...
        if self._updating == "immediate":
            ...
            trial_pop = self._mutate_many(np.arange(self.num_population_members))
    ...
    def _mutate_custom(self, candidate):
        ...
        if not len(np.shape(candidate)):
            ...
            S = candidate.shape[0]
            trial = np.array(
                [self.strategy(c, _population, rng=rng) for c in candidate], dtype=float
            )
