...
class TestEQPDirectFactorization(TestCase):
    ...
    def test_nocedal_example(self):
        H = csc_array([[6, 2, 1], [2, 5, 2], [1, 2, 4]])
        A = csc_array([[1, 0, 1], [0, 1, 1]])
        c = np.array([-8, -3, -3])
        b = -np.array([3, 0])
...
class TestProjectCG(TestCase):
    ...
    def test_compare_with_direct_fact(self):
        H = csc_array([[6, 2, 1, 3], [2, 5, 2, 4], [1, 2, 4, 5], [3, 4, 5, 7]])
        A = csc_array([[1, 0, 1, 0], [0, 1, 1, 1]])
        c = np.array([-2, -3, -3, 1])
        b = -np.array([3, 0])
        Z, _, Y = projections(A)
    ...
    def test_negative_curvature_unconstrained(self):
        H = csc_array([[1, 2, 1, 3], [2, 0, 2, 4], [1, 2, 0, 2], [3, 4, 2, 0]])
        A = csc_array([[1, 0, 1, 0], [0, 1, 0, 1]])
        c = np.array([-2, -3, -3, 1])
        b = -np.array([3, 0])
        Z, _, Y = projections(A)
        with pytest.raises(ValueError):
