...
class LowRankMatrix:
    ...
    def __array__(self, dtype=None, copy=None):
        if dtype is not None:
            warnings.warn(
                "LowRankMatrix is scipy-internal code, `dtype` "
                f"should only be None but was {dtype} (not handled)",
                stacklevel=3,
            )
        if copy is not None:
            warnings.warn(
                "LowRankMatrix is scipy-internal code, `copy` "
...
class KrylovJacobian(Jacobian):
    ...
    def __init__(
        ...
        **kw,
        ...
        valid_inner_params = [
            k
            for k in signature(self.method).parameters
        ...
        for key, value in kw.items():
            ...
            if key[6:] not in valid_inner_params:
                ...
                inner_param_suggestions = get_close_matches(
                    key[6:], valid_inner_params, n=1
                )
                if inner_param_suggestions:
                    suggestion_msg = (
                        f" Did you mean '" f"{inner_param_suggestions[0]}'?"
                    )
                else:
                    suggestion_msg = ""
