...
test3_1 = StructTest3(
    bounds=[(2, 50), (0, 50)], expected_x=[250**0.5, 2.5**0.5], expected_fun=5.0
)
...
test_table = StructTestTable(
    bounds=[(-10, 10), (-10, 10)], expected_fun=[50], expected_x=[3.0, 3.0]
)
...
test_infeasible = StructTestInfeasible(
    bounds=[(2, 50), (-1, 1)], expected_fun=None, expected_x=None
)
...
def run_test(
    ...
    if test.expected_x is not None:
        np.testing.assert_allclose(
            res.x, test.expected_x, rtol=test_atol, atol=test_atol
        )

    # (Optional tests)
    if test.expected_fun is not None:
        np.testing.assert_allclose(res.fun, test.expected_fun, atol=test_atol)

    if test.expected_xl is not None:
        np.testing.assert_allclose(res.xl, test.expected_xl, atol=test_atol)

    if test.expected_funl is not None:
        np.testing.assert_allclose(res.funl, test.expected_funl, atol=test_atol)
    return
...
class TestShgoSimplicialTestFunctions:
    ...
    def test_f5_3_cons_symmetry(self):
        ...
        run_test(
            test_s, n=10000, options=options, iters=1, sampling_method="simplicial"
        )
...
class TestShgoArguments:
    ...
    def test_1_2_simpl_iter(self):
        ...
        options = {"minimize_every_iter": False}
        run_test(
            test2_1, n=None, iters=9, options=options, sampling_method="simplicial"
        )
    ...
    def test_5_2_sobol_argless(self):
        ...
        res = shgo(
            test1_1.f, test1_1.bounds, constraints=test1_1.cons, sampling_method="sobol"
        )
        np.testing.assert_allclose(res.x, test1_1.expected_x, rtol=1e-5, atol=1e-5)
    ...
    def test_8_homology_group_diff(self):
        ...
        run_test(
            test1_1, n=None, iters=None, options=options, sampling_method="simplicial"
        )
    ...
    @pytest.mark.xfail(
        IS_PYPY and sys.platform == "win32",
    ...
    def test_20_constrained_args(self):
        ...
        cons = (
            {"type": "ineq", "fun": g1_modified, "args": (0,)},
            {"type": "ineq", "fun": g2},
            {"type": "eq", "fun": h1},
        )
    ...
    def test_21_2_derivative_options(self, derivative):
        ...
        bounds = [(-100, 100)]
        res = shgo(
            objective, bounds, minimizer_kwargs=minimizer_kwargs, options=options
        )
    ...
    def test_21_3_hess_options_rosen(self):
        ...
        ref = minimize(rosen, np.zeros(5), method="Newton-CG", **options)
        assert res.success
        assert_allclose(res.fun, ref.fun)
        assert_allclose(res.x, ref.x, atol=1e-15)
    ...
    def test_21_arg_tuple_sobol(self):
        ...
        bounds = [(0, 10)]
        res = shgo(
            fun, bounds, args=(1,), constraints=constraints, sampling_method="sobol"
        )
        ref = minimize(
            fun, np.zeros(1), bounds=bounds, args=(1,), constraints=constraints
        )
        assert res.success
        assert_allclose(res.fun, ref.fun)
        assert_allclose(res.x, ref.x)
...
class TestShgoFailures:
    ...
    def test_2_sampling(self):
        ...
        assert_raises(
            ValueError, shgo, test1_1.f, test1_1.bounds, sampling_method="not_Sobol"
        )
