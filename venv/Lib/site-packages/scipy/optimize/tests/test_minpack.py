...
class TestFSolve:
    def test_pressure_network_no_gradient(self):
        ...
        final_flows, info, ier, mesg = optimize.fsolve(
            pressure_network, initial_guess, args=(Qtot, k), full_output=True
        )
    ...
    def test_reentrant_func(self):
        ...
        initial_guess = array([2.0, 0.0, 2.0, 0.0])
        final_flows, info, ier, mesg = optimize.fsolve(
            func, initial_guess, args=(Qtot, k), full_output=True
        )
        assert_array_almost_equal(final_flows, np.ones(4))
        assert_(ier == 1, mesg)
    ...
    def test_reentrant_Dfunc(self):
        ...
        initial_guess = array([2.0, 0.0, 2.0, 0.0])
        final_flows = optimize.fsolve(
            pressure_network, initial_guess, args=(Qtot, k), fprime=deriv_func
        )
        assert_array_almost_equal(final_flows, np.ones(4))
...
class TestRootHybr:
    def test_pressure_network_no_gradient(self):
        ...
        Qtot = 4
        initial_guess = array([2.0, 0.0, 2.0, 0.0])
        final_flows = optimize.root(
            pressure_network, initial_guess, method="hybr", args=(Qtot, k)
        ).x
        assert_array_almost_equal(final_flows, np.ones(4))
...
class TestRootLM:
    def test_pressure_network_no_gradient(self):
        ...
        initial_guess = array([2.0, 0.0, 2.0, 0.0])
        final_flows = optimize.root(
            pressure_network, initial_guess, method="lm", args=(Qtot, k)
        ).x
        assert_array_almost_equal(final_flows, np.ones(4))
...
class TestLeastSq:
    ...
    def test_basic_with_gradient(self):
        p0 = array([0, 0, 0])
        params_fit, ier = leastsq(
            self.residuals, p0, args=(self.y_meas, self.x), Dfun=self.residuals_jacobian
        )
        assert_(ier in (1, 2, 3, 4), f"solution not found (ier={ier})")
        # low precision due to random
        assert_array_almost_equal(params_fit, self.abc, decimal=2)
    ...
    def test_full_output(self):
        p0 = array([[0, 0, 0]])
        full_output = leastsq(
            self.residuals, p0, args=(self.y_meas, self.x), full_output=True
        )
    ...
    def test_float32(self):
        ...
        x = np.array(
            [1.475, 1.429, 1.409, 1.419, 1.455, 1.519, 1.472, 1.368, 1.286, 1.231],
            dtype=np.float32,
        )
        y = np.array(
            [
    ...
    def test_reentrant_Dfun(self):
        ...
        p0 = array([0, 0, 0])
        params_fit, ier = leastsq(
            self.residuals, p0, args=(self.y_meas, self.x), Dfun=deriv_func
        )
        assert_(ier in (1, 2, 3, 4), f"solution not found (ier={ier})")
        # low precision due to random
        assert_array_almost_equal(params_fit, self.abc, decimal=2)
    ...
    def test_func_input_output_length_check(self):
        ...
        with assert_raises(
            TypeError, match="Improper input: func input vector length N="
        ):
...
class TestCurveFit:
    ...
    def test_regression_2639(self):
        ...
        def f_double_gauss(x, x0, x1, A0, A1, sigma, c):
            return (
                A0 * np.exp(-((x - x0) ** 2) / (2.0 * sigma**2))
                + A1 * np.exp(-((x - x1) ** 2) / (2.0 * sigma**2))
                + c
            )
    ...
    def _check_nan_policy(
        f, xdata_with_nan, xdata_without_nan, ydata_with_nan, ydata_without_nan, method
    ):
        kwargs = {
            "f": f,
            "xdata": xdata_with_nan,
            "ydata": ydata_with_nan,
            "method": method,
            "check_finite": False,
        }
        # propagate test
        ...
        error_msg = (
            r"nan_policy must be one of \{(?:'raise'|'omit'|None)"
            r"(?:, ?(?:'raise'|'omit'|None))*\}"
        )
        with assert_raises(ValueError, match=error_msg):
    ...
    def test_nan_policy_2d(self, method):
        ...
        xdata_with_nan = np.array(
            [[2, 3, np.nan, 4, 4, np.nan, 5], [2, 3, np.nan, np.nan, 4, np.nan, 7]]
        )
    ...
    def test_bounds(self):
        ...
        for method in [None, "trf", "dogbox"]:
            ...
            popt_class, pcov_class = curve_fit(
                f, xdata, ydata, bounds=bounds_class, method=method
            )
    ...
    def test_curvefit_omitnan(self, sigma_dim):
        ...
        x[i_x] = np.nan
        y[i_y] = np.nan
        res_opt, res_cov = curve_fit(
            exponential, x, y, p0=p0, sigma=sigma, nan_policy="omit"
        )
    ...
    def test_dtypes2(self):
        ...
        popt_64, _ = curve_fit(
            f=hyperbola, xdata=xdata, ydata=ydata, p0=guess, bounds=(min_fit, max_fit)
        )
    ...
    def test_broadcast_y(self):
        ...
        for method in ["lm", "trf", "dogbox"]:
            popt0, pcov0 = curve_fit(
                fit_func, xdata=xdata, ydata=np.zeros_like(xdata), method=method
            )
    ...
    def test_gh4555b(self):
        ...
        ref = [
            [+0.0158972536486215, 0.0069207183284242, -0.0007474400714749],
    ...
    def test_gh20155_error_mentions_x0(self, method):
        ...
        with pytest.raises(ValueError, match=message):
            curve_fit(func, self.x, self.y, p0=[1], bounds=(1000, 1001), method=method)
...
class TestFixedPoint:
    ...
    def test_lambertw(self):
        ...
        xxroot = fixed_point(
            lambda xx: np.exp(-2.0 * xx) / 2.0, 1.0, args=(), xtol=1e-12, maxiter=500
        )
