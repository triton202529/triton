...
def _bracket_minimum(func, x1, x2):
    ...
    step = x2 - x1
    x1, x2, f1, f2, step = (
        (x2, x1, f2, f1, -step) if f2 > f1 else (x1, x2, f1, f2, step)
    )
...
class TestChandrupatlaMinimize:
    ...
    def test_vectorization(self, shape, xp):
        ...
        args = (loc,)
        bracket = xp.asarray(-5.0), xp.asarray(0.0), xp.asarray(5.0)
    ...
    def test_flags(self, xp):
        ...
        bracket = (
            xp.asarray([0] * 4, dtype=xp.float64),
        ...
        ref_flags = xp.asarray(
            [eim._ECONVERGED, eim._ESIGNERR, eim._ECONVERR, eim._EVALUEERR],
            dtype=xp.int32,
        )
    ...
    def test_nit_expected(self, case, xp):
        ...
        res = _chandrupatla_minimize(
            func, *bracket, xatol=xatol, fatol=fatol, xrtol=xrtol, frtol=frtol
        )
    ...
    def test_special_cases(self, xp):
        ...
        assert res.success
        xp_assert_close(res.x, xp.asarray(1.0), rtol=1e-3)
        xp_assert_close(res.fun, xp.asarray(0.0), atol=1e-200)
        ...
        assert res.success
        xp_assert_equal(res.x, xp.asarray(1.0))
...
class TestFindRoot:
    ...
    def test_basic(self, p, xp):
        ...
        a, b = xp.asarray(-5.0), xp.asarray(5.0)
    ...
    def test_vectorization(self, shape, xp):
        ...
        bracket = xp.asarray(-5.0, dtype=xp.float64), xp.asarray(5.0, dtype=xp.float64)
        ...
        assert xp.all(
            (res.x[finite] == res.bracket[0][finite])
        ...
        assert xp.all(
            xp.abs(res.f_x[finite])
            == xp.minimum(
                xp.abs(res.f_bracket[0][finite]), xp.abs(res.f_bracket[1][finite])
            )
        )
    ...
    def test_flags(self, xp):
        ...
        args = (xp.arange(4, dtype=xp.int64),)
        a, b = xp.asarray([0.0] * 4), xp.asarray([xp.pi] * 4)
    ...
    def test_convergence(self, xp):
        ...
        rng = np.random.default_rng(2585255913088665241)
        p = xp.asarray(rng.random(size=3))
        bracket = (-xp.asarray(5.0), xp.asarray(5.0))
        args = (p,)
        ...
        xp_assert_less(
            res1.bracket[1] - res1.bracket[0], xp.full_like(p, xp.asarray(1e-3))
        )
        kwargs["tolerances"]["xatol"] = 1e-6
        res2 = find_root(self.f, bracket, **kwargs)
        xp_assert_less(
            res2.bracket[1] - res2.bracket[0], xp.full_like(p, xp.asarray(1e-6))
        )
        xp_assert_less(
            res2.bracket[1] - res2.bracket[0], res1.bracket[1] - res1.bracket[0]
        )
        ...
        res2 = find_root(self.f, bracket, **kwargs)
        xp_assert_less(res2.bracket[1] - res2.bracket[0], 1e-6 * xp.abs(res2.x))
        xp_assert_less(
            res2.bracket[1] - res2.bracket[0], res1.bracket[1] - res1.bracket[0]
        )
    ...
    def test_nit_expected(self, case, xp):
        ...
        bracket = (
            xp.asarray(bracket[0], dtype=xp.float64),
        ...
        xp_assert_close(
            res.f_x, xp.asarray(f(root), dtype=xp.float64), rtol=1e-8, atol=2e-3
        )
    ...
    def test_special_cases(self, xp):
        ...
        a, b = xp.asarray([0.1, 0.0, 0.0, 0.1]), xp.asarray([0.9, 1.0, 0.9, 1.0])
        ...
        assert res.success
        xp_assert_close(res.x, xp.asarray(1.0))
        ...
        xp_assert_equal(res.x, xp.asarray([xp.nan, 1.0]))
        ...
        a, b = xp.asarray(-3.0), xp.asarray(5.0)
