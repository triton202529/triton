...
class TestScalarRootFinders:
    ...
    def _run_one_test(
        self, tc, method, sig_args_keys=None, sig_kwargs_keys=None, **kwargs
    ):
        method_args = []
    ...
    def run_tests(self, tests, method, name, known_fail=None, **kwargs):
        ...
        notclose = [
            [a] + elt
            for a, c, elt in zip(approx, correct, cvgd)
            if not isclose(a, c, rtol=rtol, atol=atol)
            and elt[-1]["ID"] not in known_fail
        ]
        # If not, evaluate the function and see if is 0 at the purported root
        fvs = [
            tc["f"](aroot, *tc.get("args", tuple()))
    ...
    def run_collection(
        self, collection, method, name, smoothness=None, known_fail=None, **kwargs
    ):
...
class TestBracketMethods(TestScalarRootFinders):
    ...
    def test_basic_individual(self, method, function):
        ...
        a, b = 0.5, sqrt(3)
        root, r = method(
            function, a, b, xtol=self.xtol, rtol=self.rtol, full_output=True
        )
    ...
    def test_chandrupatla_collection(self, method):
        ...
        self.run_collection(
            "chandrupatla", method, method.__name__, known_fail=known_fail
        )
...
class TestNewton(TestScalarRootFinders):
    def test_newton_collections(self):
        ...
        for collection in ["aps", "complex"]:
            self.run_collection(
                collection, zeros.newton, "newton", smoothness=2, known_fail=known_fail
            )
    ...
    def test_halley_collections(self):
        ...
        for collection in ["aps", "complex"]:
            self.run_collection(
                collection, zeros.newton, "halley", smoothness=2, known_fail=known_fail
            )
    ...
    def test_halley_by_name(self):
        ...
        for f, f_1, f_2 in [(f1, f1_1, f1_2), (f2, f2_1, f2_2)]:
            r = root_scalar(
                f, method="halley", x0=3, fprime=f_1, fprime2=f_2, xtol=1e-6
            )
            assert_allclose(f(r.root), 0, atol=1e-6)
    ...
    def test_gh17570_defaults(self):
        ...
        res_secant_default = root_scalar(f, method="secant", x0=3, x1=2, xtol=1e-6)
        ...
        assert (
            res_secant_default.root == res_secant.root != res_newton_default.iterations
        )
        assert (
            res_secant_default.iterations
...
def test_gh_5555():
    ...
    for method in methods:
        ...
        assert_allclose(
            root, res, atol=xtol, rtol=rtol, err_msg=f"method {method.__name__}"
        )
...
class TestRootResults:
    r = zeros.RootResults(
        root=1.0, iterations=44, function_calls=46, flag=0, method="newton"
    )
...
@pytest.mark.parametrize(
    "maximum_iterations,flag_expected", [(10, zeros.CONVERR), (100, zeros.CONVERGED)]
)
...
@pytest.mark.parametrize(
    "solver_name", ["brentq", "brenth", "bisect", "ridder", "toms748"]
)
...
def test_function_calls(solver_name, rs_interface):
    ...
    solver = (
        (lambda f, a, b, **kwargs: root_scalar(f, bracket=(a, b)))
        if rs_interface
...
def test_gh13407():
    ...
    message = rf"rtol too small \({eps/2:g} < {eps:g}\)"
    with pytest.raises(ValueError, match=message):
...
@pytest.mark.parametrize(
    "method", [zeros.bisect, zeros.ridder, zeros.brentq, zeros.brenth]
)
