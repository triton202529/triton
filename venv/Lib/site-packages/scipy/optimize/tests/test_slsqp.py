...
class TestSLSQP:
    ...
    def test_minimize_equality_approximated(self):
        ...
        for jac in jacs:
            res = minimize(
                ...
                args=(-1.0,),
                jac=jac,
                constraints={"type": "eq", "fun": self.f_eqcon, "args": (-1.0,)},
                method="SLSQP",
                options=self.opts,
            )
            assert_(res["success"], res["message"])
    ...
    def test_minimize_inequality_given(self):
        ...
        res = minimize(
            ...
            jac=self.jac,
            args=(-1.0,),
            constraints={"type": "ineq", "fun": self.f_ieqcon, "args": (-1.0,)},
            options=self.opts,
        )
        assert_(res["success"], res["message"])
    ...
    def test_inconsistent_linearization(self):
        ...
        sol = minimize(
            ...
            x,
            constraints=({"type": "eq", "fun": f1}, {"type": "ineq", "fun": f2}),
...
def test_slsqp_segfault_wrong_workspace_computation():
    ...
    def efficient_metric(v, target):
        ...
        constraints = (
            {"type": "eq", "fun": lambda x: metric_a(x) - target},
            {"type": "eq", "fun": lambda x: np.sum(x) - 1},
        )
        weights = np.array([len(v) * [1.0 / len(v)]])[0]
        result = minimize(
            metric_b, weights, args=(v,), method="SLSQP", constraints=constraints
        )
        return result
