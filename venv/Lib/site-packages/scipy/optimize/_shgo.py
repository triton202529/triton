...
class SHGO:
    def __init__(
        ...
        if bnderr.any():
            raise ValueError(
                "Error: lb > ub in bounds " f"{', '.join(str(b) for b in bnderr)}."
            )
        ...
        if constraints is not None:
            ...
            self.constraints = standardize_constraints(
                constraints, np.empty(self.dim, float), "old"
            )
        ...
        if (
            self.minimizer_kwargs["method"].lower()
            in ("slsqp", "cobyla", "cobyqa", "trust-constr")
            and (
    ...
    def finite_ev(self):
        ...
        if self.disp:
            logging.info(
                f"Sampling evaluations done = {self.n_sampled} " f"/ {self.maxev}"
            )
    ...
    def finite_time(self):
        if self.disp:
            logging.info(
                f"Time elapsed = {time.time() - self.init} " f"/ {self.maxtime}"
            )
    ...
    def finite_homology_growth(self):
        ...
        if self.disp:
            logging.info(
                f"Current homology growth = {self.hgrd} "
    ...
    def iterate_hypercube(self):
        ...
        if self.disp:
            logging.info(
                "Constructing and refining simplicial complex graph " "structure"
            )
        if self.n is None:
            self.HC.refine_all()
        ...
        if self.disp:
            logging.info(
                "Triangulation completed, evaluating all constraints "
    ...
    def iterate_delaunay(self):
        ...
        if self.disp:
            ...
            logging.info(
                "Constructing and refining simplicial complex graph "
        ...
        if self.disp:
            logging.info(
                "Triangulation completed, evaluating all "
    ...
    def minimize(self, x_min, ind=None):
        ...
        if self.LMC[x_min].lres is not None:
            logging.info(f"Found self.LMC[x_min].lres = " f"{self.LMC[x_min].lres}")
    ...
    def sampling_subspace(self):
        ...
        for ind, g in enumerate(self.g_cons):
            ...
            feasible = np.array(
                [np.all(g(x_C, *self.g_args[ind]) >= 0.0) for x_C in self.C], dtype=bool
            )
        ...
        if hasattr(self, "Tri") and self.qhull_incremental:
            ...
            try:
                ...
                if str(sys.exc_info()[1])[:6] == "QH6239":
                    logging.warning(
                        "QH6239 Qhull precision error detected, "
                    ...
                    self.Tri = spatial.Delaunay(
                        self.C, incremental=self.qhull_incremental
                    )
                else:
                    raise
