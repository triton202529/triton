...
def minimize(
    ...
    **Custom minimizers**
    ...
    if (
        meth
        not in ("newton-cg", "trust-ncg", "trust-constr", "trust-krylov", "_custom")
    ...
    if meth not in ("cobyla", "cobyqa", "slsqp", "trust-constr", "_custom") and np.any(
        constraints
    ):
        warn(
            f"Method {method} cannot handle constraints.", RuntimeWarning, stacklevel=2
        )
    if (
        meth
        not in (
        ...
        warn(f"Method {method} cannot handle bounds.", RuntimeWarning, stacklevel=2)
    # - return_all
    if meth in ("l-bfgs-b", "tnc", "cobyla", "cobyqa", "slsqp") and options.get(
        "return_all", False
    ):
        warn(
            f"Method {method} does not support the return_all option.",
            RuntimeWarning,
            stacklevel=2,
        )
            ...
            if remove_vars:
                ...
                for con in constraints:  # yes, guaranteed to be a list
                    ...
                    if callable(con.get("jac", None)):
                        con["jac"] = _Remove_From_Func(
                            con["jac"], i_fixed, x_fixed, min_dim=2, remove=1
                        )
    ...
    if meth == "nelder-mead":
        ...
        res = _minimize_cobyla(
            fun, x0, args, constraints, callback=callback, bounds=bounds, **options
        )
    elif meth == "cobyqa":
        res = _minimize_cobyqa(fun, x0, args, bounds, constraints, callback, **options)
    elif meth == "slsqp":
        res = _minimize_slsqp(
            fun, x0, args, jac, bounds, constraints, callback=callback, **options
        )
    elif meth == "trust-constr":
        res = _minimize_trustregion_constr(
            fun,
            x0,
            args,
            jac,
        ...
        res = _minimize_trust_krylov(
            fun, x0, args, jac, hess, hessp, callback=callback, **options
        )
    elif meth == "trust-exact":
        res = _minimize_trustregion_exact(
...
def minimize_scalar(
    fun, bracket=None, bounds=None, args=(), method=None, tol=None, options=None
):
    ...
    **Custom minimizers**
    ...
    if meth == "_custom":
        ...
        res = _recover_from_bracket_error(
            _minimize_scalar_brent, fun, bracket, args, **options
        )
    elif meth == "bounded":
        if bounds is None:
            raise ValueError(
        ...
        res = _recover_from_bracket_error(
            _minimize_scalar_golden, fun, bracket, args, **options
        )
    else:
        raise ValueError(f"Unknown solver {method}")
...
def _optimize_result_for_equal_bounds(fun, bounds, method, args=(), constraints=()):
    ...
    if constraints:
        message = (
            "All independent variables were fixed by bounds at values"
    ...
    for c in constraints:
        ...
        if np.sum(violation):
            ...
            success = False
            message = (
                f"All independent variables were fixed by bounds, but "
