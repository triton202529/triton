...
def approx_derivative(
    ...
    if as_linear_operator:
        ...
        J, _ = _linear_operator_difference(fun_wrapped, x0, f0, rel_step, method)
    else:
        ...
        with MapWrapper(workers) as mf:
            if sparsity is None:
                J, _nfev = _dense_difference(
                    fun_wrapped, x0, f0, h, use_one_sided, method, mf
                )
            else:
                ...
                J, _nfev = _sparse_difference(
                    fun_wrapped, x0, f0, h, use_one_sided, structure, groups, method, mf
                )
...
def _sparse_difference(
    fun, x0, f0, h, use_one_sided, structure, groups, method, workers
):
    m = f0.size
    n = x0.size
    row_indices = []
...
def check_derivative(fun, jac, x0, bounds=(-np.inf, np.inf), args=(), kwargs=None):
