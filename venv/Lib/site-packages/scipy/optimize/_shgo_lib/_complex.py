...
class Complex:
    ...
    def __init__(
        ...
        if (sfield is not None) or (self.g_cons is not None):
            ...
            if sfield is not None:
                self.V = VertexCacheField(
                    field=sfield,
                    field_args=sfield_args,
                    g_cons=self.g_cons,
    ...
    def triangulate(self, n=None, symmetry=None, centroid=True, printout=False):
        ...
        if n is None:
            ...
            try:
                self.triangulated_vectors.append(
                    (tuple(self.origin), tuple(self.supremum))
                )
    ...
    def refine(self, n=1):
        if n is None:
            try:
                ...
                if (
                    str(ae) == "'Complex' object has no attribute "
    ...
    def connect_vertex_non_symm(self, v_x, near=None):
        ...
        for s_i in itertools.combinations(range(S_rows.shape[0]), r=self.dim + 1):
            ...
            for i in itertools.combinations(s_i, r=2):
                ...
                if (
                    self.V[tuple(S_rows[i[1]])] not in self.V[tuple(S_rows[i[0]])].nn
                ) and (
                    self.V[tuple(S_rows[i[0]])] not in self.V[tuple(S_rows[i[1]])].nn
                ):
                    valid_simplex = False
                    break
