...
class TestCDFlib:
    ...
    def test_gdtria(self):
        _assert_inverts(
            ...
            0,
            [ProbArg(), Arg(0, 1e3, inclusive_a=False), Arg(0, 1e4, inclusive_a=False)],
            rtol=1e-7,
    ...
    def test_gdtrib(self):
        ...
        _assert_inverts(
            ...
            lambda a, b, x: mpmath.gammainc(b, b=a * x, regularized=True),
            1,
            [Arg(0, 1e2, inclusive_a=False), ProbArg(), Arg(0, 1e3, inclusive_a=False)],
            rtol=1e-5,
        )
    ...
    def test_gdtrix(self):
        _assert_inverts(
            ...
            lambda a, b, x: mpmath.gammainc(b, b=a * x, regularized=True),
            2,
            [Arg(0, 1e3, inclusive_a=False), Arg(0, 1e3, inclusive_a=False), ProbArg()],
            rtol=1e-7,
            endpt_atol=[None, 1e-7, 1e-10],
        )
    ...
    def test_stdtr(self):
        ...
        assert_mpmath_equal(
            sp.stdtr, _student_t_cdf, [IntArg(1, 100), Arg(1e-10, np.inf)], rtol=1e-7
        )

    @pytest.mark.xfail(run=False)
    def test_stdtridf(self):
        _assert_inverts(sp.stdtridf, _student_t_cdf, 0, [ProbArg(), Arg()], rtol=1e-7)
    ...
    def test_chndtridf(self):
        ...
        _assert_inverts(
            ...
            _noncentral_chi_cdf,
            1,
            [Arg(0, 100, inclusive_a=False), ProbArg(), Arg(0, 100, inclusive_a=False)],
            n=1000,
            rtol=1e-4,
            atol=1e-15,
        )
    ...
    def test_chndtrinc(self):
        ...
        _assert_inverts(
            ...
            _noncentral_chi_cdf,
            2,
            [Arg(0, 100, inclusive_a=False), IntArg(1, 100), ProbArg()],
            n=1000,
            rtol=1e-4,
            atol=1e-15,
        )
    ...
    def test_chndtrix(self):
        ...
        _assert_inverts(
            ...
            _noncentral_chi_cdf,
            0,
            [ProbArg(), IntArg(1, 100), Arg(0, 100, inclusive_a=False)],
            n=1000,
            rtol=1e-4,
            atol=1e-15,
    ...
    def test_tklmbda_neg_shape(self):
        _assert_inverts(
            ...
            0,
            [ProbArg(), Arg(-25, 0, inclusive_b=False)],
            spfunc_first=False,
            rtol=1e-5,
...
def test_chndtrix_gh2158():
    ...
    res = sp.chndtrix(0.999999, 2, np.arange(20.0) + 1e-6)
...
def test_bdtrik_nbdtrik_inf():
    y = np.array(
        [np.nan, -np.inf, -10.0, -1.0, 0.0, 0.00001, 0.5, 0.9999, 1.0, 10.0, np.inf]
    )
    y = y[:, None]
    p = np.atleast_2d([np.nan, -np.inf, -10.0, -1.0, 0.0, 0.00001, 0.5, 1.0, np.inf])
...
class TestNoncentralTFunctions:
    ...
    @pytest.mark.parametrize(
        ...
        [
            [3.0, 5.0, -2.0, 1.5645373999149622e-09, 5e-9],
            [1000.0, 10.0, 1.0, 1.1493552133826623e-19, 1e-13],
            [1e-5, -6.0, 2.0, 0.9999999990135003, 1e-13],
            [10.0, 20.0, 0.15, 6.426530505957303e-88, 1e-13],
            [1.0, 1.0, np.inf, 1.0, 0.0],
            [1.0, 1.0, -np.inf, 0.0, 0.0],
        ],
    )
