...
from ._multiufuncs import assoc_legendre_p_all, legendre_p_all
...
def lqmn(m, n, z):
    ...
    if z.ndim == 0:
        ...
        _lqmn(
            z, out=(np.moveaxis(q, (0, 1), (-2, -1)), np.moveaxis(qd, (0, 1), (-2, -1)))
        )
...
def comb(N, k, *, exact=False, repetition=False):
    ...
    if exact:
        if int(N) == N and int(k) == k:
            ...
            raise ValueError(
                "Non-integer `N` and `k` with `exact=True` is not " "supported."
            )
    else:
...
def _factorialx_wrapper(fname, n, k, exact, extend):
    ...
    if fname == "factorial2":
        msg_needs_complex += (
            " Additionally, it will rescale the values of the double"
            " factorial at even integers by a factor of sqrt(2/pi)."
        )
    elif fname == "factorialk":
        msg_needs_complex += (
            " Additionally, it will perturb the values of the"
...
def stirling2(N, K, *, exact=False):
    ...
    nk_pairs = list(
        set([(n.take(0), k.take(0)) for n, k in np.nditer([N, K], ["refs_ok"])])
    )
    heapify(nk_pairs)
