...
from scipy._lib._array_api import is_numpy, xp_size, xp_assert_close, xp_assert_equal
...
def get_expected_input_dtype(func, xp):
    if func in [
        ...
        dtype = xp.complex128
    elif func in [fft.rfft, fft.rfftn, fft.rfft2, fft.ihfft, fft.ihfftn, fft.ihfft2]:
        dtype = xp.float64
    else:
...
class TestFFT:
    ...
    def test_ifftn(self, xp):
        ...
        xp_assert_close(
            fft.ifftn(x, norm="forward"), expect * (30 * 20 * 10), rtol=1e-7
        )
        ...
        for n in [xp_size(x), 2 * xp_size(x)]:
            for norm in [None, "backward", "ortho", "forward"]:
                xp_assert_close(
                    ...
                    fft.fft(xp.asarray(x, dtype=xp.complex128), n=n, norm=norm)[
                        : (n // 2 + 1)
                    ],
                )
            xp_assert_close(
...
@pytest.mark.parametrize(
    "fft", [fft.fft, fft.fft2, fft.fftn, fft.ifft, fft.ifft2, fft.ifftn]
)
