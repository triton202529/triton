...
class HBInfo:
    ...
    def from_data(cls, m, title="Default title", key="0", mxtype=None, fmt=None):
        ...
        return cls(
            ...
            ncols,
            nnon_zeros,
            pointer_fmt.fortran_format,
            indices_fmt.fortran_format,
    ...
    def from_file(cls, fid):
        ...
        if not len(line) > 72:
            raise ValueError(
                "Expected at least 72 characters for first line, " f"got: \n{line}"
            )
        title = line[:72]
        key = line[72:]
        ...
        if not len(line.rstrip()) >= 56:
            raise ValueError(
                "Expected at least 56 characters for second line, " f"got: \n{line}"
            )
        ...
        if not len(line) >= 70:
            raise ValueError(
                f"Expected at least 72 character for third line, " f"got:\n{line}"
            )
        ...
        if mxtype.value_type not in ["real", "integer"]:
            raise ValueError(
                "Only real or integer matrices supported for "
                f"now (detected {mxtype})"
            )
        if not mxtype.structure == "unsymmetric":
            raise ValueError(
                "Only unsymmetric matrices supported for " f"now (detected {mxtype})"
            )
    ...
    def __init__(
        ...
        if len(key) > 8:
            warnings.warn(
                f"key is > 8 characters (key is {key})", LineOverflow, stacklevel=3
            )
        self.title = title
        self.key = key
        ...
        if not isinstance(pointer_format, IntFormat):
            raise ValueError(
                "Expected int format for pointer format, got " f"{pointer_format}"
            )

        indices_format = parser.parse(indices_format_str)
        if not isinstance(indices_format, IntFormat):
            raise ValueError(
                "Expected int format for indices format, got " f"{indices_format}"
            )
        ...
        if isinstance(values_format, ExpFormat):
            if mxtype.value_type not in ["real", "complex"]:
                raise ValueError(
                    f"Inconsistency between matrix type {mxtype} and "
    ...
    def dump(self):
        ...
        header.append(
            f"{self.total_nlines:14d}{self.pointer_nlines:14d}{self.indices_nlines:14d}{self.values_nlines:14d}"
        )
        header.append(
            f"{self.mxtype.fortran_format.ljust(14):14s}{self.nrows:14d}{self.ncols:14d}{self.nnon_zeros:14d}{0:14d}"
        )
        ...
        header.append(
            f"{pffmt.ljust(16):16s}{iffmt.ljust(16):16s}{vffmt.ljust(20):20s}"
        )
        return "\n".join(header)
...
class HBMatrixType:
    ...
    def from_fortran(cls, fmt):
        if not len(fmt) == 3:
            raise ValueError(
                "Fortran format for matrix type should be 3 " "characters long"
            )
        try:
