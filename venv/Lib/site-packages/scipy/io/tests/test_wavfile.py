...
def test_read_4():
    ...
    for mmap in [False, True]:
        with suppress_warnings() as sup:
            sup.filter(
                wavfile.WavFileWarning, "Chunk .non-data. not understood, skipping it"
            )
...
def test_read_5():
    ...
    for mmap in [False, True]:
        ...
        assert_(
            ...
            or (sys.byteorder == "big" and data.dtype.byteorder == "=")
        )
        assert_equal(data.shape, (441, 2))
...
def test_36_bit_odd_size():
    ...
    correct = [
        [-0x8000_0000_0000_0000, -0x7FFF_FFFF_F000_0000, -0x2000_0000],
...
def test_45_bit_even_size():
    ...
    #            Fixed-point 2**(N-1)    Full-scale 2**(N-1)-1      LSB
    correct = [
        [-0x8000_0000_0000_0000, -0x7FFF_FFFF_FFF8_0000, -0x10_0000],
...
def test_53_bit_odd_size():
    ...
    #            Fixed-point 2**(N-1)    Full-scale 2**(N-1)-1    LSB
    correct = [
        [-0x8000_0000_0000_0000, -0x7FFF_FFFF_FFFF_F800, -0x1000],
...
def test_64_bit_even_size():
    ...
    for mmap in [False, True]:
        ...
        #            Fixed-point 2**(N-1)    Full-scale 2**(N-1)-1   LSB
        correct = [
            [-0x8000_0000_0000_0000, -0x7FFF_FFFF_FFFF_FFFF, -0x2],
...
def test_rifx():
    ...
    for rifx, riff in {
        ("test-44100Hz-be-1ch-4bytes.wav", "test-44100Hz-le-1ch-4bytes.wav"),
        ("test-8000Hz-be-3ch-5S-24bit.wav", "test-8000Hz-le-3ch-5S-24bit.wav"),
    }:
...
def test_rf64():
    ...
    for rf64, riff in {
        ("test-44100Hz-le-1ch-4bytes-rf64.wav", "test-44100Hz-le-1ch-4bytes.wav"),
        ("test-8000Hz-le-3ch-5S-24bit-rf64.wav", "test-8000Hz-le-3ch-5S-24bit.wav"),
    }:
...
def test_read_unknown_wave_format():
    ...
    for mmap in [False, True]:
        ...
        with open(datafile(filename), "rb") as fp:
            with raises(
                ValueError,
                match="Unknown wave file format.*MULAW.*" "Supported formats",
            ):
                wavfile.read(fp, mmap=mmap)
...
@pytest.mark.parametrize(
    "dt_str",
    ["<i2", "<i4", "<i8", "<f4", "<f8", ">i2", ">i4", ">i8", ">f4", ">f8", "|u1"],
)
