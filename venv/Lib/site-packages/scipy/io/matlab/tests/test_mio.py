...
case_table4.append(
    {
        ...
        "expected": {
            "teststring": array(['"Do nine men interpret?" "Nine men," I nod.'])
        },
    }
)
case_table4.append(
...
CA = mlarr(
    (  # tuple for object array creation
        [],
        mlarr([1]),
        mlarr([[1, 2]]),
        mlarr([[1, 2, 3]]),
...
case_table5 = [
    {"name": "cell", "classes": {"testcell": "cell"}, "expected": {"testcell": CA}}
]
CAE = mlarr(
    (  # tuple for object array creation
        mlarr(1),
...
case_table5.append(
    {
        ...
        "expected": {"teststringarray": array(["one  ", "two  ", "three"])},
    }
)
case_table5.append(
    {
...
ODT = np.dtype(
    [
        (n, object)
        for n in ["expr", "inputExpr", "args", "isEmpty", "numArgs", "version"]
    ]
)
...
case_table5.append(
    {
        ...
        "expected": {"testbools": array([[True], [False]])},
    }
)
...
def _check_level(label, expected, actual):
    ...
    if issparse(expected):  # allow different types of sparse matrices
        ...
        assert_array_almost_equal(
            actual.toarray(), expected.toarray(), err_msg=label, decimal=5
        )
        return
    ...
    if ex_dtype.fields:  # probably recarray
        ...
        return
    if ex_dtype.type in (str, np.str_, np.bool_):  # string or bool
...
def test_gzip_simple():
    ...
    assert_array_almost_equal(
        actual["x"].toarray(), expected["x"].toarray(), err_msg=repr(actual)
    )
...
def test_to_writeable():
    ...
    with pytest.warns(MatWriteWarning, match="Starting field name with"):
        assert_equal(
            to_writeable({"1": 1, "f": 2}), np.array([(2,)], dtype=[("f", "|O8")])
        )
...
def test_corrupted_data():
    ...
    for exc, fname in [
        (ValueError, "corrupted_zlib_data.mat"),
...
def test_read_both_endian():
    ...
    for fname in ("big_endian.mat", "little_endian.mat"):
        ...
        assert_array_equal(
            d["floats"], np.array([[2.0, 3.0], [3.0, 4.0]], dtype=np.float32)
        )
...
def test_str_round():
    ...
    in_from_str = np.ndarray(
        shape=a.shape, dtype=in_arr.dtype, order="F", buffer=in_str
    )
...
def test_gh_17992(tmp_path):
    ...
    savemat(
        outfile, {"data": list_of_arrays}, long_field_names=True, do_compression=True
    )
    # round trip check
    new_dict = {}
...
def test_large_m4():
    ...
    match = (
        "Not enough bytes to read matrix 'a';"
        if np.intp == np.int64
        else "Variable 'a' has byte length longer than largest possible"
    )
...
def test_invalid_field_name_warning():
    names_vars = (("_1", mlarr(np.arange(10))), ("mystr", mlarr("a string")))
...
def test_corrupt_files():
    ...
    for n in (2, 4, 10, 19):
        with pytest.raises(MatReadError, match="Mat file appears to be truncated"):
            loadmat(BytesIO(b"\x00" * n))
    with pytest.raises(MatReadError, match="Mat file appears to be corrupt"):
