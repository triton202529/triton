...
class VarReader4:
    ...
    def read_header(self):
        ...
        if M not in (0, 1):
            warnings.warn(
                f"We do not support byte ordering '{order_codes[M]}';"
    ...
    def read_char_array(self, hdr):
        ...
        return np.ndarray(
            shape=hdr.dims, dtype=np.dtype("U1"), buffer=np.array(S)
        ).copy()
    ...
    def shape_from_header(self, hdr):
        ...
        if mclass == mxFULL_CLASS:
            ...
            rows = np.ndarray(
                shape=(), dtype=dt, buffer=self.mat_stream.read(dt.itemsize)
            )
            self.mat_stream.seek(dt.itemsize * (dims[0] - 1), 1)
            cols = np.ndarray(
                shape=(), dtype=dt, buffer=self.mat_stream.read(dt.itemsize)
...
def arr_to_2d(arr, oned_as="row"):
    ...
    if len(dims) > 2:
        raise ValueError(
            "Matlab 4 files cannot save arrays with more than " "2 dimensions"
        )
...
class VarWriter4:
    ...
    def write_char(self, arr, name):
        ...
        if arr.dtype.kind == "U":
            ...
            st_arr = np.ndarray(
                shape=(), dtype=arr_dtype_number(arr, n_chars), buffer=arr
            )
