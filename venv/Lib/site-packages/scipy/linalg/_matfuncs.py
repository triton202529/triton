...
def expm(A):
    ...
    for ind in product(*[range(x) for x in a.shape[:-2]]):
        ...
        if m < 0:
            raise MemoryError(
                "scipy.linalg.expm could not allocate sufficient"
        ...
        if info != 0:
            if info <= -11:
                ...
                raise MemoryError(
                    "scipy.linalg.expm could not allocate "
                ...
                raise RuntimeError(
                    "scipy.linalg.expm got an internal LAPACK "
        ...
        if s != 0:  # squaring needed
            ...
            if (lu[1] == 0) or (lu[0] == 0):  # lower/upper triangular
                ...
                for i in range(s - 1, -1, -1):
                    ...
                    # diagonal
                    np.einsum("ii->i", eAw)[:] = np.exp(diag_aw * 2.0 ** (-i))
                    exp_sd = _exp_sinch(diag_aw * (2.0 ** (-i))) * (sd * 2 ** (-i))
                    if lu[1] == 0:  # lower
...
def sqrtm(A, disp=_NoValue, blocksize=_NoValue):
    ...
    if blocksize is not _NoValue:
        warnings.warn(
            "The `blocksize` argument is deprecated and will be removed in "
            "SciPy 1.18.0.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    if a.dtype.char not in "fdFD":
        raise TypeError(
            "scipy.linalg.sqrtm is not supported for the data type" f" {a.dtype}"
        )
    ...
    if isSingular or isIllconditioned:
        if isSingular:
            msg = (
                "Matrix is singular. The result might be inaccurate or the"
                " array might not have a square root."
            )
        else:
            msg = (
                "Matrix is ill-conditioned. The result might be inaccurate"
