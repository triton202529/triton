...
def _cholesky(a, lower=False, overwrite_a=False, clean=True, check_finite=True):
    ...
    if a1.shape[0] != a1.shape[1]:
        raise ValueError(
            "Input array is expected to be square but has " f"the shape: {a1.shape}."
        )
...
def _cho_solve(c, b, lower, overwrite_b, check_finite):
    ...
    if b1.size == 0:
        dt = cho_solve(
            (np.eye(2, dtype=b1.dtype), True), np.ones(2, dtype=c.dtype)
        ).dtype
...
def cho_solve_banded(cb_and_lower, b, overwrite_b=False, check_finite=True):
    ...
    return _cho_solve_banded(
        cb, b, lower, overwrite_b=overwrite_b, check_finite=check_finite
    )
