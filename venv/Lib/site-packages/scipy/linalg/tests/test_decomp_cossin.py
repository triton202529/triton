...
def test_cossin_error_invalid_shape():
    ...
    with pytest.raises(
        ValueError, match=r"Invalid x12 dimensions: desired \(3, 4\), got \(3, 6\)"
    ):
    ...
    with pytest.raises(
        ValueError, match=r"Invalid x21 dimensions: desired \(3, 4\), got \(1, 5\)"
    ):
...
def test_properties(m, p, q, swap_sign):
    ...
    u, cs, vh = linalg.cossin(
        X, separate=True, compute_u=False, compute_vh=False, **kwargs
    )
