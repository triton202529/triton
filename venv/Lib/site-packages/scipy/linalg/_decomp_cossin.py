...
def cossin(
    ...
    if p or q:
        ...
        raise ValueError(
            "When p and q are None, X must be an Iterable"
...
def _cossin(x11, x12, x21, x22, separate, swap_sign, compute_u, compute_vh):
    ...
    if x12.shape != (p, mmq):
        raise ValueError(
            f"Invalid x12 dimensions: desired {(p, mmq)}, " f"got {x12.shape}"
        )

    if x21.shape != (mmp, q):
        raise ValueError(
            f"Invalid x21 dimensions: desired {(mmp, q)}, " f"got {x21.shape}"
        )
    ...
    *_, theta, u1, u2, v1h, v2h, info = csd(
    ...
    if info < 0:
        raise ValueError(
            f"illegal value in argument {-info} " f"of internal {method_name}"
        )
    if info > 0:
