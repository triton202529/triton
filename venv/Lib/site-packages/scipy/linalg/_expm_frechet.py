...
def _diff_pade7(A, E, ident):
    ...
    Lu = A.dot(b[7] * M6 + b[5] * M4 + b[3] * M2) + E.dot(
...
def _diff_pade9(A, E, ident):
    ...
    Lu = A.dot(b[9] * M8 + b[7] * M6 + b[5] * M4 + b[3] * M2) + E.dot(
        b[9] * A8 + b[7] * A6 + b[5] * A4 + b[3] * A2 + b[1] * ident
    )
...
def expm_frechet_kronform(A, method=None, check_finite=True):
    ...
    for i in range(n):
        for j in range(n):
            ...
            F = expm_frechet(
                A, E, method=method, compute_expm=False, check_finite=False
            )
            cols.append(vec(F))
