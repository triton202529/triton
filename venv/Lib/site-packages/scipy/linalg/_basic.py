...
def _solve_check(n, info, lamch=None, rcond=None):
    ...
    if not (rcond >= E):  # `rcond < E` doesn't handle NaN
        warn(
            f"Ill-conditioned matrix (rcond={rcond:.6g}): "
...
def solve(
    ...
    if assume_a == "diagonal":
        ...
        a2, n_below, n_above = (
            (a1.T, n_above, n_below) if transposed else (a1, n_below, n_above)
        )
    ...
    # Banded case
        ...
        a2[n_below:, :] = ab
        lu, piv, x, info = gbsv(
            n_below, n_above, a2, b1, overwrite_ab=True, overwrite_b=overwrite_b
        )
        _solve_check(n, info)
    ...
    # Triangular case
        ...
        x, info = _solve_triangular(
            a1, b1, lower=lower, overwrite_b=overwrite_b, trans=transposed
        )
        _solve_check(n, info)
...
def _solve_triangular(
    a1, b1, trans=0, lower=False, unit_diagonal=False, overwrite_b=False
):
...
def solve_banded(
    l_and_u, ab, b, overwrite_ab=False, overwrite_b=False, check_finite=True
):
    """
...
def solve_circulant(c, b, singular="raise", tol=None, caxis=-1, baxis=0, outaxis=0):
    ...
    if b.size == 0:
        dt = solve_circulant(
            np.arange(3, dtype=c.dtype), np.ones(3, dtype=b.dtype)
        ).dtype
...
def det(a, overwrite_a=False, check_finite=True):
    ...
    if a1.dtype.char not in "fdFD":
        ...
        if not dtype_char:  # No casting possible
            raise TypeError(
                f'The dtype "{a1.dtype.name}" cannot be cast '
...
def lstsq(
    ...
    lapack_func, lapack_lwork = get_lapack_funcs((driver, f"{driver}_lwork"), (a1, b1))
...
def _validate_args_for_toeplitz_ops(
    ...
    if c.ndim > 1 or r.ndim > 1:
        msg = (
            "Beginning in SciPy 1.17, multidimensional input will be treated as a "
