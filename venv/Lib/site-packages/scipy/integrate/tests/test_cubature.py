...
def genz_malik_1980_f_5_random_args(rng, shape, xp):
    ...
    difficulty = 21.0
    normalisation_factors = xp.sqrt(xp.sum(alphas ** xp.asarray(2.0), axis=-1))[
        ..., None
    ]
...
class TestCubature:
    ...
    def test_pass_array_like_not_array(self, xp):
        ...
        res = cubature(basic_1d_integrand, a, b, args=(n, xp))
...
class TestCubatureProblems:
    ...
    @skip_xp_backends(
        "dask.array", reason="Dask hangs/takes a long time for some test cases"
    )
    @pytest.mark.parametrize(
        "problem",
        [
        ...
        [
            ...
            pytest.param(
                (
                    ...
                    # Constant arguments
                    lambda rng, shape, xp: (
                        xp.asarray([0, 1, 2, 3, 4], dtype=xp.float64),
                    ),
                    tuple(),
                    [0, 0, -math.inf, -math.inf],
...
class TestRulesQuadrature:
    ...
    @pytest.mark.parametrize(
        ("rule_pair", "rule_pair_args"),
        [
            ((GaussLegendreQuadrature, GaussLegendreQuadrature), (10, 5)),
        ],
    )
    def test_base_1d_quadratures_error_from_difference(
        self, rule_pair, rule_pair_args, xp
    ):
        n = xp.arange(5, dtype=xp.float64)
        a = xp.asarray([0], dtype=xp.float64)
        b = xp.asarray([2], dtype=xp.float64)
    ...
    @pytest.mark.parametrize("quadrature", [GaussLegendreQuadrature])
