...
class TestTanhSinh:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [(-math.inf, math.inf), 1.0],  # a and b infinite
            [(math.inf, -math.inf), -1.0],  # flipped limits
            [(1, -1), stats.norm.cdf(-1.0) - stats.norm.cdf(1.0)],  # flipped limits
        ],
    )
    def test_integral_transforms(self, limits, ref, xp):
        ...
        assert (
            xp.isdtype(logres.integral.dtype, "real floating")
            if ref > 0
    ...
    @pytest.mark.skip_xp_backends(
        np_only=True, reason="Cumbersome to convert everything."
    )
    ...
    @pytest.mark.skip_xp_backends(
        np_only=True, reason="Distributions aren't xp-compatible."
    )
    ...
    def test_flags_preserve_shape(self, xp):
        ...
        def f(x):
            res = [
                xp.exp(-x[0] ** 2),  # converges
                xp.exp(x[1]),  # reaches maxiter due to order=2
                xp.full_like(x[2], xp.nan),
            ]  # stops due to NaN
            return xp.stack(res)
    ...
    def test_preserve_shape(self, xp):
        ...
        res = _tanhsinh(
            lambda x: f(x, xp), xp.asarray(0), xp.asarray(1), preserve_shape=True
        )
        dtype = xp.asarray(0.0).dtype
    ...
    def test_convergence(self, xp):
        ...
        a, b = xp.asarray(0.0, dtype=dtype), xp.asarray(1.0, dtype=dtype)
    ...
    def test_options_and_result_attributes(self, xp):
        ...
        b = xp.asarray(1.0, dtype=xp.float64)
    ...
    @pytest.mark.skip_xp_backends(
        "torch",
        reason="https://github.com/scipy/scipy/pull/21149#issuecomment-2330477359",
    )
    ...
    def test_log(self, rtol, xp):
        ...
        a = xp.asarray(-1.0, dtype=xp.float64)
        b = xp.asarray(2.0, dtype=xp.float64)
        ...
        with np.errstate(all="ignore"):
            xp_assert_close(
                xp.exp(res.integral), ref.integral, **test_tols, check_dtype=False
            )
            xp_assert_close(
                xp.exp(res.error), ref.error, **test_tols, check_dtype=False
            )
        assert res.nfev == ref.nfev
    ...
    def test_complex(self, xp):
        ...
        a, b = xp.asarray(0.0), xp.asarray(xp.pi / 4)
        res = _tanhsinh(f, a, b)
    ...
    def test_special_cases(self, xp):
        ...
        xp_assert_close(res.integral, xp.asarray(0.0))
    ...
    def test_gh_22681_finite_error(self, xp):
        ...
        a, b = xp.asarray(0.0, dtype=xp.float64), xp.asarray(1.0, dtype=xp.float64)
...
@pytest.mark.skip_xp_backends(
    "dask.array", reason="Data-dependent shapes in boolean index assignment"
)
class TestNSum:
    ...
    def test_input_validation(self, xp):
        ...
        with np.errstate(all="ignore"):
            res = nsum(f, xp.asarray([np.nan, np.inf]), xp.asarray(1.0))
            ...
            res = nsum(f, xp.asarray(10.0), xp.asarray([np.nan, 1]))
            assert (res.status[0] == -1) and not res.success[0]
            assert xp.isnan(res.sum[0]) and xp.isnan(res.error[0])
            assert (res.status[1] == 0) and res.success[1]
            assert res.sum[1] == res.error[1]
    ...
    def test_basic(self, f_number, xp):
        ...
        with np.errstate(divide="ignore"):
            logres = nsum(lambda *args: xp.log(f(*args)), a, b, log=True, args=args)
    ...
    def test_integral(self, maxterms, xp):
        ...
        rtol = 1e-12
        res = nsum(
            f, a, b_original, step=step, maxterms=maxterms, tolerances=dict(rtol=rtol)
        )
        ...
        logres = nsum(
            ...
            step=step,
            log=True,
            tolerances=dict(rtol=math.log(rtol)),
            maxterms=maxterms,
        )
        xp_assert_close(xp.exp(logres.sum), res.sum)
    ...
    def test_vectorization(self, shape, xp):
        ...
        res = nsum(
            f, xp.asarray(a), xp.asarray(b), maxterms=1000, args=(xp.asarray(p),)
        )
    ...
    def test_inclusive(self, xp):
        ...
        a = xp.asarray([1, 4])
        b = xp.asarray(xp.inf)
        res = nsum(lambda k: 1 / k**2, a, b, maxterms=500, tolerances=dict(atol=0.1))
