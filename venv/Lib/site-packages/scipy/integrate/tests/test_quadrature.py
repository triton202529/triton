...
class TestTrapezoid:
    def test_simple(self, xp):
        x = xp.arange(-10, 10, 0.1)
        r = trapezoid(xp.exp(-0.5 * x**2) / xp.sqrt(2 * xp.asarray(xp.pi)), dx=0.1)
    ...
    def test_gh21908(self, xp):
        ...
        xp_assert_close(trapezoid(y, x=xp.asarray([0, 10.0, 20.0]), axis=0), out0)
        # x needs to be broadcastable against y
        xp_assert_close(
            trapezoid(y, x=xp.asarray([0, 10.0, 20.0])[:, None], axis=0), out0
        )
        with pytest.raises(Exception):
            # x is not broadcastable against y
            trapezoid(y, x=xp.asarray([0, 10.0, 20.0])[None, :], axis=0)

        out1 = xp.asarray([40.5, 130.5, 220.5])
        xp_assert_close(trapezoid(y, x=x, axis=1), out1)
        xp_assert_close(trapezoid(y, x=xp.linspace(0, 9, 10), axis=1), out1)
    ...
    @skip_xp_backends(
        np_only=True, reason="array-likes only supported for NumPy backend"
    )
...
class TestQMCQuad:
    ...
    def basic_test(self, n_points=2**8, n_estimates=8, signs=None):
        ...
        res = qmc_quad(
            func, a, b, n_points=n_points, n_estimates=n_estimates, qrng=qrng
        )
        ...
        assert_allclose(
            np.exp(logres.standard_error), res.standard_error, rtol=1e-14, atol=1e-16
        )
...
class TestCumulativeSimpson:
    ...
    def test_nd(self, axis, x_ndim, x_len, i_ndim, dx):
        ...
        if dx:
            ...
            x = (
                np.sort(rng.random(size=shape), axis=axis)
                if x_ndim > 1
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                dict(x=[x0, x0, x0], y=[y0, y0, y0], axis=0),
            ),
    ...
    @given(
        y=hyp_num.arrays(
            ...
            elements=st.floats(-10, 10, allow_nan=False).filter(
                lambda x: abs(x) > 1e-7
            ),
        )
    )
    def test_cumulative_simpson_against_simpson_with_default_dx(self, y):
        ...
        def simpson_reference(y):
            return np.stack(
                [simpson(y[..., :i], dx=1.0) for i in range(2, y.shape[-1] + 1)],
                axis=-1,
            )
