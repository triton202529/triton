...
def quad(
    ...
    **Integration of Complex Function of a Real Variable**
    ...
    if weight in ["cos", "sin"] and (b == np.inf or a == -np.inf):
        ...
        explain = {
            1: "The maximum number of subdivisions (= limit) has been \n  "
    ...
    if ier in [1, 2, 3, 4, 5, 7]:
        ...
        if epsabs <= 0:  # Small error tolerance - applies to all methods
            if epsrel < max(50 * sys.float_info.epsilon, 5e-29):
                ...
                msg = (
                    "Sine or cosine weighted integrals with infinite domain"
                    " must have 'epsabs'>0."
                )
            ...
            if points is None:  # QAGSE/QAGIE
                msg = (
                    "Invalid 'limit' argument. There must be"
                ...
                if not (min(a, b) <= min(points) <= max(points) <= max(a, b)):
                    ...
                    msg = (
                        f"Number of break points ({len(points):d}) "
            ...
            if maxp1 < 1:
                ...
                msg = (
                    "Parameter 'wvar' must not equal" " integration limits 'a' or 'b'."
                )
