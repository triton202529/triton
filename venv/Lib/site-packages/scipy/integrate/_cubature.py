...
from scipy._lib._array_api import array_namespace, xp_size, xp_copy, xp_promote
...
def cubature(
    ...
    **1D integral with singularities avoided using** `points`:
    ...
    a, b, *points = xp_promote(
        a, b, *points, broadcast=True, force_floating=True, xp=xp
    )
...
class _InfiniteLimitsTransform(_VariableTransform):
    ...
    def __call__(self, t, *args, **kwargs):
        ...
        x[double_inf_mask] = (1 - self._xp.abs(t[double_inf_mask])) / t[double_inf_mask]
        ...
        jacobian_det = self._xp.reshape(
            jacobian_det, (-1, *([1] * (len(f_x.shape) - 1)))
        )
