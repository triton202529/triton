...
class ProductNestedFixed(NestedFixedRule):
    ...
    def __init__(self, base_rules):
        for rule in base_rules:
            if not isinstance(rule, NestedFixedRule):
                raise ValueError(
                    "base rules for product need to be instance of" "NestedFixedRule"
                )
    ...
    def nodes_and_weights(self):
        ...
        weights = self.xp.prod(
            _cartesian_product([rule.nodes_and_weights[1] for rule in self.base_rules]),
            axis=-1,
        )
...
def _apply_fixed_rule(f, a, b, orig_nodes, orig_weights, args, xp):
    ...
    if rule_ndim != a_ndim or rule_ndim != b_ndim:
        raise ValueError(
            f"rule and function are of incompatible dimension, nodes have"
