...
def test_kde_1d():
    ...
    assert_almost_equal(
        gkde.integrate_kde(gkde), (kdepdf**2).sum() * intervall, decimal=2
    )
    assert_almost_equal(
        gkde.integrate_gaussian(xnmean, xnstd**2),
        (kdepdf * normpdf).sum() * intervall,
        decimal=2,
    )
...
@pytest.mark.parametrize(
    "n_basesample", [20, pytest.param(500, marks=[pytest.mark.xslow])]
)
def test_kde_2d(n_basesample):
    ...
    assert_allclose(
        gkde(xx.T), pdf(arg, cov=gkde.covariance).sum(axis=-1) / gkde.n, rtol=5e-14
    )
    ...
    normpdf = stats.multivariate_normal.pdf(
        np.dstack([x, y]), mean=mean, cov=covariance
    )
    ...
    assert_almost_equal(prob2, 0.5, decimal=1)
    assert_almost_equal(
        gkde.integrate_kde(gkde), (kdepdf**2).sum() * (intervall**2), decimal=2
    )
    assert_almost_equal(
        gkde.integrate_gaussian(mean, covariance),
        (kdepdf * normpdf).sum() * (intervall**2),
        decimal=2,
    )
...
def test_kde_2d_weighted(n_basesample):
    ...
    assert_allclose(
        gkde.integrate_box(lo, hi, rng=rng),
        np.sum(
            cdf(hi_, lower_limit=lo_, cov=gkde.covariance, rng=rng) * gkde.weights,
            axis=-1,
        ),
        rtol=5e-6,
    )
...
def test_kde_output_dtype(dtype, bw_type):
    ...
    assert result.dtype == np.result_type(
        dataset, points, np.float64(weights), k.factor
    )
...
def test_logpdf_overflow():
    ...
    n_dimensions = 2500
    n_samples = 5000
    xn = np.array([rng.normal(0, 1, n_samples) + (n) for n in range(0, n_dimensions)])
