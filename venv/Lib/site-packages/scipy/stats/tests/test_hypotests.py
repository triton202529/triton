...
class TestMannWhitneyU:
    ...
    def test_exact_distribution(self):
        ...
        for n, table in p_tables.items():
            for m, p in table.items():
                ...
                assert_allclose(
                    _mwu_state.s.cdf(k=u2)
                    + _mwu_state.s.sf(k=u2)
                    - _mwu_state.s.pmf(k=u2),
                    1,
                )
    ...
    def test_permutation_method(self, alternative):
        ...
        res = stats.mannwhitneyu(
            x, y, method=stats.PermutationMethod(), alternative=alternative, axis=1
        )
        res2 = stats.mannwhitneyu(x, y, method="exact", alternative=alternative, axis=1)
...
class TestSomersD(_TestPythranFunc):
    ...
    def test_contingency_table_with_zero_rows_cols(self):
        ...
        s = stats.multinomial.rvs(N, p=np.ones(size) / size, random_state=rng).reshape(
            shape
        )
        res = stats.somersd(s)
...
class TestCvm_2samp:
    @pytest.mark.parametrize("args", [([], np.arange(5)), (np.arange(5), [1])])
...
class TestGamesHowell:
    ...
    @pytest.mark.parametrize(
        "data, res_expect_str",
        ((data_same_size, spss_same_size), (data_diff_size, spss_diff_size)),
    ...
    def test_compare_spss(self, data, res_expect_str):
        ...
        """
        res_expect = np.asarray(
            res_expect_str.replace(" - ", " ").split()[7:], dtype=float
        ).reshape(-1, 6)
    ...
    @pytest.mark.parametrize(
        "data, res_expect_str",
        ((data_same_size, r_same_size), (data_diff_size, r_diff_size)),
        ids=["equal size sample", "unequal sample size"],
    )
    def test_compare_r(self, data, res_expect_str):
        ...
        """
        res_expect = np.asarray(
            res_expect_str.replace(" - ", " ").replace(" == ", " ").split()[3:],
            dtype=float,
        ).reshape(-1, 5)
        res_games = stats.tukey_hsd(*data, equal_var=False)
        # loop over the comparisons
...
class TestPoissonMeansTest:
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            [50, 15, 1, 1, 0.09920321053409643, "two-sided", 0.05],
