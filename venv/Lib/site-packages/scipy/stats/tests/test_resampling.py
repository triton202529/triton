...
def test_bootstrap_against_theory(method):
    ...
    config = dict(
        data=(data,), statistic=np.mean, n_resamples=5000, method=method, rng=rng
    )
...
def test_bootstrap_against_R(method, expected):
    ...
    res = bootstrap((x,), np.mean, n_resamples=1000000, method=method, rng=0)
...
def test_multisample_BCa_against_R():
    ...
    res_basic = stats.bootstrap((x, y), statistic, method="basic", batch=100, rng=rng)
    res_percent = stats.bootstrap(
        (x, y), statistic, method="percentile", batch=100, rng=rng
    )
    res_bca = stats.bootstrap((x, y), statistic, method="bca", batch=100, rng=rng)
...
def test_BCa_acceleration_against_reference():
    ...
    batch = 100
    _, _, a_hat = _resampling._bca_interval(
        data, statistic, axis, alpha, theta_hat_b, batch
    )
    assert_allclose(a_hat, 0.011008228344026734)


tests_against_itself_1samp = {"basic": 1789, "percentile": 1790, "BCa": 1789}
...
def test_bootstrap_min():
    ...
    data = (data,)
    res = bootstrap(
        data, np.min, method="BCa", n_resamples=100, rng=np.random.default_rng(3942)
    )
...
def test_re_bootstrap(additional_resamples):
    ...
    rng = np.random.default_rng(296689032789913033)
    ref = stats.bootstrap(
        (x,), np.mean, n_resamples=n3, rng=rng, confidence_level=0.90, method="BCa"
    )
...
def test_bootstrap_alternative(method):
    ...
    assert_allclose(l.confidence_interval.high, t.confidence_interval.high, rtol=1e-14)
    assert_allclose(g.confidence_interval.low, t.confidence_interval.low, rtol=1e-14)
...
def test_vector_valued_statistic(method):
    ...
    counts = np.sum(
        (res.confidence_interval.low.T < params)
...
def test_vector_valued_statistic_gh17715():
    ...
    assert_allclose(
        res.confidence_interval.low[0], ref.confidence_interval.low, atol=1e-15
    )
    assert_allclose(
        res.confidence_interval.high[0], ref.confidence_interval.high, atol=1e-15
    )
...
class TestMonteCarloHypothesisTest:
    ...
    def get_rvs(self, rvs_in, rs, dtype=None, xp=np):
        return lambda *args, **kwds: xp.asarray(
            rvs_in(*args, random_state=rs, **kwds), dtype=dtype
        )
    ...
    def test_axis_dtype(self, axis, xp):
        ...
        dtype_default = xp.asarray(1.0).dtype
        ...
        expected = stats.ttest_1samp(x, popmean=0.0, axis=axis)
        ...
        res = monte_carlo_test(
            x, rvs, statistic, vectorized=True, n_resamples=20000, axis=axis
        )
    ...
    def test_alternative(self, alternative, xp):
        ...
        ref = stats.ttest_1samp(x, 0.0, alternative=alternative)
    ...
    def test_against_ks_1samp(self, alternative, a):
        ...
        def statistic1d(x):
            return stats.ks_1samp(
                x, stats.norm.cdf, mode="asymp", alternative=alternative
            ).statistic
    ...
    def test_against_normality_tests(self, hypotest, alternative, a):
        ...
        norm_rvs = self.get_rvs(stats.norm.rvs, rng)
        res = monte_carlo_test(
            x, norm_rvs, statistic, vectorized=True, alternative=alternative
        )
    ...
    def test_against_normaltest(self, a):
        ...
        norm_rvs = self.get_rvs(stats.norm.rvs, rng)
        res = monte_carlo_test(
            x, norm_rvs, statistic, vectorized=True, alternative="greater"
        )
    ...
    def test_p_never_zero(self):
        ...
        x = np.zeros(100)
        res = monte_carlo_test(
            x, rng.random, np.mean, vectorized=True, alternative="less"
        )
    ...
    def test_against_f_oneway(self):
        ...
        rng = np.random.default_rng(219017667302737545)
        data = (
            rng.random(size=(2, 100)),
            rng.random(size=(2, 101)),
            rng.random(size=(2, 102)),
            rng.random(size=(2, 103)),
        )
        ...
        res = stats.monte_carlo_test(
            data, rvs, statistic, axis=-1, alternative="greater"
        )
    ...
    def test_finite_precision_statistic(self):
        ...
        def rvs(size):
            return 1.0 * stats.bernoulli(p=0.333).rvs(size=size, random_state=rng)

        x = rvs(100)
        res = stats.monte_carlo_test(
            x, rvs, np.var, alternative="less", n_resamples=n_resamples
        )
...
class TestPower:
    ...
    def test_vectorization(self):
        ...
        def test(x, alternative, axis=-1):
            ...
            return stats.ttest_1samp(
                x, popmeans_expanded, alternative=alternative, axis=axis
            )
        ...
        res = stats.power(
            test, rng.normal, nx, significance=significance, kwargs=kwargs
        )
            ...
            for nx_i in nx:
                for alternative_i in kwargs["alternative"]:
                    for popmean_i in popmeans:
                        ...
                        def test2(x, axis=-1):
                            return stats.ttest_1samp(
                                x, popmean_i, axis=axis, alternative=alternative_i
                            )

                        tmp = stats.power(
                            test2, rng.normal, nx_i, significance=significance_i
                        )
    ...
    def test_ttest_1samp_power(self):
        ...
        ref = [
            [[0.0126515, 0.10269751, 0.40415802], [0.01657775, 0.29734608, 0.86228288]],
            [[0.0592903, 0.29317561, 0.71718121], [0.07094116, 0.56450441, 0.96815163]],
        ]
...
class TestPermutationTest:
    ...
    def test_batch(self, permutation_type, random_state):
        ...
        res1 = stats.permutation_test(
            (x, y), statistic, batch=1, random_state=random_state(0), **kwds
        )
        assert_equal(statistic.counter, 1001)
        assert_equal(statistic.batch_size, 1)
        ...
        res2 = stats.permutation_test(
            (x, y), statistic, batch=50, random_state=random_state(0), **kwds
        )
        assert_equal(statistic.counter, 21)
        assert_equal(statistic.batch_size, 50)

        statistic.counter = 0
        res3 = stats.permutation_test(
            (x, y), statistic, batch=1000, random_state=random_state(0), **kwds
        )
        assert_equal(statistic.counter, 2)
        assert_equal(statistic.batch_size, 1000)
    ...
    def test_permutations(self, permutation_type, exact_size, random_state):
        ...
        res = stats.permutation_test(
            (x, y), statistic, n_resamples=3, random_state=random_state(0), **kwds
        )
    ...
    def test_against_ks_2samp(self, alternative):
        ...
        res = permutation_test(
            (x, y), statistic1d, n_resamples=np.inf, alternative="greater", rng=self.rng
        )
    ...
    def test_finite_precision_statistic(self):
        ...
        res = stats.permutation_test(
            (x, y), statistic, vectorized=False, permutation_type="pairings"
        )
...
@pytest.mark.parametrize(
    "fun_name", ["bootstrap", "permutation_test", "monte_carlo_test"]
)
def test_parameter_vectorized(fun_name):
    ...
    fun_options = {
        "bootstrap": {"data": (sample,), "rng": rng, "method": "percentile"},
        "permutation_test": {
            "data": (sample,),
            "rng": rng,
            "permutation_type": "samples",
