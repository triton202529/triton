...
class TestEntropy:
    ...
    def test_entropy_base(self, xp):
        ...
        qk = xp.ones(16)
        qk = xp.where(xp.arange(16) < 8, 2.0, qk)
...
class TestDifferentialEntropy:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                "correa",
                marks=skip_xp_backends(
                    "array_api_strict", reason="Needs fancy indexing."
                ),
            ),
        ],
    )
    ...
    rmse_std_cases = {norm: norm_rmse_std_cases, expon: expon_rmse_std_cases}
    ...
    def test_rmse_std(self, method, dist, xp):
        ...
        xp_assert_close(
            xp.sqrt(xp.mean((res - true_entropy) ** 2)), rmse_expected, atol=0.005
        )
    ...
    def test_dtypes_gh21192(self, xp, method, dtype):
        ...
        dtype_out = getattr(xp, str(dtype), xp.asarray(1.0).dtype)
