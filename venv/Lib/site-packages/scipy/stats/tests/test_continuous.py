...
class Test_RealInterval:
    ...
    @pytest.mark.parametrize("x", [rng.uniform(10, 10, size=(2, 3, 4)), -np.inf, np.pi])
    ...
    @pytest.mark.slow
    @given(
        shapes=npst.mutually_broadcastable_shapes(num_shapes=3, min_side=0),
    ...
    def test_contains(self, shapes, inclusive_a, inclusive_b, data):
        ...
        a_elements = dict(
            allow_nan=False, allow_infinity=False, min_value=-1e3, max_value=1
        )
        b_elements = dict(
            allow_nan=False, allow_infinity=False, min_value=2, max_value=1e3
        )
        a = data.draw(npst.arrays(npst.floating_dtypes(), shape_a, elements=a_elements))
        b = data.draw(npst.arrays(npst.floating_dtypes(), shape_b, elements=b_elements))
        # ensure some points are to the left, some to the right, and some
        # are exactly on the boundary
        d = b - a
        x = np.concatenate(
            [
                np.linspace(a - d, a, 10),
                np.linspace(a, b, 10),
        ...
        domain = _RealInterval(
            endpoints=("a", "b"), inclusive=(inclusive_a, inclusive_b)
        )
        domain.define_parameters(
            _RealParameter("a", domain=_RealInterval()),
    ...
    def test_contains_function_endpoints(self, inclusive, a, b):
        ...
        x = np.asarray(
            [(a - 2 * b) / 2, (a - b) / 2, a / 2, (a + b) / 2, (a + 2 * b) / 2]
        )
        res = domain.contains(x, dict(a=a, b=b))
    ...
    def test_str2(self, a, b, inclusive_a, inclusive_b):
        ...
        domain = _RealInterval(endpoints=(a, b), inclusive=(inclusive_a, inclusive_b))
...
def draw_distribution_from_family(family, data, rng, proportions, min_side=0):
    ...
    if n_parameterizations > 0:
        ...
        shapes, result_shape = data.draw(
            npst.mutually_broadcastable_shapes(
                num_shapes=n_parameters, min_side=min_side
            )
        )
        dist = family._draw(
            shapes, rng=rng, proportions=proportions, i_parameterization=i
        )
    else:
    ...
    # arguments.
    x_shape = data.draw(npst.broadcastable_shapes(result_shape, min_side=min_side))
    x = dist._variable.draw(
        x_shape,
    ...
    y_shape = data.draw(npst.broadcastable_shapes(x_result_shape, min_side=min_side))
    y = dist._variable.draw(
        y_shape,
        parameter_values=dist._parameters,
        proportions=proportions,
        rng=rng,
...
continuous_families = [StandardNormal, Normal, Uniform, _LogUniform]
...
class TestDistributions:
    ...
    def test_support_moments_sample(self, family, data, seed):
        ...
        sample_shape = data.draw(npst.array_shapes(min_dims=0, min_side=0, max_side=20))
    ...
    for method in methods:
        if isinstance(dist, DiscreteDistribution):
            ...
            with pytest.raises(NotImplementedError, match=message):
                res = (
                    np.exp(dist.logcdf(x, y, method=method))
                    if log
...
def check_ccdf2(dist, log, x, y, result_shape, methods):
    ...
    for method in methods:
        ...
        if isinstance(dist, DiscreteDistribution):
            with pytest.raises(NotImplementedError, match=message):
                res = (
                    np.exp(dist.logccdf(x, y, method=method))
                    if log
...
def check_nans_and_edges(dist, fname, arg, res):
    ...
    valid_parameters, *classified_args = np.broadcast_arrays(
    ...
    if fname not in exclude and not (
...
def check_moment_funcs(dist, result_shape):
    ...
    for i in range(6):
        ...
        assert ref.shape == result_shape
        check(i, "standardized", "formula", ref, success=has_formula(i, "standardized"))
        if not (
            isinstance(dist, Binomial)
            and np.any((dist.n == 0) | (dist.p == 0) | (dist.p == 1))
        ):
...
def get_valid_parameters(dist):
    ...
    for name, value in parameter_values.items():
        ...
        assert_equal(
            valid, parameter.domain.contains(value, parameter_values=parameter_values)
        )
...
def classify_arg(dist, arg, arg_domain):
    ...
    a, b = arg_domain.get_numerical_endpoints(parameter_values=dist._parameters)
...
def test_input_validation():
    ...
    message = (
        "The `Test` distribution family does not accept parameters, "
    ...
    message = "Argument `order` of `Test.moment` must be a " "finite, positive integer."
    with pytest.raises(ValueError, match=message):
        Test().moment(-1)
    with pytest.raises(ValueError, match=message):
        Test().moment(np.inf)
    ...
    message = (
        "The provided parameters `{a}` do not match a supported "
    ...
    message = (
        "The `Test2` distribution family requires parameters, but none "
        "were provided."
    )
    with pytest.raises(ValueError, match=message):
        Test2()
    ...
    message = (
        "The parameters `{c, d}` provided to the `Test2` "
    ...
    message = (
        "The argument provided to `Test2.pdf` cannot be be broadcast to "
...
class TestAttributes:
    ...
    def test_iv_policy(self):
        ...
        assert X.pdf(np.asarray(2.0)) == 1

        # Tests _set_invalid_nan
        a, b = np.asarray(1.0), np.asarray(0.0)  # invalid parameters
...
class TestMakeDistribution:
    ...
    def test_rv_generic(self, i, distdata):
        ...
        skip_raw = {
            2: {"alpha", "foldcauchy", "halfcauchy", "levy", "levy_l"},
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            if distname not in skip_kurtosis:
                assert_allclose(
                    X.kurtosis(convention="excess"), k, rtol=rtol, atol=atol
                )
            ...
            for order in range(3, 4):
                if distname not in skip_standardized:
                    assert_allclose(
                        X.moment(order, kind="standardized"),
    ...
    def test_custom(self):
        ...
        class MyLogUniform:
            ...
            def parameters(self):
                return {
                    "a": {"endpoints": (0, np.inf), "inclusive": (False, False)},
        ...
        Y = stats.exp(Uniform(a=1.0, b=3.0))
        x = X.sample(shape=10, rng=rng)
        p = X.cdf(x)
        ...
        for kind in ["raw", "central", "standardized"]:
            for order in range(5):
                assert_allclose(X.moment(order, kind=kind), Y.moment(order, kind=kind))
    ...
    @pytest.mark.parametrize("c", [-1, 0, 1, np.asarray([-2.1, -1.0, 0.0, 1.0, 2.1])])
    def test_custom_variable_support(self, c):
        ...
        class MyGenExtreme:
            ...
            def pdf(self, x, *, c, mu, sigma):
                ...
                t[~mask] = (1 - c[~mask] * (x[~mask] - mu[~mask]) / sigma[~mask]) ** (
                    1 / c[~mask]
                )
    ...
    def test_loc_scale(self, data, seed):
        ...
        with np.errstate(invalid="ignore", divide="ignore"):
            ...
            for i in range(1, 5):
                ...
                assert_allclose(
                    dist.moment(i, "central"), dist0.moment(i, "central") * scale**i
                )
                assert_allclose(
        ...
        with np.errstate(invalid="ignore", divide="ignore"):
            ...
            for i in range(1, 5):
                ...
                assert_allclose(
                    dist.moment(i, "standardized"), dist0.moment(i, "standardized")
                )
...
class TestFullCoverage:
    ...
    def test_Domain(self):
        with pytest.raises(NotImplementedError):
            _Domain.contains(None, 1.0)
        with pytest.raises(NotImplementedError):
            _Domain.get_numerical_endpoints(None, 1.0)
        with pytest.raises(NotImplementedError):
            _Domain.__str__(None)

    def test_Parameter(self):
        with pytest.raises(NotImplementedError):
            _Parameter.validate(None, 1.0)

    @pytest.mark.parametrize(
        ("dtype_in", "dtype_out"), [(np.float16, np.float16), (np.int16, np.float64)]
    )
    ...
    def test_ContinuousDistribution_set_invalid_nan(self):
        ...
        class TestDist(ContinuousDistribution):
            _variable = _RealParameter("x", domain=_RealInterval(endpoints=(0.0, 1.0)))
    ...
    def test_fiinfo(self):
        assert _fiinfo(np.float64(1.0)).max == np.finfo(np.float64).max
...
class MixedDist(ContinuousDistribution):
    ...
    def _pdf_formula(self, x, *args, **kwargs):
        return 0.4 * 1 / (1.1 * np.sqrt(2 * np.pi)) * np.exp(
            -0.5 * ((x + 0.25) / 1.1) ** 2
...
class TestMixture:
    ...
    def test_basic(self, shape):
        ...
        X = Mixture(
            (Normal(mu=-0.25, sigma=1.1), Normal(mu=0.5, sigma=0.9)), weights=(0.4, 0.6)
        )
        Y = MixedDist()
        x = rng.random(shape)
        ...
        for kind in ["raw", "central", "standardized"]:
            for order in range(5):
                assert_allclose(
                    X.moment(order, kind=kind), Y.moment(order, kind=kind), atol=1e-15
                )
