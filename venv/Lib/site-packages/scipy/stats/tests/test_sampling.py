...
@pytest.mark.parametrize(
    "domain, err, msg", bad_domains + bad_sized_domains + nan_domains
...
@pytest.mark.parametrize(
    "size", [None, 0, (0,), 1, (10, 3), (2, 3, 4, 5), (0, 0), (0, 1)]
)
...
class TestTransformedDensityRejection:
    ...
    def test_bad_construction_points_scalar(self, construction_points):
        with pytest.raises(
            ValueError, match=r"`construction_points` must be " r"a positive integer."
        ):
            TransformedDensityRejection(
    ...
    def test_bad_construction_points_array(self):
        ...
        with pytest.raises(
            ValueError,
            match=r"`construction_points` must "
            r"either be a "
        ...
        with pytest.warns(
            RuntimeWarning,
            match=r"33 : starting points not " r"strictly monotonically " r"increasing",
        ):
            TransformedDensityRejection(
                StandardNormal(), construction_points=construction_points
            )
        ...
        with pytest.raises(UNURANError, match=r"50 : bad construction " r"points."):
            TransformedDensityRejection(
                StandardNormal(), construction_points=construction_points
            )
        ...
        with pytest.warns(
            RuntimeWarning, match=r"50 : starting point out of " r"domain"
        ):
            TransformedDensityRejection(
                StandardNormal(),
                domain=(-3, 3),
    ...
    def test_ppf_hat(self, u):
        ...
        rng = TransformedDensityRejection(
            StandardNormal(), max_squeeze_hat_ratio=0.9999
        )
...
class TestDiscreteAliasUrn:
    ...
    def test_basic(self, distname, params):
        if distname in self.basic_fail_dists:
            msg = (
                "DAU fails on these probably because of large domains "
    ...
    bad_pmf = [
        # inf returned
        (lambda x: np.inf, ValueError, r"must contain only finite / non-nan values"),
        # nan returned
        (lambda x: np.nan, ValueError, r"must contain only finite / non-nan values"),
        # all zeros
        (lambda x: 0.0, ValueError, r"must contain at least one non-zero value"),
        # Undefined name inside the function
        (
            lambda x: foo,
            NameError,  # type: ignore[name-defined]  # noqa: F821
            r"name 'foo' is not defined",
        ),
        # Returning wrong type.
        (lambda x: [], ValueError, r"setting an array element with a sequence."),
        # probabilities < 0
    ...
    def test_bad_args(self):
        msg = (
            r"`domain` must be provided when the "
    ...
    def test_gh19359(self):
        ...
        check_discr_samples(rng, pv, (1532 / 2, (1532**2 - 1) / 12), rtol=5e-3)
...
class TestNumericalInverseHermite:
    ...
    class dist0:
        ...
        def cdf(self, x):
            return (1.0 + 2.0 * np.pi * (1 + x) - np.cos(2.0 * np.pi * x)) / (
                4.0 * np.pi
            )
    ...
    def test_inaccurate_CDF(self):
        ...
        match = (
            "98 : one or more intervals very short; possibly due to "
...
class TestDiscreteGuideTable:
    ...
    def test_basic(self, distname, params):
        if distname in self.basic_fail_dists:
            msg = (
                "DGT fails on these probably because of large domains "
                "and small computation errors in PMF."
            )
            pytest.skip(msg)
...
class TestSimpleRatioUniforms:
    ...
    def test_basic(self, dist, mv_ex):
        ...
        check_cont_samples(rng, dist, mv_ex)
        rng = SimpleRatioUniforms(
            dist, mode=dist.mode, cdf_at_mode=dist.cdf(dist.mode), random_state=42
        )
        check_cont_samples(rng, dist, mv_ex)
