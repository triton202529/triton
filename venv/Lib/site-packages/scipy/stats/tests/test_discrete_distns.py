...
class TestZipfian:
    def test_zipfian_asymptotic(self):
        ...
        assert_allclose(
            zipfian.stats(a, N, moments="msvk"), zipf.stats(a, moments="msvk")
        )
    ...
    def test_zipfian_continuity(self):
        ...
        N = 30
        k = np.arange(1, N + 1)
        assert_allclose(zipfian.pmf(k, alt1, N), zipfian.pmf(k, agt1, N), rtol=5e-7)
        assert_allclose(zipfian.cdf(k, alt1, N), zipfian.cdf(k, agt1, N), rtol=5e-7)
        assert_allclose(zipfian.sf(k, alt1, N), zipfian.sf(k, agt1, N), rtol=5e-7)
        assert_allclose(
            zipfian.stats(alt1, N, moments="msvk"),
            zipfian.stats(agt1, N, moments="msvk"),
            rtol=5e-7,
        )
    ...
    rng = np.random.RandomState(0)
    naive_tests = np.vstack((np.logspace(-2, 1, 10), rng.randint(2, 40, 10))).T
    ...
    def test_zipfian_naive(self, a, n):
        ...
        assert_allclose(
            zipfian.stats(a, n, moments="mvsk"), [mean, var, skew, kurtosis]
        )
...
class TestNCH:
    ...
    @pytest.mark.parametrize(
        "dist_name", ["nchypergeom_fisher", "nchypergeom_wallenius"]
    )
    ...
    def test_nchypergeom_wallenius_naive(self):
        ...
        with suppress_warnings() as sup:
            ...
            assert_allclose(
                nchypergeom_wallenius.mean(N, m1, n, w), mean(N, m1, n, w), rtol=2e-2
            )
    ...
    def test_wallenius_against_mpmath(self):
        ...
        assert_allclose(
            nchypergeom_wallenius.pmf(sup, M, n, N, odds), pmf, rtol=1e-13, atol=1e-13
        )
...
@pytest.mark.parametrize(
    "mu, q, expected",
    [[10, 120, -1.240089881791596e-38], [1500, 0, -86.61466680572661]],
)
...
class TestPoissonBinomial:
    def test_pmf(self):
        ...
        ref = [
            0.0343763443678060318,
            0.6435428452689714307,
            0.2936345519235536994,
            0.0277036647503902354,
            0.0007425936892786034,
        ]
