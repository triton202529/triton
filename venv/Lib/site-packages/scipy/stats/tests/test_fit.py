    ...
    for fit_size in fit_sizes:
        ...
        with np.errstate(all="ignore"):
            ...
            if method == "MLE":
                ...
                msg = (
                    "Different results fitting uncensored data wrapped as"
            ...
            if method == "MLE" and distname not in fail_interval_censored:
                ...
                msg = (
                    "Different results fitting interval-censored"
...
def _check_loc_scale_mle_fit(name, data, desired, atol=None):
    ...
    actual = d.fit(data)[-2:]
    assert_allclose(
        actual, desired, atol=atol, err_msg=f"poor mle fit of (loc, scale) in {name}"
    )
...
def cases_test_fit_mle():
    ...
    skip_basic_fit = {
        ...
        "beta",
        "nakagami",
        "truncnorm",  # don't meet tolerance
        "poisson_binom",
    }  # vector-valued shape parameter
...
def cases_test_fitstart():
    for distname, shapes in dict(distcont).items():
        if not isinstance(distname, str) or distname in {
            "studentized_range",
            "recipinvgauss",
        }:  # slow
            continue
...
def assert_nlff_less_or_close(
    dist, data, params1, params0, rtol=1e-7, atol=0, nlff_name="nnlf"
):
...
class TestFit:
    ...
    def basic_fit_test(self, dist_name, method, rng=1):
        ...
        assert_nlff_less_or_close(
            dist, data, res.params, ref, **self.tols, nlff_name=nlff_name
        )
    ...
    def test_arcsine(self):
        ...
        assert_nlff_less_or_close(
            dist, data, res.params, shapes, nlff_name="_penalized_nlpsf", **self.tols
        )
    ...
    def test_argus(self, method):
        ...
        assert_nlff_less_or_close(
            dist, data, res.params, shapes, **self.tols, nlff_name=nlff_name
        )
    ...
    def test_beta(self):
        ...
        rng = np.random.default_rng(self.seed)
        dist = stats.beta
        shapes = (2.3098496451481823, 0.62687954300963677, 1.0, 2.0)
        data = dist.rvs(*shapes, size=N, random_state=rng)
        shape_bounds = {
            "a": (0.1, 10),
            "b": (0.1, 10),
        ...
        assert_nlff_less_or_close(
            dist, data, res.params, shapes, nlff_name="nnlf", **self.tols
        )
    ...
    def test_foldnorm(self):
        ...
        rng = np.random.default_rng(self.seed)
        dist = stats.foldnorm
        shapes = (1.952125337355587, 2.0, 3.0)
        data = dist.rvs(*shapes, size=N, random_state=rng)
    ...
    def test_nakagami(self):
        ...
        rng = np.random.default_rng(self.seed)
        dist = stats.nakagami
        shapes = (4.9673794866666237, 1.0, 2.0)
        data = dist.rvs(*shapes, size=N, random_state=rng)
    ...
    def test_powerlaw(self):
        ...
        rng = np.random.default_rng(self.seed)
        dist = stats.powerlaw
        shapes = (1.6591133289905851, 1.0, 2.0)
        data = dist.rvs(*shapes, size=N, random_state=rng)
...
class TestGoodnessOfFit:
    ...
    def test_against_ks(self):
        ...
        res = goodness_of_fit(
            stats.norm, x, known_params=known_params, statistic="ks", rng=rng
        )
    ...
    def test_against_cvm(self):
        ...
        known_params = {"loc": np.mean(x), "scale": np.std(x, ddof=1)}
        res = goodness_of_fit(
            stats.norm, x, known_params=known_params, statistic="cvm", rng=rng
        )
    ...
    def test_against_anderson_case_0(self):
        ...
        res = goodness_of_fit(
            stats.norm, x, known_params=known_params, statistic="ad", rng=rng
        )
    ...
    def test_against_anderson_case_3(self):
        ...
        x = stats.skewnorm.rvs(
            1.4477847789132101, loc=1, scale=2, size=100, random_state=rng
        )
    ...
    def test_against_anderson_gumbel_r(self):
        ...
        # c that produced critical value of statistic found w/ root_scalar
        x = stats.genextreme(0.051896837188595134, loc=0.5, scale=1.5).rvs(
    ...
    def test_against_filliben_norm(self):
        ...
        res = stats.goodness_of_fit(
            stats.norm, y, known_params=known_params, statistic="filliben", rng=rng
        )
    ...
    def test_filliben_property(self):
        ...
        res = stats.goodness_of_fit(stats.norm, x, statistic="filliben", rng=rng)
        known_params = {"loc": 0, "scale": 1}
        ref = stats.goodness_of_fit(
            stats.norm, x, known_params=known_params, statistic="filliben", rng=rng
        )
    ...
    @pytest.mark.parametrize(
        "case",
        [
            (25, [0.928, 0.937, 0.950, 0.958, 0.966]),
            (50, [0.959, 0.965, 0.972, 0.977, 0.981]),
            (95, [0.977, 0.979, 0.983, 0.986, 0.989]),
        ],
    )
    ...
    def test_against_ppcc(self, case):
        ...
        x = rng.normal(size=n)
        res = stats.goodness_of_fit(stats.rayleigh, x, statistic="filliben", rng=rng)
    ...
    def test_params_effects(self):
        ...
        x = stats.skewnorm.rvs(
            -5.044559778383153, loc=1, scale=2, size=50, random_state=rng
        )
        ...
        assert not np.allclose(
            res2.fit_result.params.c, res1.fit_result.params.c, rtol=1e-8
        )
        assert not np.allclose(
            res2.null_distribution, res1.null_distribution, rtol=1e-8
        )
