...
def test_discrete_basic(distname, arg, first_case, num_parallel_threads):
    if (
        isinstance(distname, str)
        and distname.startswith("nchypergeom")
...
def test_isf_with_loc(dist, args):
    ...
    re_locs = [
        np.random.randint(-10, -1, size=(5, 3)),
        np.zeros((5, 3)),
...
def check_cdf_ppf(distfn, arg, supp, msg):
    ...
    npt.assert_array_equal(
        distfn.ppf(distfn.cdf(supp, *arg) - 1e-8, *arg), supp, msg + "-roundtrip"
    )
    ...
    if not hasattr(distfn, "xk"):
        ...
        supp1 = supp[supp < _b]
        npt.assert_array_equal(
            distfn.ppf(distfn.cdf(supp1, *arg) + 1e-8, *arg),
            supp1 + distfn.inc,
            msg + " ppf-cdf-next",
        )
...
def check_oth(distfn, arg, supp, msg):
    ...
    npt.assert_allclose(
        distfn.sf(supp, *arg), 1.0 - distfn.cdf(supp, *arg), atol=1e-10, rtol=1e-10
    )

    q = np.linspace(0.01, 0.99, 20)
    npt.assert_allclose(
        distfn.isf(q, *arg), distfn.ppf(1.0 - q, *arg), atol=1e-10, rtol=1e-10
    )
...
@pytest.mark.parametrize(
    "method", ["pmf", "logpmf", "cdf", "logcdf", "sf", "logsf", "ppf", "isf"]
)
...
def test_methods_with_lists(method, distname, args):
    ...
    npt.assert_allclose(
        result, [dist_method(*v) for v in zip(z, *p2, loc)], rtol=1e-15, atol=1e-15
    )
...
def cases_test_discrete_integer_shapes():
    ...
    for distname, shapes in distdiscrete:
        ...
        for i, shape in enumerate(shape_info):
            if (
                shape.name in integrality_exceptions.get(distname, set())
                or not shape.integrality
            ):
                continue
