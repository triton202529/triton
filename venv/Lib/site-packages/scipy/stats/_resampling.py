...
def _bca_interval(data, statistic, axis, alpha, theta_hat_b, batch):
    ...
    theta_hat_ji = [
        np.concatenate(theta_hat_i, axis=-1) for theta_hat_i in theta_hat_ji
    ]

    n_j = [theta_hat_i.shape[-1] for theta_hat_i in theta_hat_ji]

    theta_hat_j_dot = [
        theta_hat_i.mean(axis=-1, keepdims=True) for theta_hat_i in theta_hat_ji
    ]

    U_ji = [
        (n - 1) * (theta_hat_dot - theta_hat_i)
        for theta_hat_dot, theta_hat_i, n in zip(theta_hat_j_dot, theta_hat_ji, n_j)
    ]
...
def _bootstrap_iv(
    ...
    if bootstrap_result is not None and not hasattr(
        bootstrap_result, "bootstrap_distribution"
    ):
        raise ValueError(message)

    message = (
        "Either `bootstrap_result.bootstrap_distribution.size` or "
        "`n_resamples` must be positive."
    )
    if (
        not bootstrap_result or not bootstrap_result.bootstrap_distribution.size
...
def bootstrap(
    ...
    *,
    ...
    theta_hat_b = (
        [] if bootstrap_result is None else [bootstrap_result.bootstrap_distribution]
    )
    ...
    for k in range(0, n_resamples, batch_nominal):
        ...
        resampled_data = []
        for sample in data:
            resample = _bootstrap_resample(sample, n_resamples=batch_actual, rng=rng)
...
def _monte_carlo_test_iv(
    data, rvs, statistic, vectorized, n_resamples, batch, alternative, axis
):
    ...
    if vectorized is None:
        try:
            ...
            message = (
                f"Signature inspection of {statistic=} failed; "
    ...
    if not vectorized:
        if is_numpy(xp):
            ...
            message = (
                "`statistic` must be vectorized (i.e. support an `axis` "
...
def monte_carlo_test(
    ...
    *,
    ...
    for k in range(0, n_resamples, batch_nominal):
        batch_actual = min(batch_nominal, n_resamples - k)
        resamples = [
            rvs_i(size=(batch_actual, n_observations_i))
    ...
    eps = (
        0
        if not xp.isdtype(observed.dtype, ("real floating"))
    ...
    def less(null_distribution, observed):
        ...
        pvalues = (xp.sum(cmps, axis=0, dtype=dtype) + 1.0) / (n_resamples + 1.0)
        return pvalues
    ...
    pvalues = xp.clip(pvalues, 0.0, 1.0)
...
def _wrap_kwargs(fun):
    ...
    def wrapped_rvs_i(*args, keys=keys, fun=fun, **all_kwargs):
        kwargs = {key: val for key, val in all_kwargs.items() if key in keys}
...
def _power_iv(
    rvs, test, n_observations, significance, vectorized, n_resamples, batch, kwargs
):
    """Input validation for `monte_carlo_test`."""
    ...
    if not len(rvs) == len(n_observations):
        message = (
            "If `rvs` is a sequence, `len(rvs)` " "must equal `len(n_observations)`."
        )
        raise ValueError(message)

    significance = np.asarray(significance)[()]
    if (
        not np.issubdtype(significance.dtype, np.floating)
        or np.min(significance) < 0
        or np.max(significance) > 1
    ):
    ...
    *alternative* hypothesis and calculate the power.
    ...
    for nobs_i, args_i in zip(nobs, args):
        ...
        for k in range(0, n_resamples, batch_nominal):
            batch_actual = min(batch_nominal, n_resamples - k)
            resamples = [
                rvs_j(size=(batch_actual, nobs_ij), **kwargs_i)
...
def _pairings_permutations_gen(n_permutations, n_samples, n_obs_sample, batch, rng):
...
def _calculate_null_both(data, statistic, n_permutations, batch, rng=None):
    """
...
def _calculate_null_pairings(data, statistic, n_permutations, batch, rng=None):
    """
    ...
    if n_permutations >= n_max:
        ...
        perm_generator = product(
            *(permutations(range(n_obs_sample)) for i in range(n_samples))
        )
...
def _calculate_null_samples(data, statistic, n_permutations, batch, rng=None):
    """
...
def permutation_test(
    ...
    *difference* between two paired elements. Accordingly, if ``data`` contains
    ...
    null_calculator_args = (data, statistic, n_resamples, batch, rng)
    ...
    eps = (
        0
        if not np.issubdtype(observed.dtype, np.inexact)
...
class MonteCarloMethod(ResamplingMethod):
    ...
    def _asdict(self):
        ...
        return dict(
            n_resamples=self.n_resamples, batch=self.batch, rvs=self.rvs, rng=self.rng
        )


_rs_deprecation = (
    "Use of attribute `random_state` is deprecated and replaced by "
...
class BootstrapMethod(ResamplingMethod):
    ...
    def __init__(
        self, n_resamples=9999, batch=None, random_state=None, method="BCa", *, rng=None
    ):
        # Uncomment in SciPy 1.17.0
        # warnings.warn(_rs_deprecation.replace('attribute', 'argument'),
        #               DeprecationWarning, stacklevel=2)
    ...
    def _asdict(self):
        # `dataclasses.asdict` deepcopies; we don't want that.
        d = dict(n_resamples=self.n_resamples, batch=self.batch, method=self.method)
        if self.rng is not None:
            d["rng"] = self.rng
        if self.random_state is not None:
            d["random_state"] = self.random_state
