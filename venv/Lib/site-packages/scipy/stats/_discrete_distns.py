...
class poisson_gen(rv_discrete):
    ...
    def _stats(self, mu):
        ...
        mu_nonzero = tmp > 0
        g1 = xpx.apply_where(
            mu_nonzero, tmp, lambda x: sqrt(1.0 / x), fill_value=np.inf
        )
...
class poisson_binom_gen(rv_discrete):
    ...
    def _rvs(self, *args, size=None, random_state=None):
        ...
        size = (
            p.shape
            if size is None
            else (size, 1) if np.isscalar(size) else tuple(size) + (1,)
        )
...
poisson_binom = poisson_binom_gen(
    name="poisson_binom", longname="A Poisson binomial", shapes="p"
)
