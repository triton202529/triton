...
class WilcoxonDistribution:
    ...
    def cdf(self, k):
        ...
        return xpx.apply_where(
            k <= mn, (k, self.n), self._cdf, lambda k, n: 1 - self._sf(k + 1, n)
        )[()]

    def sf(self, k):
        k, mn, out = self._prep(k)
        return xpx.apply_where(
            k <= mn, (k, self.n), self._sf, lambda k, n: 1 - self._cdf(k - 1, n)
        )[()]
...
def _wilcoxon_iv(x, y, zero_method, correction, alternative, method, axis):
    ...
    if not isinstance(method, stats.PermutationMethod):
        ...
        message = (
            f"`method` must be one of {methods} or "
...
def _wilcoxon_nd(
    ...
    if method == "asymptotic":
        ...
        if alternative == "less":
            ...
            p = 2 * np.minimum(dist.sf(np.floor(r_plus)), dist.cdf(np.ceil(r_plus)))
            p = np.clip(p, 0, 1)
    else:  # `PermutationMethod` instance (already validated)
        p = stats.permutation_test(
            (d,),
            lambda d: _wilcoxon_statistic(d, method, zero_method)[0],
