...
TheilslopesResult = _make_tuple_bunch(
    "TheilslopesResult", ["slope", "intercept", "low_slope", "high_slope"]
)
...
def theilslopes(y, x=None, alpha=0.95, method="separate"):
    ...
    if method not in ["joint", "separate"]:
        raise ValueError(
            "method must be either 'joint' or 'separate'." f"'{method}' is invalid."
        )
    ...
    return TheilslopesResult(
        slope=medslope, intercept=medinter, low_slope=delta[0], high_slope=delta[1]
    )
