...
def _generate_domain_support(dist_family):
    ...
    if n_parameterizations == 0:
        ...
        parameterizations = [f"- {str(p)}" for p in dist_family._parameterizations]
...
class TruncatedDistribution(TransformedDistribution):
    ...
    def __str__(self):
        with np.printoptions(threshold=10):
            return (
                f"truncate({str(self._dist)}, " f"lb={str(self.lb)}, ub={str(self.ub)})"
            )
...
class OrderStatisticDistribution(TransformedDistribution):
    ...
    def __str__(self):
        with np.printoptions(threshold=10):
            return (
                f"order_statistic({str(self._dist)}, r={str(self.r)}, "
                f"n={str(self.n)})"
            )
