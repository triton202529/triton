...
def _Phi_Z1(alpha, t):
    return np.tan(np.pi * alpha / 2) if alpha != 1 else -2.0 * np.log(np.abs(t)) / np.pi
...
def _pdf_single_value_piecewise_Z0(x0, alpha, beta, **kwds):
    ...
    if alpha == 2.0:
        ...
        arg = 1 / (4 * np.abs(x0))
        return (np.sin(arg) * (0.5 - S[0]) + np.cos(arg) * (0.5 - C[0])) / np.sqrt(
            2 * np.pi * np.abs(x0) ** 3
        )
    ...
    def _pdf(self, x, alpha, beta):
        ...
        for pair in uniq_param_pairs:
            ...
            if pdf_single_value_method is not None:
                ...
                if q > MAX_Q:
                    raise RuntimeError(
                        "fft_n_points_two_power has a maximum " + f"value of {MAX_Q}"
                    )
...
def pdf_from_cf_with_fft(cf, h=0.01, q=9, level=3):
    ...
    if level > 1:
        ...
        s1 = np.sum(
            (-1) ** steps
            * Cotes[n, indices]
            * np.fft.fft((-1) ** steps * cf(-L + h * steps + h * indices / (n - 1)))
            * np.exp(
                1j * np.pi * indices / (n - 1)
                - 2 * 1j * np.pi * indices * steps / (N * (n - 1))
            ),
            axis=0,
        )
    else:
        s1 = (
            (-1) ** steps * Cotes[n, 0] * np.fft.fft((-1) ** steps * cf(-L + h * steps))
        )
