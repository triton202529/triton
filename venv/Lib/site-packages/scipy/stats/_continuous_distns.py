...
from . import _stats
from ._tukeylambda_stats import (
...
def _get_left_bracket(fun, rbrack, lbrack=None):
    ...
    while not interval_contains_root(lbrack, rbrack):
        ...
        msg = (
            "The solver could not find a bracket containing a "
...
class beta_gen(rv_continuous):
    ...
    def _stats(self, a, b):
        ...
        _beta_kurtosis_excess_n = 6 * (
            (a - b) ** 2 * (a_plus_b + 1) - a * b * (a_plus_b + 2)
        )
        _beta_kurtosis_excess_d = a * b * (a_plus_b + 2) * (a_plus_b + 3)
        _beta_kurtosis_excess = _beta_kurtosis_excess_n / _beta_kurtosis_excess_d
        return (_beta_mean, _beta_variance, _beta_skewness, _beta_kurtosis_excess)
    ...
    def fit(self, data, *args, **kwds):
        ...
        if f0 is not None or f1 is not None:
            ...
            theta, info, ier, mesg = optimize.fsolve(
                _beta_mle_ab, [a, b], args=(len(data), s1, s2), full_output=True
            )
            if ier != 1:
                raise FitSolverError(mesg=mesg)
            a, b = theta
...
class betaprime_gen(rv_continuous):
    ...
    def _munp(self, n, a, b):
        return xpx.apply_where(
            ...
            (a, b),
            lambda a, b: np.prod(
                [(a + i - 1) / (b - i) for i in range(1, int(n) + 1)], axis=0
            ),
            fill_value=np.inf,
        )
...
class bradford_gen(rv_continuous):
    ...
    def _stats(self, c, moments="mv"):
        ...
        if "s" in moments:
            g1 = np.sqrt(2) * (
                12 * c * c - 9 * c * k * (c + 2) + 2 * k * k * (c * (c + 3) + 3)
            )
...
class burr_gen(rv_continuous):
    ...
    def _pdf(self, x, c, d):
        ...
        output = xpx.apply_where(
            ...
            lambda x_, c_, d_: (
                c_ * d_ * (x_ ** (-c_ - 1.0)) / ((1 + x_ ** (-c_)) ** (d_ + 1.0))
            ),
        )
    ...
    def _munp(self, n, c, d):
        ...
        return xpx.apply_where(
            (c > n) & (n == n) & (d == d), (n, c, d), __munp, fill_value=np.nan
        )
...
class burr12_gen(rv_continuous):
    ...
    def _munp(self, n, c, d):
        ...
        return xpx.apply_where(
            c * d > n, (n, c, d), moment_if_exists, fill_value=np.nan
        )
...
class chi2_gen(rv_continuous):
    ...
    def _entropy(self, df):
        ...
        def asymptotic_formula(half_df):
            ...
            h = 0.5 / half_df
            return (
                h * (-2 / 3 + h * (-1 / 3 + h * (-4 / 45 + h / 7.5)))
                + 0.5 * np.log(half_df)
                + c
            )

        return xpx.apply_where(
            half_df < 125, half_df, regular_formula, asymptotic_formula
        )
...
class cosine_gen(rv_continuous):
    ...
    def _logpdf(self, x):
        c = np.cos(x)
        return xpx.apply_where(
            c != -1, c, lambda c: np.log1p(c) - np.log(2 * np.pi), fill_value=-np.inf
        )
...
class dgamma_gen(rv_continuous):
    ...
    def _ppf(self, q, a):
        return np.where(
            q > 0.5, sc.gammaincinv(a, 2 * q - 1), -sc.gammainccinv(a, 2 * q)
        )
...
class dpareto_lognorm_gen(rv_continuous):
    ...
    def _shape_info(self):
        return [
            _ShapeInfo("u", False, (-np.inf, np.inf), (False, False)),
...
class f_gen(rv_continuous):
    ...
    def _stats(self, dfn, dfd):
        ...
        mu = xpx.apply_where(
            v2 > 2, (v2, v2_2), lambda v2, v2_2: v2 / v2_2, fill_value=np.inf
        )

        mu2 = xpx.apply_where(
            v2 > 4,
            (v1, v2, v2_2, v2_4),
            lambda v1, v2, v2_2, v2_4: 2
            * v2
            * v2
            * (v1 + v2_2)
            / (v1 * v2_2**2 * v2_4),
        ...
        g1 = xpx.apply_where(
            v2 > 6,
            (v1, v2_2, v2_4, v2_6),
            lambda v1, v2_2, v2_4, v2_6: (2 * v1 + v2_2)
            / v2_6
            * np.sqrt(v2_4 / (v1 * (v1 + v2_2))),
            fill_value=np.nan,
        )
        g1 *= np.sqrt(8.0)
...
class truncweibull_min_gen(rv_continuous):
    ...
    def _munp(self, n, c, a, b):
        gamma_fun = sc.gamma(n / c + 1.0) * (
            sc.gammainc(n / c + 1.0, pow(b, c)) - sc.gammainc(n / c + 1.0, pow(a, c))
        )
        denum = np.exp(-pow(a, c)) - np.exp(-pow(b, c))
...
class genpareto_gen(rv_continuous):
    ...
    def _get_support(self, c):
        ...
        b = xpx.apply_where(c < 0, c, lambda c: -1.0 / c, fill_value=np.inf)
        return a, b
    ...
    def _logpdf(self, x, c):
        return xpx.apply_where(
            (x == x) & (c != 0),
            (x, c),
            lambda x, c: -sc.xlog1py(c + 1.0, c * x) / c,
            fill_value=-x,
        )
    ...
    def _stats(self, c, moments="mv"):
        ...
        if "m" in moments:
            m = xpx.apply_where(c < 1, c, lambda xi: 1 / (1 - xi), fill_value=np.inf)
...
class genexpon_gen(rv_continuous):
    ...
    def _pdf(self, x, a, b, c):
        ...
        return (a + b * (-sc.expm1(-c * x))) * np.exp(
            (-a - b) * x + b * (-sc.expm1(-c * x)) / c
        )

    def _logpdf(self, x, a, b, c):
        return (
            np.log(a + b * (-sc.expm1(-c * x)))
            + (-a - b) * x
            + b * (-sc.expm1(-c * x)) / c
        )
...
class genextreme_gen(rv_continuous):
    ...
    def _logpdf(self, x, c):
        ...
        cx = xpx.apply_where((x == x) & (c != 0), (c, x), operator.mul, fill_value=0.0)
        logex2 = sc.log1p(-cx)
    ...
    def _stats(self, c):
        ...
        def sk1_eval(c, *args):
            ...
            return xpx.apply_where(
                c >= -1.0 / 3, (c, *args), sk1_eval_f, fill_value=np.nan
            )

        sk_fill = 12 * np.sqrt(6) * _ZETA3 / np.pi**3
        args = (g1, g2, g3, g2mg12)
        sk = xpx.apply_where(
            abs(c) > eps**0.29, (c, *args), sk1_eval, fill_value=sk_fill
        )
        ...
        def ku1_eval(c, *args):
            ...
            return xpx.apply_where(c >= -1.0 / 4, args, ku1_eval_f, fill_value=np.nan)

        args = (g1, g2, g3, g4, g2mg12)
        ku = xpx.apply_where(
            abs(c) > eps**0.23, (c, *args), ku1_eval, fill_value=12.0 / 5.0
        )
    ...
    def _munp(self, n, c):
        k = np.arange(0, n + 1)
        vals = (
            1.0 / c**n * np.sum(sc.comb(n, k) * (-1) ** k * sc.gamma(c * k + 1), axis=0)
        )
...
class gamma_gen(rv_continuous):
    ...
    def _entropy(self, a):
        ...
        def asymptotic_formula(a):
            ...
            #              1/12a - 1/360a^3
            return (
                0.5 * (1.0 + np.log(2 * np.pi) + np.log(a))
                - 1 / (3 * a)
                - (a**-2.0) / 12
                - (a**-3.0) / 90
                + (a**-4.0) / 120
    ...
    def fit(self, data, *args, **kwds):
        ...
        if isinstance(data, CensoredData) or floc is None and method.lower() != "mm":
        ...
        if fscale is None:
            ...
            if f0 is not None:
                ...
                xb = aest * (1 + 0.4)
                a = optimize.brentq(
                    lambda a: np.log(a) - sc.digamma(a) - s, xa, xb, disp=0
                )
...
class gengamma_gen(rv_continuous):
    ...
    def _logpdf(self, x, a, c):
        return xpx.apply_where(
            ...
            (x, c),
            lambda x, c: (
                np.log(abs(c)) + sc.xlogy(c * a - 1, x) - x**c - sc.gammaln(a)
            ),
            fill_value=-np.inf,
        )
    ...
    def _entropy(self, a, c):
        ...
        def asymptotic(a, c):
            ...
            return (
                ...
                + (a**-1.0) / 6
                - (a**-3.0) / 90
                + (np.log(a) - (a**-1.0) / 2 - (a**-2.0) / 12 + (a**-4.0) / 120) / c
            )
...
class genhyperbolic_gen(rv_continuous):
    ...
    def _argcheck(self, p, a, b):
        return np.logical_and(np.abs(b) < a, p >= 0) | np.logical_and(
            np.abs(b) <= a, p < 0
        )
    ...
    def _integrate_pdf(x0, x1, p, a, b):
        ...
        if x0 < mean < x1:
            ...
            intgrl = (
                integrate.quad(llc, x0, mean, epsrel=epsrel, epsabs=epsabs)[0]
                + integrate.quad(llc, mean, x1, epsrel=epsrel, epsabs=epsabs)[0]
            )
        else:
            intgrl = integrate.quad(llc, x0, x1, epsrel=epsrel, epsabs=epsabs)[0]
        if np.isnan(intgrl):
            msg = (
                "Infinite values encountered in scipy.special.kve. "
    ...
    def _stats(self, p, a, b):
        ...
        m3e = np.float_power(b, 3) * np.float_power(t2, 3) * (
            r3 - 3 * b2 * b1 * np.float_power(b0, -2) + 2 * np.float_power(r1, 3)
        ) + 3 * b * np.float_power(t2, 2) * (r2 - np.float_power(r1, 2))
...
class gumbel_r_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        if fscale is not None:
            ...
            while not interval_contains_root(lbrack, rbrack) and (
                lbrack > 0 or rbrack < np.inf
            ):
                lbrack /= 2
                rbrack *= 2

            res = optimize.root_scalar(
                func, bracket=(lbrack, rbrack), rtol=1e-14, xtol=1e-14
            )
            scale = res.root
...
class gumbel_l_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        (
            loc_r,
            scale_r,
        ) = gumbel_r.fit(-np.asarray(data), *args, **kwds)
...
class halflogistic_gen(rv_continuous):
    ...
    def _isf(self, q):
        return xpx.apply_where(
            q < 0.5, q, lambda q: -sc.logit(0.5 * q), lambda q: 2 * np.arctanh(1 - q)
        )
...
class invgamma_gen(rv_continuous):
    ...
    def _stats(self, a, moments="mvsk"):
        m1 = xpx.apply_where(a > 1, a, lambda x: 1.0 / (x - 1.0), fill_value=np.inf)
        m2 = xpx.apply_where(
            a > 2, a, lambda x: 1.0 / (x - 1.0) ** 2 / (x - 2.0), fill_value=np.inf
        )
        ...
        if "s" in moments:
            g1 = xpx.apply_where(
                a > 3,
                a,
                lambda x: 4.0 * np.sqrt(x - 2.0) / (x - 3.0),
                fill_value=np.nan,
            )
        if "k" in moments:
            g2 = xpx.apply_where(
                a > 4,
                a,
                lambda x: 6.0 * (5.0 * x - 11.0) / (x - 3.0) / (x - 4.0),
                fill_value=np.nan,
            )
    ...
    def _entropy(self, a):
        ...
        def asymptotic(a):
            ...
            # psi(a) ~ ln(a) - 1 / (2 * a)
            h = (
                (1 - 3 * np.log(a) + np.log(2) + np.log(np.pi)) / 2
                + 2 / 3 * a**-1.0
                + a**-2.0 / 12
                - a**-3.0 / 90
                - a**-4.0 / 120
...
class invgauss_gen(rv_continuous):
    ...
    def _pdf(self, x, mu):
        ...
        return (
            1.0
            / np.sqrt(2 * np.pi * x**3.0)
            * np.exp(-1.0 / (2 * x) * (x / mu - 1) ** 2)
        )
    ...
    def fit(self, data, *args, **kwds):
        ...
        if (
            isinstance(data, CensoredData)
            or isinstance(self, wald_gen)
            or method.lower() == "mm"
        ):
            return super().fit(data, *args, **kwds)

        data, fshape_s, floc, fscale = _check_fit_input_parameters(
            self, data, args, kwds
        )
        """
...
class geninvgauss_gen(rv_continuous):
    ...
    def _logpdf(self, x, p, b):
        ...
        if np.isnan(z).any():
            msg = (
                "Infinite values encountered in scipy.special.kve(p, b). "
                "Values replaced by NaN to avoid incorrect results."
            )
            warnings.warn(msg, RuntimeWarning, stacklevel=3)
        return z
    ...
    def _rvs(self, p, b, size=None, random_state=None):
        ...
        if np.isscalar(p) and np.isscalar(b):
            ...
            it = np.nditer(
                [p, b], flags=["multi_index"], op_flags=[["readonly"], ["readonly"]]
            )
            while not it.finished:
                ...
                idx = tuple(
                    (it.multi_index[j] if not bc[j] else slice(None))
                    for j in range(-len(size), 0)
                )
                out[idx] = self._rvs_scalar(
                    it[0], it[1], numsamples, random_state
                ).reshape(shp)
                it.iternext()
        ...
        if ratio_unif:
            ...
            while simulated < N:
                ...
                if (simulated == 0) and (i * N >= 50000):
                    msg = (
                        "Not a single random variate could be generated "
    ...
    def _munp(self, n, p, b):
        ...
        if inf_vals.any():
            msg = (
                "Infinite values encountered in the moment calculation "
...
class landau_gen(rv_continuous):
    ...
    def _rvs(self, size=None, random_state=None):
        ...
        S = (
            2
            / np.pi
            * ((pi_2 + U) * np.tan(U) - np.log((pi_2 * W * np.cos(U)) / (pi_2 + U)))
        )
        return S
...
class kappa4_gen(rv_continuous):
    ...
    def _logpdf(self, x, h, k):
        ...
        def f0(x, h, k):
            ...
            logpdf = ...
            """
            return sc.xlog1py(1.0 / k - 1.0, -k * x) + sc.xlog1py(
                1.0 / h - 1.0, -h * (1.0 - k * x) ** (1.0 / k)
            )
...
def _ncx2_log_pdf(x, df, nc):
    ...
    return xpx.apply_where(
        corr > 0, (res, corr), lambda r, c: r + np.log(c), fill_value=-np.inf
    )
...
class ncx2_gen(rv_continuous):
    ...
    def _logpdf(self, x, df, nc):
        return xpx.apply_where(
            nc != 0, (x, df, nc), _ncx2_log_pdf, lambda x, df, _: chi2._logpdf(x, df)
        )

    def _pdf(self, x, df, nc):
        with np.errstate(over="ignore"):  # see gh-17432
            return xpx.apply_where(
                nc != 0, (x, df, nc), scu._ncx2_pdf, lambda x, df, _: chi2._pdf(x, df)
            )

    def _cdf(self, x, df, nc):
        with np.errstate(over="ignore"):  # see gh-17432
            return xpx.apply_where(
                nc != 0, (x, df, nc), scu._ncx2_cdf, lambda x, df, _: chi2._cdf(x, df)
            )

    def _ppf(self, q, df, nc):
        with np.errstate(over="ignore"):  # see gh-17432
            return xpx.apply_where(
                nc != 0, (q, df, nc), scu._ncx2_ppf, lambda x, df, _: chi2._ppf(x, df)
            )

    def _sf(self, x, df, nc):
        with np.errstate(over="ignore"):  # see gh-17432
            return xpx.apply_where(
                nc != 0, (x, df, nc), scu._ncx2_sf, lambda x, df, _: chi2._sf(x, df)
            )

    def _isf(self, x, df, nc):
        with np.errstate(over="ignore"):  # see gh-17432
            return xpx.apply_where(
                nc != 0, (x, df, nc), scu._ncx2_isf, lambda x, df, _: chi2._isf(x, df)
            )
    ...
    def _stats(self, df, nc):
        ...
        _ncx2_skewness = (
            np.sqrt(8.0) * k_plus_cl(df, nc, 3) / np.sqrt(k_plus_cl(df, nc, 2.0) ** 3)
        )
        _ncx2_kurtosis_excess = (
            12.0 * k_plus_cl(df, nc, 4.0) / k_plus_cl(df, nc, 2.0) ** 2
        )
        return (
            _ncx2_mean,
            _ncx2_variance,
...
class ncf_gen(rv_continuous):
    ...
    def _stats(self, dfn, dfd, nc, moments="mv"):
        ...
        g2 = (
            scu._ncf_kurtosis_excess(dfn, dfd, nc) - 3  # isn't really excess kurtosis!
            if "k" in moments
            else None
        )
    ...
    def fit(self, data, *args, **kwds):
        if kwds.get("method", None) == "MM":
            raise NotImplementedError(
                "Fit `method='MM'` is not available for "
...
class powerlaw_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        if fscale is not None:
            if fscale <= 0:
                raise ValueError(
                    "Negative or zero `fscale` is outside the "
...
class irwinhall_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        fit_notes = (
            "The generic `fit` implementation is unreliable for this "
    ...
    def _munp(self, order, n):
        ...
        def vmunp(order, n):
            ...
            return sc.stirling2(n + order, n, exact=True) / sc.comb(
                n + order, n, exact=True
            )
...
class recipinvgauss_gen(rv_continuous):
    ...
    def _logpdf(self, x, mu):
        return xpx.apply_where(
            ...
            (x, mu),
            lambda x, mu: (
                -((1 - mu * x) ** 2.0) / (2 * x * mu**2.0) - 0.5 * np.log(2 * np.pi * x)
            ),
            fill_value=-np.inf,
        )
...
class skewnorm_gen(rv_continuous):
    ...
    def _pdf(self, x, a):
        return xpx.apply_where(
            ...
            lambda x, a: 2.0 * _norm_pdf(x) * _norm_cdf(a * x),
        )
    ...
    def _munp(self, order, a):
        if order % 2:
            if order > 19:
                raise NotImplementedError(
                    "skewnorm noncentral moments not "
    ...
    def fit(self, data, *args, **kwds):
        ...
        def skew_d(d):  # skewness in terms of delta
            return (
                (4 - np.pi)
                / 2
                * ((d * np.sqrt(2 / np.pi)) ** 3 / (1 - 2 * d**2 / np.pi) ** (3 / 2))
            )
    ...
    def _cdf(self, x, c, d):
        return _lazyselect(
            ...
            [
                ...
                lambda x, c, d: 1 - ((1 - x) ** 2 / (d - c + 1) / (1 - d)),
            ],
            (x, c, d),
        )
...
class truncnorm_gen(rv_continuous):
    ...
    def _isf(self, q, a, b):
        ...
        def isf_right(q, a, b):
            log_Phi_x = _log_diff(
                _norm_logcdf(-a), np.log1p(-q) + _log_gauss_mass(a, b)
            )
            return -sc.ndtri_exp(np.real(log_Phi_x))
    ...
    def _munp(self, n, a, b):
        def n_th_moment(n, a, b):
            ...
            for k in range(1, n + 1):
                ...
                vals = xpx.apply_where(
                    cond, (probs, ab), lambda x, y: x * y ** (k - 1), fill_value=0
                )
    ...
    def _stats(self, a, b, moments="mv"):
        ...
        def _truncnorm_stats_scalar(a, b, pA, pB):
            ...
            probs = np.asarray([pA, -pB])
            cond = probs != 0
            vals = xpx.apply_where(cond, (probs, ab), lambda x, y: x * y, fill_value=0)
            m2 = 1 + np.sum(vals)
            vals = xpx.apply_where(
                cond, (probs, ab - mu), lambda x, y: x * y, fill_value=0
            )
            # mu2 = m2 - mu**2, but not as numerically stable as:
            # mu2 = (a-mu)*pA - (b-mu)*pB + 1
            mu2 = 1 + np.sum(vals)
            vals = xpx.apply_where(
                cond, (probs, ab), lambda x, y: x * y**2, fill_value=0
            )
            m3 = 2 * m1 + np.sum(vals)
            vals = xpx.apply_where(
                cond, (probs, ab), lambda x, y: x * y**3, fill_value=0
            )
            m4 = 3 * m2 + np.sum(vals)
...
class truncpareto_gen(rv_continuous):
    ...
    def _entropy(self, b, c):
        return -(
            np.log(b / (1 - 1 / c**b)) + (b + 1) * (np.log(c) / (c**b - 1) - 1 / b)
        )
    ...
    def fit(self, data, *args, **kwds):
        ...
        if (
            ...
            if fb is None:
                ...
                i = 0
                # Iteratively look for a bracket for root_scalar.
                while lbrack > -np.inf and (
                    dL_dLoc(lbrack, fb) * dL_dLoc(rbrack, fb) >= 0
                ):
                    i += 1
                    lbrack = rbrack - 2**i
                if not lbrack > -np.inf:
...
class tukeylambda_gen(rv_continuous):
    ...
    def _get_support(self, lam):
        b = xpx.apply_where(lam > 0, lam, lambda lam: 1 / lam, fill_value=np.inf)
        return -b, b
...
class crystalball_gen(rv_continuous):
    ...
    def _cdf(self, x, beta, m):
        ...
        def rhs(x, beta, m):
            return (m / beta) * np.exp(-(beta**2) / 2.0) / (m - 1) + _norm_pdf_C * (
                _norm_cdf(x) - _norm_cdf(-beta)
            )
    ...
    def _sf(self, x, beta, m):
        ...
        def rhs(x, beta, m):
            ...
            M = m / beta / (m - 1) * np.exp(-(beta**2) / 2) + _norm_pdf_C * _norm_cdf(
                beta
            )
    ...
    def _rvs(self, chi, size=None, random_state=None):
        ...
        if chi.size == 1:
            ...
            while not it.finished:
                ...
                r = self._rvs_scalar(
                    it[0], numsamples=numsamples, random_state=random_state
                )
...
class rv_histogram(rv_continuous):
    ...
    def __init__(self, histogram, *args, density=None, **kwargs):
        ...
        if density is None and bins_vary:
            message = (
                "Bin widths are not constant. Assuming `density=True`."
