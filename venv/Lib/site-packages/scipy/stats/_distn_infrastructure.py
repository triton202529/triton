...
_doc_default = "".join(
    [_doc_default_longsummary, _doc_allmethods, "\n", _doc_default_example]
)
...
_doc_default_before_notes = "".join(
    [docdict_discrete["longsummary"], docdict_discrete["allmethods"]]
)
docdict_discrete["before_notes"] = _doc_default_before_notes

_doc_default_disc = "".join(
    [
        docdict_discrete["longsummary"],
...
class rv_generic:
    ...
    def __init__(self, seed=None):
        ...
        self._stats_has_moments = (
            (sig.varkw is not None)
            or ("moments" in sig.args)
            or ("moments" in sig.kwonlyargs)
        )
    ...
    def _construct_doc(self, docdict, shapes_vals=None):
        ...
        for i in range(2):
            ...
            try:
                ...
                raise Exception(
                    "Unable to construct docstring for "
                    f'distribution "{self.name}": {repr(e)}'
                ) from e
    ...
    def _construct_default_doc(
        self, longname=None, docdict=None, discrete="continuous"
    ):
        ...
        self.__doc__ = "".join(
            [
                f"{longname} {discrete} random variable.",
                "\n\n%(before_notes)s\n",
                docheaders["notes"],
                "\n%(example)s",
            ]
    ...
    def _argcheck_rvs(self, *args, **kwargs):
        ...
        ok = all(
            [bcdim == 1 or bcdim == szdim for (bcdim, szdim) in zip(bcast_shape, size_)]
        )
        if not ok:
            raise ValueError(
...
class _ShapeInfo:
    def __init__(
        self, name, integrality=False, domain=(-np.inf, np.inf), inclusive=(True, True)
    ):
        self.name = name
    ...
    **Deepcopying / Pickling**
    ...
    def __init__(
        ...
        self._construct_argparser(
            meths_to_inspect=[self._pdf, self._cdf],
        ...
        if sys.flags.optimize < 2:
            ...
            if self.__doc__ is None:
                self._construct_default_doc(
                    longname=longname, docdict=docdict, discrete="continuous"
                )
            else:
                dct = dict(distcont)
    ...
    def _reduce_func(self, args, kwds, data=None):
        ...
        if method == "mm":
            ...
            raise ValueError(
                f"Method '{method}' not available; " f"must be one of {methods}"
            )
    ...
    def _moment_error(self, theta, x, data_moments):
        ...
        dist_moments = np.array(
            [
                self.moment(i + 1, *args, loc=loc, scale=scale)
        ...
        return (
            ((data_moments - dist_moments) / np.maximum(np.abs(data_moments), 1e-8))
            ** 2
        ).sum()
...
class rv_discrete(rv_generic):
    ...
    **Deepcopying / Pickling**
    ...
    def __init__(
        ...
        self._construct_argparser(
            meths_to_inspect=[self._pmf, self._cdf],
            locscale_in="loc=0",
    ...
    def _construct_docstrings(self, name, longname):
        ...
        if sys.flags.optimize < 2:
            ...
            if self.__doc__ is None:
                self._construct_default_doc(
                    longname=longname, docdict=docdict_discrete, discrete="discrete"
                )
            else:
    ...
    def _entropy(self, *args):
        if hasattr(self, "pk"):
            ...
            _a, _b = self._get_support(*args)
            return _expect(
                lambda x: entr(self._pmf(x, *args)),
                _a,
                _b,
                self._ppf(0.5, *args),
                self.inc,
...
def _expect(fun, lb, ub, x0, inc, maxcount=1000, tolerance=1e-10, chunksize=32):
    ...
    for x in _iter_chunked(x0, ub + 1, chunksize=chunksize, inc=inc):
        ...
        if count > maxcount:
            warnings.warn(
                "expect(): sum did not converge", RuntimeWarning, stacklevel=3
            )
            return tot
...
class rv_sample(rv_discrete):
    ...
    def _pmf(self, x):
        return np.select(
            [x == k for k in self.xk],
            [np.broadcast_arrays(p, x)[0] for p in self.pk],
            0,
        )
