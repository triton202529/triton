...
def cramervonmises(rvs, cdf, args=()):
    ...
    p = np.clip(1.0 - _cdf_cvm(w, n), 0.0, None)
    ...
    *p*-value returned by `kendalltau` with the statistic returned by
    ...
    if x.ndim == 1:
        ...
        if np.any(x != x.astype(int)):
            raise ValueError(
                "All elements of the contingency table must be " "integer."
            )
...
def barnard_exact(table, alternative="two-sided", pooled=True, n=32):
    ...
    * :math:`T(X) \geq |T(X_0)|` when `alternative` = "two-sided".
    ...
    if n <= 0:
        raise ValueError(
            "Number of points `n` must be strictly positive, " f"found {n!r}"
        )
...
class TukeyHSDResult:
    ...
    def __str__(self):
        ...
        for i in range(self.pvalue.shape[0]):
            for j in range(self.pvalue.shape[0]):
                if i != j:
                    s += (
                        f" ({i} - {j}) {self.statistic[i, j]:>10.3f}"
    ...
    def confidence_interval(self, confidence_level=0.95):
        ...
        if (
            self._ci is not None
            and self._ci_cl is not None
            and confidence_level == self._ci_cl
        ):
            return self._ci
...
def tukey_hsd(*args, equal_var=True):
    ...
    return TukeyHSDResult(mean_differences, pvalues, ntreatments, df, stand_err)
