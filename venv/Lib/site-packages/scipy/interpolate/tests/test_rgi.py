...
class TestRegularGridInterpolator:
    ...
    def test_derivatives(self):
        ...
        xp_assert_close(
            interp(sample, nu=(1, 0, 0, 0)), np.asarray([1.0, 1, 1]), atol=1e-15
        )
        xp_assert_close(
            interp(sample, nu=(0, 1, 0, 0)), np.asarray([10.0, 10, 10]), atol=1e-15
        )

        # 2nd derivatives of a linear function are zero
        xp_assert_close(
            interp(sample, nu=(0, 1, 1, 0)), np.asarray([0.0, 0, 0]), atol=2e-12
        )
    ...
    def test_length_one_axis(self):
        ...
        xp_assert_close(interp([[1, 0.3], [1, 11.5]]), [1.3, 12.5], atol=1e-15)

        xp_assert_close(interp([[1.5, 0.3], [1.9, 11.5]]), [1.3, 12.5], atol=1e-15)
        ...
        xp_assert_close(
            interp([[1.5, 1.8], [-4, 5.1]]), np.asarray([3.0, 6]), atol=1e-15
        )
