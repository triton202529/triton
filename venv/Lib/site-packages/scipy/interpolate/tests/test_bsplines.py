...
class TestBSpline:
    ...
    def test_bernstein(self):
        ...
        xp_assert_close(splev(xx, (t, c, k)), bspl(xx), atol=1e-14)
    ...
    def test_extrap(self):
        ...
        xp_assert_close(b(xx[mask], extrapolate=True), b(xx[mask], extrapolate=False))

        # extrapolated values agree with FITPACK
        xp_assert_close(b(xx, extrapolate=True), splev(xx, (t, c, k), ext=0))
    ...
    def test_derivative_jumps(self):
        ...
        xp_assert_close(b(x[x != 6] - 1e-10), b(x[x != 6] + 1e-10))
        assert not np.allclose(b(6.0 - 1e-10), b(6 + 1e-10))

        # 1st derivative jumps at double knots, 1 & 6:
        x0 = np.asarray([3, 4])
        xp_assert_close(b(x0 - 1e-10, nu=1), b(x0 + 1e-10, nu=1))
        x1 = np.asarray([1, 6])
    ...
    def test_basis_element_quadratic(self):
        ...
        xp_assert_close(b(xx), splev(xx, (b.t, b.c, b.k)), atol=1e-14)
        xp_assert_close(b(xx), B_0123(xx), atol=1e-14)

        b = BSpline.basis_element(t=[0, 1, 1, 2])
        xx = np.linspace(0, 2, 10)
        xp_assert_close(b(xx), np.where(xx < 1, xx * xx, (2.0 - xx) ** 2), atol=1e-14)
    ...
    def test_integral(self):
        ...
        xp_assert_close(b.integrate(0.5, 1.5), np.asarray(i(1.5) - i(0.5)))
        xp_assert_close(b.integrate(1.5, 3), np.asarray(i(1) - i(0) + i(2) - i(1.5)))
        xp_assert_close(
            b.integrate(1.5 + 12, 3 + 12), np.asarray(i(1) - i(0) + i(2) - i(1.5))
        )
        xp_assert_close(
            b.integrate(1.5, 3 + 12),
        ...
        xp_assert_close(b.integrate(0, -9), np.asarray(i(1) - i(2) - 4 * period_int))
    ...
    def test_integrate_ppoly(self):
        ...
        for x0, x1 in [(-5, 0.5), (0.5, 5), (-4, 13)]:
            xp_assert_close(b.integrate(x0, x1), p.integrate(x0, x1))
    ...
    def test_design_matrix_same_as_BSpline_call(self, extrapolate, degree):
        ...
        k = degree
        t = np.r_[
            np.linspace(xmin - 2, xmin - 1, degree),
    ...
    def test_design_matrix_t_shapes(self):
        ...
        xp_assert_close(des_matr, [[0.25, 0.58333333, 0.16666667, 0.0]], atol=1e-14)
    ...
    def test_memmap(self, tmpdir):
        ...
        t_mm = np.memmap(
            str(tmpdir.join(f"t{tid}.dat")), mode="w+", dtype=b.t.dtype, shape=b.t.shape
        )
        t_mm[:] = b.t
        c_mm = np.memmap(
            str(tmpdir.join(f"c{tid}.dat")), mode="w+", dtype=b.c.dtype, shape=b.c.shape
        )
        c_mm[:] = b.c
        b.t = t_mm
        b.c = c_mm
...
class TestInsert:
    ...
    def test_insert(self, xval):
        ...
        xp_assert_close(
            spl_yy1.c,
            np.c_[spl.insert_knot(xval).c, spl_y1.insert_knot(xval).c],
            atol=1e-15,
        )
    ...
    @pytest.mark.parametrize(
        "xv", [0, 0.1, 2.0, 4.0, 4.5, 5.5, 6.0, 6.1, 7.0]  # l.h. edge  # r.h. edge
    )
...
class TestInterop:
    ...
    def test_splev(self):
        ...
        xp_assert_close(splev(xnew, b), b(xnew), atol=1e-15, rtol=1e-15)
        xp_assert_close(splev(xnew, b.tck), b(xnew), atol=1e-15, rtol=1e-15)
        xp_assert_close(
            np.asarray([splev(x, b) for x in xnew]), b(xnew), atol=1e-15, rtol=1e-15
        )
        ...
        tck = (b2.t, cc, b2.k)
        xp_assert_close(
            np.asarray(splev(xnew, tck)), b2(xnew).transpose(sh), atol=1e-15, rtol=1e-15
        )
    ...
    def test_splint(self):
        ...
        xp_assert_close(
            splint(0, 1, b), splint(0, 1, b.tck), atol=1e-14, check_0d=False
        )
        xp_assert_close(splint(0, 1, b), b.integrate(0, 1), atol=1e-14, check_0d=False)
        ...
        xp_assert_close(integr, splint(0, 1, b), atol=1e-14, check_shape=False)
    ...
    def test_insert(self):
        ...
        xp_assert_close(splev(xx, bn), splev(xx, tck_n), atol=1e-15)
...
class TestInterp:
    ...
    def test_cubic_deriv(self):
        ...
        xp_assert_close(b(self.xx), self.yy, atol=1e-14, rtol=1e-14)
        xp_assert_close(
            np.asarray([b(self.xx[0], 1), b(self.xx[-1], 1)]),
            np.asarray([der_l[0][1], der_r[0][1]]),
            atol=1e-14,
            rtol=1e-14,
        )
    ...
    def test_quintic_derivs(self):
        ...
        xp_assert_close(
            np.asarray([b(x[0], 1), b(x[0], 2)]),
            np.asarray([val for (nu, val) in der_l]),
        )
        xp_assert_close(
            np.asarray([b(x[-1], 1), b(x[-1], 2)]),
...
class TestLSQ:
    ...
    def test_multiple_rhs_2(self, method):
        ...
        bb = [make_lsq_spline(x, y[:, i], t, k, method=method) for i in range(nrhs)]
    ...
    def test_complex(self, method):
        ...
        xp_assert_close(b(x), b_re(x) + 1.0j * b_im(x), atol=1e-15, rtol=1e-15)
...
class TestGivensQR:
    ...
    def test_vs_full(self):
        ...
        # signs may differ
        xp_assert_close(
            np.minimum(R.todense() + r, R.todense() - r), np.zeros_like(r), atol=1e-15
        )
        xp_assert_close(
            np.minimum(abs(qTy - y_[:, 0]), abs(qTy + y_[:, 0])),
            np.zeros_like(qTy),
            atol=2e-13,
        )
...
class TestSmoothingSpline:
    ...
    def test_invalid_input(self):
        ...
        y = x**2 * np.sin(4 * x) + x**3 + rng.normal(0.0, 1.5, n)
    ...
    def test_non_regularized_case(self):
        ...
        xp_assert_close(spline_GCV(grid), spline_interp(grid), atol=1e-15)
    ...
    def test_weighted_smoothing_spline(self):
        ...
        for ind in rng.choice(range(100), size=10):
            ...
            if orig < weighted:
                raise ValueError(
                    f"Spline with weights should be closer to the"
...
def bspline2(xy, t, c, k):
    ...
    assert ny >= k + 1
    res = sum(
        c[ix, iy] * B(x, k, ix, tx) * B(y, k, iy, ty)
        for ix in range(nx)
        for iy in range(ny)
    )
...
class NdBSpline0:
    ...
    def __call__(self, x):
        ...
        for idx in itertools.product(*iters):
            term = self.c[idx] * np.prod(
                [B(x[d], self.k[d], idx[d], self.t[d]) for d in range(ndim)]
            )
            result += term
...
class TestNdBSpline:
    ...
    def test_1D(self):
        ...
        xp_assert_close(nb(xi[:, None]), b(xi), atol=1e-14)
    ...
    def test_2D_separable(self):
        ...
        xp_assert_close(
            np.asarray([bspline2(xy, t2, c2, k) for xy in xi]),
            np.asarray(target),
            check_shape=False,
            atol=1e-14,
        )
        ...
        xp_assert_close(bspl2(xi), target, atol=1e-14)
        ...
        xp_assert_close(result.ravel(), x**3 * (y**3 + 2 * y), atol=1e-14)
    ...
    def test_2D_separable_2(self):
        ...
        xp_assert_close(
            bspl2_4(xi), np.asarray(target)[:, None], check_shape=False, atol=5e-14
        )
        ...
        xp_assert_close(
            result, [[val_single, val_single], [val_single, val_single]], atol=1e-14
        )
    ...
    def test_2D_random(self):
        ...
        xi = (1.0, 1.0)
        xp_assert_close(spl(xi), bspline2(xi, (tx, ty), c, k), atol=1e-14)

        xi = np.c_[[1, 1.5, 2], [1.1, 1.6, 2.1]]
        xp_assert_close(
            spl(xi), [bspline2(xy, (tx, ty), c, k) for xy in xi], atol=1e-14
        )
    ...
    def test_2D_derivative(self):
        ...
        der = bspl2(xi, nu=(1, 0))
        xp_assert_close(der, [3 * x**2 * (y**2 + 2 * y) for x, y in xi], atol=1e-14)

        der = bspl2(xi, nu=(1, 1))
        xp_assert_close(der, [3 * x**2 * (2 * y + 2) for x, y in xi], atol=1e-14)

        der = bspl2(xi, nu=(0, 0))
        xp_assert_close(der, [x**3 * (y**2 + 2 * y) for x, y in xi], atol=1e-14)
    ...
    def test_2D_mixed_random(self):
        ...
        xp_assert_close(bspl2(xi), [bspl2_0(xp) for xp in xi], atol=1e-14)
    ...
    def test_3D_derivative(self):
        ...
        xp_assert_close(
            bspl3(xi, nu=(1, 0, 0)),
            3 * x**2 * (y**3 + 2 * y) * (z**3 + 3 * z + 1),
            atol=1e-14,
        )

        xp_assert_close(
            bspl3(xi, nu=(2, 0, 0)),
            6 * x * (y**3 + 2 * y) * (z**3 + 3 * z + 1),
            atol=1e-14,
        )

        xp_assert_close(
            bspl3(xi, nu=(2, 1, 0)),
            6 * x * (3 * y**2 + 2) * (z**3 + 3 * z + 1),
            atol=1e-14,
        )

        xp_assert_close(
            bspl3(xi, nu=(2, 1, 3)), 6 * x * (3 * y**2 + 2) * (6), atol=1e-14
        )

        xp_assert_close(bspl3(xi, nu=(2, 1, 4)), np.zeros(len(xi)), atol=1e-14)
    ...
    def test_3D_random_complex(self):
        ...
        tz = np.r_[0, 0, 0, 0, np.sort(rng.uniform(size=8)) * 4, 4, 4, 4, 4]
        c = (
            rng.uniform(size=(tx.size - k - 1, ty.size - k - 1, tz.size - k - 1))
        ...
        xi = np.c_[[1, 1.5, 2], [1.1, 1.6, 2.1], [0.9, 1.4, 1.9]]
        xp_assert_close(spl(xi), spl_re(xi) + 1j * spl_im(xi), atol=1e-14)
...
class TestMakeND:
    ...
    def test_2D_separable_trailing_dims(self):
        ...
        xp_assert_close(result.reshape(6, 6, 4), target, atol=1e-14)
        ...
        xp_assert_close(
            result.reshape(6, 6, 2, 2), target.reshape((6, 6, 2, 2)), atol=1e-14
        )
...
class TestGenerateKnots:
    ...
    def test_s_switch(self):
        ...
        wanted = [
            ...
            [0.0, 0.0, 0.0, 0.0, 4.0, 7.0, 7.0, 7.0, 7.0],
            [0.0, 0.0, 0.0, 0.0, 2.0, 4.0, 7.0, 7.0, 7.0, 7.0],
            [0.0, 0.0, 0.0, 0.0, 2.0, 4.0, 6.0, 7.0, 7.0, 7.0, 7.0],
            [0.0, 0.0, 0.0, 0.0, 2.0, 3.0, 4.0, 5.0, 7, 7.0, 7.0, 7.0],
        ]
    ...
    def test_nest(self):
        ...
        xp_assert_close(
            knots[-1], [0.0, 0.0, 0.0, 0.0, 2.0, 4.0, 7.0, 7.0, 7.0, 7.0], atol=1e-15
        )
...
class TestMakeSplrep:
    ...
    def test_s_too_small(self):
        ...
        with suppress_warnings() as sup:
            ...
            xp_assert_close(np.r_[spl.c, [0] * (spl.k + 1)], tck[1], atol=5e-13)
...
class TestMakeSplprep:
    ...
    def test_simple_vs_splprep(self, s):
        ...
        # values: note axis=1
        xp_assert_close(spl(u), BSpline(t, c, k, axis=1)(u), atol=1e-15)
