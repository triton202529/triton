...
class TestInterp1D:
    ...
    def test_validation(self):
        ...
        interp1d(
            self.x2, self.y210, kind="linear", axis=0, fill_value=(np.ones(10), -1)
        )
        ...
        assert_raises(
            ...
            self.y10,
            kind="linear",
            fill_value=np.array((-1, -1, -1)),
        )  # doesn't broadcast
        assert_raises(
            ValueError, interp1d, self.x10, self.y10, kind="linear", fill_value=[[-1]]
        )  # doesn't broadcast
        assert_raises(
            ValueError, interp1d, self.x10, self.y10, kind="linear", fill_value=[-1, -1]
        )  # doesn't broadcast
        assert_raises(
            ValueError,
            interp1d,
        ...
        assert_raises(
            ValueError, interp1d, self.x10, self.y10, kind="linear", fill_value=()
        )  # doesn't broadcast
        assert_raises(
            ValueError,
            interp1d,
    ...
    def test_assume_sorted(self):
        ...
        assert_array_almost_equal(
            interp10_unsorted([2.4, 5.6, 6.0]), interp10([2.4, 5.6, 6.0])
        )

        # Check assume_sorted keyword (defaults to False)
        interp10_assume_kw = interp1d(
            self.x10[::-1], self.y10[::-1], assume_sorted=False
        )
        assert_array_almost_equal(interp10_assume_kw(self.x10), self.y10)

        interp10_assume_kw2 = interp1d(
            self.x10[::-1], self.y10[::-1], assume_sorted=True
        )
        ...
        assert_array_almost_equal(
            interp10_y_2d(self.x10), interp10_y_2d_unsorted(self.x10)
        )
    ...
    def _check_linear(self, kind):
        ...
        xp_assert_close(
            extrapolator([-1.0, 0, 9, 11]), np.asarray([-1.0, 0, 9, 11]), rtol=1e-14
        )
    ...
    def test_nearest(self):
        ...
        extrapolator = interp1d(
            self.x10, self.y10, kind="nearest", fill_value="extrapolate"
        )
        xp_assert_close(extrapolator([-1.0, 0, 9, 11]), [0.0, 0, 9, 9], rtol=1e-14)
    ...
    def test_nearest_up(self):
        ...
        # test fill_value="extrapolate"
        extrapolator = interp1d(
            self.x10, self.y10, kind="nearest-up", fill_value="extrapolate"
        )
        xp_assert_close(extrapolator([-1.0, 0, 9, 11]), [0.0, 0, 9, 9], rtol=1e-14)
    ...
    def test_previous(self):
        ...
        # test fill_value="extrapolate"
        extrapolator = interp1d(
            self.x10, self.y10, kind="previous", fill_value="extrapolate"
        )
        xp_assert_close(extrapolator([-1.0, 0, 9, 11]), [np.nan, 0, 9, 9], rtol=1e-14)

        # Tests for gh-9591
        interpolator1D = interp1d(
            self.x10, self.y10, kind="previous", fill_value="extrapolate"
        )
        xp_assert_close(
            interpolator1D([-1, -2, 5, 8, 12, 25]), [np.nan, np.nan, 5, 8, 9, 9]
        )

        interpolator2D = interp1d(
            self.x10, self.y210, kind="previous", fill_value="extrapolate"
        )
        xp_assert_close(
            interpolator2D([-1, -2, 5, 8, 12, 25]),
            [[np.nan, np.nan, 5, 8, 9, 9], [np.nan, np.nan, 15, 18, 19, 19]],
        )

        interpolator2DAxis0 = interp1d(
            self.x10, self.y102, kind="previous", axis=0, fill_value="extrapolate"
        )
        xp_assert_close(
            interpolator2DAxis0([-2, 5, 12]), [[np.nan, np.nan], [10, 11], [18, 19]]
        )
        ...
        xp_assert_close(
            interpolator1D([-2, -1, 0, 1, 2, 3, 5]), [np.nan, np.nan, 0, 1, -1, -1, -1]
        )

        interpolator1D = interp1d(
            [2, 0, 1],  # x is not ascending
            [-1, 0, 1],
            kind="previous",
            fill_value="extrapolate",
            assume_sorted=False,
        ...
        interpolator2D = interp1d(
            self.x10, self.y210_edge_updated, kind="previous", fill_value="extrapolate"
        )
        xp_assert_close(
            interpolator2D([-1, -2, 5, 8, 12, 25]),
            [[np.nan, np.nan, 5, 8, -30, -30], [np.nan, np.nan, 15, 18, -30, -30]],
        )

        interpolator2DAxis0 = interp1d(
            self.x10,
            self.y102_edge_updated,
            kind="previous",
            axis=0,
            fill_value="extrapolate",
        )
        xp_assert_close(
            interpolator2DAxis0([-2, 5, 12]), [[np.nan, np.nan], [10, 11], [-30, -30]]
        )
    ...
    def test_next(self):
        ...
        assert_array_almost_equal(
            interp10([2.4, 5.6, 6.0]),
            np.array([3.0, 6.0, 6.0]),
        )

        # test fill_value="extrapolate"
        extrapolator = interp1d(
            self.x10, self.y10, kind="next", fill_value="extrapolate"
        )
        xp_assert_close(extrapolator([-1.0, 0, 9, 11]), [0, 0, 9, np.nan], rtol=1e-14)
        ...
        xp_assert_close(
            interpolator1D([-1, -2, 5, 8, 12, 25]), [0, 0, 5, 8, np.nan, np.nan]
        )

        interpolator2D = interp1d(
            self.x10, self.y210, kind="next", fill_value="extrapolate"
        )
        xp_assert_close(
            interpolator2D([-1, -2, 5, 8, 12, 25]),
            [[0, 0, 5, 8, np.nan, np.nan], [10, 10, 15, 18, np.nan, np.nan]],
        )

        interpolator2DAxis0 = interp1d(
            self.x10, self.y102, kind="next", axis=0, fill_value="extrapolate"
        )
        xp_assert_close(
            interpolator2DAxis0([-2, 5, 12]), [[0, 1], [10, 11], [np.nan, np.nan]]
        )
        ...
        xp_assert_close(
            interpolator1D([-2, -1, 0, 1, 2, 3, 5]), [0, 0, 0, 1, -1, np.nan, np.nan]
        )
        ...
        interpolator2D = interp1d(
            self.x10, self.y210_edge_updated, kind="next", fill_value="extrapolate"
        )
        xp_assert_close(
            interpolator2D([-1, -2, 5, 8, 12, 25]),
            [[30, 30, 5, 8, np.nan, np.nan], [30, 30, 15, 18, np.nan, np.nan]],
        )
        ...
        xp_assert_close(
            interpolator2DAxis0([-2, 5, 12]), [[30, 30], [10, 11], [np.nan, np.nan]]
        )
    ...
    def _check_fill_value(self, kind):
        interp = interp1d(
            self.x10, self.y10, kind=kind, fill_value=(-100, 100), bounds_error=False
        )
        assert_array_almost_equal(interp(10), np.asarray(100.0))
        assert_array_almost_equal(interp(-10), np.asarray(-100.0))
        ...
        for y in (self.y235, self.y325, self.y225, self.y25):
            interp = interp1d(
                self.x5, y, kind=kind, axis=-1, fill_value=100, bounds_error=False
            )
            assert_array_almost_equal(interp(10), np.asarray(100.0))
            assert_array_almost_equal(interp(-10), np.asarray(100.0))
            assert_array_almost_equal(interp([-10, 10]), np.asarray(100.0))
        ...
        assert_array_almost_equal(interp(-10), [[100, 200, 300]] * 2)
        assert_array_almost_equal(
            interp([-10, 10]), [[[100, 100], [200, 200], [300, 300]]] * 2
        )
        ...
        assert_array_almost_equal(interp(-10), [[-100, -200, -300]] * 2)
        assert_array_almost_equal(
            interp([-10, 10]), [[[-100, 100], [-200, 100], [-300, 100]]] * 2
        )
        ...
        for ii in range(2):
            ...
            assert_array_almost_equal(
                interp([-10, 10]),
                [[[100, 100], [200, 200]], [[1000, 1000], [2000, 2000]]],
            )
    ...
    def _nd_check_interp(self, kind="linear"):
        ...
        interp10 = interp1d(self.x10, self.y10, kind=kind)
        assert_array_almost_equal(
            interp10(np.array([[3.0, 5.0], [2.0, 7.0]])),
            np.array([[3.0, 5.0], [2.0, 7.0]]),
        )
        ...
        assert_array_almost_equal(
            interp210(x_new),
            np.array([[[3.0, 5.0], [2.0, 7.0]], [[13.0, 15.0], [12.0, 17.0]]]),
        )
        assert_array_almost_equal(
            interp102(x_new),
            np.array([[[6.0, 7.0], [10.0, 11.0]], [[4.0, 5.0], [14.0, 15.0]]]),
        )
...
class TestAkima1DInterpolator:
    ...
    def test_linear_interpolant_edge_case_2d(self):
        x = np.array([0.0, 1.0])
        y = np.column_stack((x, 2.0 * x, 3.0 * x, 4.0 * x))

        ak = Akima1DInterpolator(x, y)
        xi = np.array([0.5, 1.0])
        yi = np.array([[0.5, 1.0, 1.5, 2.0], [1.0, 2.0, 3.0, 4.0]])
        xp_assert_close(ak(xi), yi)
...
class TestPPoly:
    ...
    def test_periodic(self):
        ...
        xp_assert_close(p(1.3), np.asarray(1 * 0.3**2 + 2 * 0.3 + 3))
        xp_assert_close(p(-0.3), np.asarray(4 * (0.7 - 0.5) ** 2 + 5 * (0.7 - 0.5) + 6))
    ...
    def test_descending(self):
        ...
        for m in [10, 20, 30]:
            ...
            for a, b in rng.uniform(-10, 20, (5, 2)):
                ...
                xp_assert_close(pa_i(b) - pa_i(a), pd_i(b) - pd_i(a), rtol=1e-13)
    ...
    def test_antiderivative_regression_4355(self):
        ...
        xp_assert_close(np.asarray(q(2) - q(0)), np.asarray(1.5))
    ...
    def test_antiderivative_vs_spline(self):
        ...
        for dx in range(0, 10):
            ...
            xi = np.linspace(0, 1, 200)
            xp_assert_close(pp2(xi), splev(xi, spl2), rtol=1e-7)
    ...
    def test_integrate_periodic(self):
        ...
        xp_assert_close(P.integrate(1.5, 2.5), np.asarray(I(2.5) - I(1.5)))
        xp_assert_close(P.integrate(3.5, 5), np.asarray(I(2) - I(1) + I(4) - I(3.5)))
        xp_assert_close(
            P.integrate(3.5 + 12, 5 + 12), np.asarray(I(2) - I(1) + I(4) - I(3.5))
        )
        xp_assert_close(
            P.integrate(3.5, 5 + 12),
            np.asarray(I(2) - I(1) + I(4) - I(3.5) + 4 * period_int),
        )
        xp_assert_close(P.integrate(0, -1), np.asarray(I(2) - I(3)))
        xp_assert_close(P.integrate(-9, -10), np.asarray(I(2) - I(3)))
        xp_assert_close(P.integrate(0, -10), np.asarray(I(2) - I(3) - 3 * period_int))
    ...
    def test_roots_idzero(self):
        ...
        pp = PPoly(c, x)
        xp_assert_equal(pp.roots(), [0.25, 0.4, np.nan, 0.6 + 0.25])
        ...
        xp_assert_equal(pp1.solve(const), [0.25, 0.4, np.nan, 0.6 + 0.25])
...
class TestBPoly:
    def test_simple(self):
        ...
        c = [[3]]
        bp = BPoly(c, x)
        xp_assert_close(bp(0.1), np.asarray(3.0))

    def test_simple2(self):
        x = [0, 1]
        c = [[3], [1]]
        bp = BPoly(c, x)  # 3*(1-x) + 1*x
        xp_assert_close(bp(0.1), np.asarray(3 * 0.9 + 1.0 * 0.1))
    ...
    def test_simple3(self):
        ...
        xp_assert_close(
            bp(0.2), np.asarray(3 * 0.8 * 0.8 + 1 * 2 * 0.2 * 0.8 + 4 * 0.2 * 0.2)
        )
    ...
    def test_simple4(self):
        ...
        xp_assert_close(
            bp(0.3),
            np.asarray(0.7**3 + 3 * 0.7**2 * 0.3 + 3 * 0.7 * 0.3**2 + 2 * 0.3**3),
        )
    ...
    def test_descending(self):
        ...
        for m in [10, 20, 30]:
            ...
            for a, b in rng.uniform(-10, 20, (5, 2)):
                ...
                xp_assert_close(pa_i(b) - pa_i(a), pd_i(b) - pd_i(a), rtol=1e-12)
...
class TestBPolyCalculus:
    def test_derivative(self):
        ...
        xp_assert_close(
            np.asarray([bp(0.4, nu) for nu in [1, 2, 3]]),
            np.asarray([-6 * (1 - 0.4), 6.0, 0.0]),
        )
        xp_assert_close(
            np.asarray([bp(1.7, nu) for nu in [1, 2, 3]]), np.asarray([0.7, 1.0, 0])
        )
    ...
    def test_antiderivative_simple(self):
        ...
        xp_assert_close(
            bi(xx),
            np.where(xx < 1, xx**2 / 2.0, 0.5 * xx * (xx / 2.0 - 1) + 3.0 / 4),
            atol=1e-12,
            rtol=1e-12,
        )
    ...
    def test_der_antider(self):
        ...
        xp_assert_close(
            bp.antiderivative().derivative()(xx), bp(xx), atol=1e-12, rtol=1e-12
        )
    ...
    def test_antider_ppoly(self):
        ...
        xp_assert_close(
            bp.antiderivative(2)(xx), pp.antiderivative(2)(xx), atol=1e-12, rtol=1e-12
        )
    ...
    def test_antider_continuous(self):
        ...
        xx = bp.x[1:-1]
        xp_assert_close(bp(xx - 1e-14), bp(xx + 1e-14), atol=1e-12, rtol=1e-12)
    ...
    def test_integrate_extrap(self):
        ...
        xp_assert_close(b.integrate(0, 2), np.asarray(2.0), atol=1e-14, check_0d=False)
    ...
    def test_integrate_periodic(self):
        ...
        xp_assert_close(P.integrate(3.5 + 12, 5 + 12), I(2) - I(1) + I(4) - I(3.5))
        xp_assert_close(
    ...
    def test_antider_neg(self):
        ...
        xp_assert_close(
            b.derivative(-1)(xx), b.antiderivative()(xx), atol=1e-12, rtol=1e-12
        )
        xp_assert_close(
            b.derivative(1)(xx), b.antiderivative(-1)(xx), atol=1e-12, rtol=1e-12
        )
...
class TestBPolyFromDerivatives:
    ...
    def test_make_poly_12(self):
        ...
        pp = BPoly(c[:, None], [0, 1])
        for j in range(6):
            xp_assert_close(pp(0.0), ya[j], check_0d=False)
            xp_assert_close(pp(1.0), yb[j], check_0d=False)
            pp = pp.derivative()
    ...
    def test_order_zero(self):
        ...
        xi, yi = self._make_random_mk(m, k)
        assert_raises(
            ValueError, BPoly.from_derivatives, **dict(xi=xi, yi=yi, orders=0)
        )
...
class TestNdPPoly:
    ...
    def test_deriv_3d_simple(self):
        ...
        xp_assert_close(ip((xi, yi, zi)), xi * yi**2 * zi**4 / (gamma(3) * gamma(5)))
    ...
    def test_integrate_2d(self):
        ...
        for ranges in [
            ...
            xp_assert_close(
                ig, ig2, rtol=1e-5, atol=1e-5, check_0d=False, err_msg=repr(ranges)
            )
...
def _ppoly2d_eval(c, xs, xnew, ynew, nu=None):
    ...
    for jout, (x, y) in enumerate(zip(xnew, ynew)):
        if not ((xs[0][0] <= x <= xs[0][-1]) and (xs[1][0] <= y <= xs[1][-1])):
            out[jout] = np.nan
            continue
        ...
        for k1 in range(c.shape[0]):
            for k2 in range(c.shape[1]):
                for k3 in range(c.shape[2]):
                    val += (
                        c[nx - k1 - 1, ny - k2 - 1, nz - k3 - 1, j1, j2, j3]
                        * _dpow(s1, k1, nu[0])
                        * _dpow(s2, k2, nu[1])
                        * _dpow(s3, k3, nu[2])
                    )
