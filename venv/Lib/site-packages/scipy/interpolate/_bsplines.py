    ...
    **Implementation details**
    ...
    def design_matrix(cls, x, t, k, extrapolate=False):
        ...
        if t.ndim != 1 or np.any(t[1:] < t[:-1]):
            raise ValueError(
                f"Expect t to be a 1-D sorted array_like, but " f"got t={t}."
            )
        ...
        return csr_array(
            (data, indices, indptr), shape=(x.shape[0], t.shape[0] - k - 1)
        )
    ...
    def integrate(self, a, b, extrapolate=None):
        ...
        if extrapolate == "periodic":
            ...
            if n_periods > 0:
                ...
                out = _dierckx.evaluate_spline(
                    ta, ca.reshape(ca.shape[0], -1), ka, x, 0, False
                )
            ...
            x = np.asarray([a, b], dtype=np.float64)
            out = _dierckx.evaluate_spline(
                ta, ca.reshape(ca.shape[0], -1), ka, x, 0, extrapolate
            )
            integral = out[1] - out[0]
    ...
    def from_power_basis(cls, pp, bc_type="not-a-knot"):
        ...
        if not isinstance(pp, CubicSpline):
            raise NotImplementedError(
                f"Only CubicSpline objects are accepted "
...
def make_interp_spline(x, y, k=3, t=None, bc_type=None, axis=0, check_finite=True):
    ...
    if k == 0:
        if any(_ is not None for _ in (t, deriv_l, deriv_r)):
            raise ValueError(
                "Too much info for k=0: t and bc_type can only " "be None."
            )
        t = np.r_[x, x[-1]]
        c = np.asarray(y)
...
def make_lsq_spline(x, y, t, k=3, w=None, axis=0, check_finite=True, *, method="qr"):
    ...
    **NaN handling**: If the input arrays contain ``nan`` values, the result is
    ...
    if method == "norm-eq":
        ...
        _dierckx._norm_eq_lsq(x, t, k, yy, w, ab.T, rhs)
...
def _compute_optimal_gcv_parameter(X, wE, y, w):
    ...
    if y.ndim == 1:
        ...
        if gcv_est.success:
            return gcv_est.x
        raise ValueError(
            f"Unable to find minimum of the GCV " f"function: {gcv_est.message}"
        )
    elif y.ndim == 2:
...
def make_smoothing_spline(x, y, w=None, lam=None, *, axis=0):
    ...
    for j in range(2, n - 2):
        wE[:, j] = (
            (x[j + 2] - x[j - 2])
            * _coeff_of_divided_diff(x[j - 2 : j + 3])
            / w[j - 2 : j + 3]
        )
