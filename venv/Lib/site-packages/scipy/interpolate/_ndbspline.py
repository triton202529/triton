...
class NdBSpline:
    ...
    def __call__(self, xi, *, nu=None, extrapolate=None):
        ...
        _strides_c1 = np.asarray(
            [s // c1.dtype.itemsize for s in c1.strides], dtype=np.int64
        )

        num_c_tr = c1.shape[-1]  # # of trailing coefficients
        out = _dierckx.evaluate_ndbspline(
            xi,
            self._t,
            self._len_t,
            self._k,
    ...
    def design_matrix(cls, xvals, t, k, extrapolate=True):
        ...
        data, indices, indptr = _dierckx._coloc_nd(
            xvals, _t, len_t, k, _indices_k1d, cstrides
        )
...
def _preprocess_inputs(k, t_tpl):
    ...
    if not isinstance(t_tpl, tuple):
        raise ValueError(
            f"Expect `t` to be a tuple of array-likes. " f"Got {t_tpl} instead."
        )
    ...
    for d in range(ndim):
        ...
        n = td.shape[0] - kd - 1
        if kd < 0:
            raise ValueError(f"Spline degree in dimension {d} cannot be" f" negative.")
        if td.ndim != 1:
            raise ValueError(
        ...
        if n < kd + 1:
            raise ValueError(
                f"Need at least {2*kd + 2} knots for degree" f" {kd} in dimension {d}."
            )
        if (np.diff(td) < 0).any():
            raise ValueError(
        ...
        if len(np.unique(td[kd : n + 1])) < 2:
            raise ValueError(f"Need at least two internal knots in" f" dimension {d}.")
...
def make_ndbspl(points, values, k=3, *, solver=ssl.gcrotmk, **solver_args):
    ...
    t = tuple(
        _not_a_knot(np.asarray(points[d], dtype=float), k[d]) for d in range(ndim)
    )
