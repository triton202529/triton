...
_iermess2 = {
    ...
    -3: [
        "Warning. The coefficients of the spline have been computed as the\n"
...
def splprep(
    ...
    if per:
        for i in range(idim):
            if x[i][0] != x[i][-1]:
                if not quiet:
                    warnings.warn(
                        RuntimeWarning(f"Setting x[{i}][{m}]=x[{i}][0]"), stacklevel=2
                    )
                x[i][-1] = x[i][0]
    if not 0 < idim < 11:
    ...
    t, c, o = _fitpack._parcur(
        ravel(transpose(x)), w, u, ub, ue, k, task, ipar, s, t, nest, wrk, iwrk, per
    )
    _parcur_cache["u"] = o["u"]
...
def splev(x, tck, der=0, ext=0):
    ...
    if parametric:
        return list(
            map(lambda c, x=x, t=t, k=k, der=der: splev(x, [t, c, k], der, ext), c)
        )
    else:
        if not (0 <= der <= k):
...
def sproot(tck, mest=10):
    ...
    if parametric:
        ...
        if ier == 10:
            raise TypeError(
                "Invalid input data. " "t1<=..<=t4<t5<..<tn-3<=..<=tn must hold."
            )
        if ier == 0:
            return z[:m]
        if ier == 1:
            warnings.warn(
                RuntimeWarning("The number of zeros exceeds mest"), stacklevel=2
            )
            return z[:m]
...
def bisplrep(
    ...
    lwrk1 = _int_overflow(
        u * v * (2 + b1 + b2) + 2 * (u + v + km * (m + ne) + ne - kx - ky) + b2 + 1,
        OverflowError,
        msg=msg,
    )
    lwrk2 = _int_overflow(u * v * (b2 + 1) + b2, OverflowError, msg=msg)
    tx, ty, c, o = _fitpack._surfit(
        x,
        y,
        z,
        w,
...
def splder(tck, n=1):
    ...
    if n > k:
        raise ValueError(
            f"Order of derivative (n = {n!r}) must be <= "
    ...
    with np.errstate(invalid="raise", divide="raise"):
        try:
            ...
            raise ValueError(
                ...
                f"and is not differentiable {n} times"
            ) from e
