...
class TestDerivative:
    ...
    def test_basic(self, x, xp):
        ...
        default_dtype = xp.asarray(1.0).dtype
    ...
    def test_vectorization(self, order, shape, xp):
        ...
        xp_assert_close(xp.reshape(res.error, (-1,)), xp.asarray(ref_error), atol=1e-12)
    ...
    def test_convergence(self, xp):
        ...
        x = xp.asarray(1.0, dtype=xp.float64)
        f = special.ndtr
        ref = float(stats.norm.pdf(1.0))
    ...
    def test_vectorized_step_direction_args(self, xp):
        ...
        ref = xp.asarray(ref, dtype=xp.asarray(1.0).dtype)
        xp_assert_close(res.df, ref)
    ...
    def test_initial_step(self, xp):
        ...
        x = xp.asarray(0.0, dtype=xp.float64)
        step_direction = xp.asarray([-1, 0, 1])
        h0 = xp.reshape(xp.logspace(-3, 0, 10), (-1, 1))
        res = derivative(
            f, x, initial_step=h0, order=2, maxiter=1, step_direction=step_direction
        )
    ...
    def test_special_cases(self, xp):
        ...
        if not is_torch(xp):  # torch defaults to float32
            ...
            assert res.success
            xp_assert_close(res.df, xp.asarray(99 * 7.0**98))
        ...
        xp_assert_close(res.df, xp.asarray(3.0))
...
class TestJacobian(JacobianHessianTest):
    ...
    def df2(z):
        r, phi = z
        return [[np.cos(phi), -r * np.sin(phi)], [np.sin(phi), r * np.cos(phi)]]
    ...
    def test_attrs(self, xp):
        ...
        for attr in ["success", "status", "df", "nit", "nfev"]:
            ref_attr = xp.asarray(
                [
                    [getattr(res00, attr), getattr(res01, attr)],
...
class TestHessian(JacobianHessianTest):
    ...
    @pytest.mark.skip_xp_backends(
        np_only=True, reason="Python list input uses NumPy backend"
    )
    def test_small_rtol_warning(self, xp):
        message = "The specified `rtol=1e-15`, but..."
        with pytest.warns(RuntimeWarning, match=message):
            hessian(xp.sin, [1.0], tolerances=dict(rtol=1e-15))
