...
def _derivative_iv(
    ...
    tols = np.asarray(
        [atol if atol is not None else 1, rtol if rtol is not None else 1, step_factor]
    )
    if (
...
def derivative(
    ...
    *,
    ...
    def post_func_eval(x, f, work):
        ...
        work.fs = xp.zeros(
            (ic.shape[0], work.fs.shape[-1] + 2 * n_new), dtype=work.dtype
        )
...
def _derivative_weights(work, n, xp):
    ...
    return (
        xp.asarray(diff_state.central, dtype=work.dtype),
...
def hessian(
    f, x, *, tolerances=None, maxiter=10, order=8, initial_step=0.5, step_factor=2.0
):
    ...
    kwargs = dict(
        maxiter=maxiter, order=order, initial_step=initial_step, step_factor=step_factor
    )
    tolerances = {} if tolerances is None else tolerances
    atol = tolerances.get("atol", None)
    rtol = tolerances.get("rtol", None)
    ...
    message = (
        f"The specified `{rtol=}`, but error estimates are likely to be "
