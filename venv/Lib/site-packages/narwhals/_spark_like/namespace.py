...
class SparkLikeNamespace(
    ...
    def all_horizontal(
        self, *exprs: SparkLikeExpr, ignore_nulls: bool
    ) -> SparkLikeExpr:
        def func(cols: Iterable[Column]) -> Column:
            it = (
                (
                    self._F.coalesce(col, self._F.lit(True)) for col in cols
                )  # noqa: FBT003
                if ignore_nulls
                else cols
            )
    ...
    def any_horizontal(
        ...
        def func(cols: Iterable[Column]) -> Column:
            it = (
                (
                    self._F.coalesce(col, self._F.lit(False)) for col in cols
                )  # noqa: FBT003
                if ignore_nulls
                else cols
            )
    ...
    def concat(
        ...
        if how == "vertical":
            ...
            for i, df in enumerate(dfs[1:], start=1):
                ...
                if not (
                    (len(cols_current) == len(cols_0)) and (cols_current == cols_0)
                ):
                    msg = (
