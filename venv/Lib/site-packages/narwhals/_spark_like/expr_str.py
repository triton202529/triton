...
class SparkLikeExprStringNamespace(
    ...
    def replace_all(self, pattern: str, value: str, *, literal: bool) -> SparkLikeExpr:
        def func(expr: Column) -> Column:
            replace_all_func = (
                self.compliant._F.replace
                if literal
                else self.compliant._F.regexp_replace
            )
    ...
    def starts_with(self, prefix: str) -> SparkLikeExpr:
        return self.compliant._with_elementwise(
            lambda expr: self.compliant._F.startswith(
                expr, self.compliant._F.lit(prefix)
            )
        )
    ...
    def slice(self, offset: int, length: int | None) -> SparkLikeExpr:
        ...
        def func(expr: Column) -> Column:
            ...
            _length = (
                self.compliant._F.lit(length) if length is not None else col_length
            )
