...
class SparkLikeExpr(SQLExpr["SparkLikeLazyFrame", "Column"]):
    ...
    def all(self) -> Self:
        def f(expr: Column) -> Column:
            return self._F.coalesce(
                self._F.bool_and(expr), self._F.lit(True)
            )  # noqa: FBT003
    ...
    def any(self) -> Self:
        def f(expr: Column) -> Column:
            return self._F.coalesce(
                self._F.bool_or(expr), self._F.lit(False)
            )  # noqa: FBT003
    ...
    def median(self) -> Self:
        def _median(expr: Column) -> Column:
            ...
            } and Implementation.PYSPARK._backend_version() < (
                3,
                4,
            ):  # pragma: no cover
    ...
    def clip(
        ...
        def _clip_both(
            expr: Column, lower_bound: Column, upper_bound: Column
        ) -> Column:
            return (
    ...
    def is_nan(self) -> Self:
        def _is_nan(expr: Column) -> Column:
            return self._F.when(self._F.isnull(expr), None).otherwise(
                self._F.isnan(expr)
            )
