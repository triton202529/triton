...
class IbisExpr(SQLExpr["IbisLazyFrame", "ir.Value"]):
    ...
    def all(self) -> Self:
        return self._with_callable(
            lambda expr: expr.all().fill_null(lit(True))
        )  # noqa: FBT003

    def any(self) -> Self:
        return self._with_callable(
            lambda expr: expr.any().fill_null(lit(False))
        )  # noqa: FBT003
    ...
    def rank(self, method: RankMethod, *, descending: bool) -> Self:
        def _rank(expr: ir.Column) -> ir.Column:
            order_by = next(
                self._sort(expr, descending=[descending], nulls_last=[True])
            )
            ...
            if method == "max":
                ...
                avg = cast(
                    "ir.NumericValue",
                    (cnt - cast("ir.IntegerScalar", lit(1))) / lit(2.0),
                )
                rank_ = rank_ + avg
