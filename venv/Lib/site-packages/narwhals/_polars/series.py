...
class PolarsSeries:
    ...
    def from_iterable(
        ...
        *,
        ...
        native = pl.Series(
            name=name, values=cast("Sequence[Any]", data), dtype=dtype_pl
        )
    ...
    def __ge__(self, other: Any) -> Self:
        return self._with_native(
            self.native.__ge__(extract_native(other))
        )  # pyright: ignore[reportArgumentType]

    def __gt__(self, other: Any) -> Self:
        return self._with_native(
            self.native.__gt__(extract_native(other))
        )  # pyright: ignore[reportArgumentType]

    def __le__(self, other: Any) -> Self:
        return self._with_native(
            self.native.__le__(extract_native(other))
        )  # pyright: ignore[reportArgumentType]

    def __lt__(self, other: Any) -> Self:
        return self._with_native(
            self.native.__lt__(extract_native(other))
        )  # pyright: ignore[reportArgumentType]
    ...
    def hist(  # noqa: C901, PLR0912
        ...
        *,
        ...
        if (bins is not None and len(bins) <= 1) or (
            bin_count == 0
        ):  # pragma: no cover
            data: list[pl.Series] = []
        ...
        if self.native.count() < 1:
            ...
            if bins is not None:
                ...
                data_dict = {
                    "breakpoint": pl.int_range(1, bin_count + 1, eager=True)
                    / bin_count,
        ...
        if (self._backend_version < (1, 15)) and (
            ...
            bins = (
                pl.int_range(0, bin_count + 1, eager=True) * width + lower
            ).to_list()
            bin_count = None
