...
class SQLExpr(
    ...
    def _push_down_window_function(
        ...
        def window_f(
            ...
            other_native_series = {
                key: (
                    df._evaluate_window_expr(value, window_inputs)
                    if self._is_expr(value)
                    else self._lit(value)
                )
                for key, value in expressifiable_args.items()
        ...
        def func(
            ...
            return [
                self._when(
                    ...
                    self._window_expression(
                        self._function(func_, expr), **window_kwargs
                    ),
                )
                for expr in self(df)
            ]
    ...
    def _from_elementwise_horizontal_op(
        ...
        def window_function(
            ...
            cols = (
                col
                for _expr in exprs
                for col in _expr.window_function(df, window_inputs)
            )
            return [func(cols)]
    ...
    def cum_count(self, *, reverse: bool) -> Self:
        return self._with_window_function(
            self._cum_window_func("count", reverse=reverse)
        )
