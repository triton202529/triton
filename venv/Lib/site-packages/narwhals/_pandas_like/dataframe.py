...
class PandasLikeDataFrame(
        ...
        for name, series in data.items():
            if isinstance(series, Series):
                ...
                if left_most is None:
                    ...
                    aligned_data[name] = align_and_extract_native(left_most, compliant)[
                        1
                    ]
            else:
                aligned_data[name] = series
    ...
    def from_numpy(
        ...
        *,
        ...
        DataFrame: Constructor = (
            implementation.to_native_namespace().DataFrame
        )  # noqa: N806
    ...
    def simple_select(self, *column_names: str) -> Self:
        return self._with_native(
            select_columns_by_name(
                self.native, list(column_names), self._implementation
            ),
            validate_column_names=False,
        )
    ...
    def gather_every(self, n: int, offset: int) -> Self:
        return self._with_native(
            self.native.iloc[offset::n], validate_column_names=False
        )
    ...
    def explode(self, columns: Sequence[str]) -> Self:
        ...
        if len(columns) == 1:
            ...
            plx = self.__native_namespace__()
            return self._with_native(
                plx.concat([exploded_frame, *exploded_series], axis=1)[
                    original_columns
                ],
                validate_column_names=False,
            )
