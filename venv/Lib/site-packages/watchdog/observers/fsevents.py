...
class FSEventsEmitter(EventEmitter):
    ...
    def _queue_created_event(
        self, event: FileSystemEvent, src_path: bytes | str, dirname: bytes | str
    ) -> None:
        cls = DirCreatedEvent if event.is_directory else FileCreatedEvent
        self.queue_event(cls(src_path))
        self.queue_event(DirModifiedEvent(dirname))

    def _queue_deleted_event(
        self, event: FileSystemEvent, src_path: bytes | str, dirname: bytes | str
    ) -> None:
    ...
    def events_callback(
        self, paths: list[bytes], inodes: list[int], flags: list[int], ids: list[int]
    ) -> None:
    ...
    def on_thread_start(self) -> None:
        if self.suppress_history:
            watch_path = (
                os.fsdecode(self.watch.path)
                if isinstance(self.watch.path, bytes)
                else self.watch.path
            )
