...
class InotifyBuffer(BaseThread):
    ...
    def __init__(
        self, path: bytes, *, recursive: bool = False, event_mask: int | None = None
    ) -> None:
        super().__init__()
        # XXX: Remove quotes after Python 3.9 drop
        self._queue = DelayedQueue["InotifyEvent | tuple[InotifyEvent, InotifyEvent]"](
            self.delay
        )
    ...
    def _group_events(
        self, event_list: list[InotifyEvent]
    ) -> list[InotifyEvent | tuple[InotifyEvent, InotifyEvent]]:
        ...
        for inotify_event in event_list:
            ...
            def matching_from_event(
                event: InotifyEvent | tuple[InotifyEvent, InotifyEvent],
            ) -> bool:
                return (
