    ...
    def __repr__(self) -> str:
        if self.event_filter is not None:
            event_filter_str = "|".join(
                sorted(_cls.__name__ for _cls in self.event_filter)
            )
...
class EventEmitter(BaseThread):
    ...
    def queue_event(self, event: FileSystemEvent) -> None:
        ...
        :type event:
            ...
            or a subclass.
        """
        if self._event_filter is None or any(
            isinstance(event, cls) for cls in self._event_filter
        ):
...
class BaseObserver(EventDispatcher):
    ...
    def __init__(
        ...
        *,
        ...
        self._handlers: defaultdict[ObservedWatch, set[FileSystemEventHandler]] = (
            defaultdict(set)
        )
