...
_orc_msg = (
    "The pyarrow installation is not built with support for the ORC file " "format."
)
...
def _ensure_multiple_sources(paths, filesystem=None):
    ...
    is_local = isinstance(filesystem, (LocalFileSystem, _MockFileSystem)) or (
        isinstance(filesystem, SubTreeFileSystem)
...
def _ensure_write_partitioning(part, schema, flavor):
    if isinstance(part, PartitioningFactory):
        raise ValueError(
            "A PartitioningFactory cannot be used. "
    ...
    if isinstance(part, Partitioning) and flavor:
        ...
        part = partitioning(
            schema=pa.schema([schema.field(f) for f in part]), flavor=flavor
        )
    elif part is None:
    ...
    if not isinstance(part, Partitioning):
        raise ValueError(
            "partitioning must be a Partitioning object or " "a list of column names"
        )
...
'delete_matching'
    ...
    if format != file_options.format:
        raise TypeError(
            f"Supplied FileWriteOptions have format {format}, "
    ...
    partitioning = _ensure_write_partitioning(
        partitioning, schema=partitioning_schema, flavor=partitioning_flavor
    )
