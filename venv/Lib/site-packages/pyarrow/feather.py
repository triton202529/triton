...
class FeatherDataset:
    ...
    def validate_schemas(self, piece, table):
        if not self.schema.equals(table.schema):
            raise ValueError(
                f"Schema in {piece} was different. \n"
                f"{self.schema}\n\nvs\n\n{table.schema}"
            )
...
def check_chunked_overflow(name, col):
    ...
    if col.type in (ext.binary(), ext.string()):
        raise ValueError(
            f"Column '{name}' exceeds 2GB maximum capacity of "
...
def write_feather(
    df, dest, compression=None, compression_level=None, chunksize=None, version=2
):
    """
    ...
    if version == 1:
        ...
        if compression is None and Codec.is_available("lz4_frame"):
            ...
            raise ValueError(
                f'compression="{compression}" not supported, must be '
...
def read_feather(source, columns=None, use_threads=True, memory_map=False, **kwargs):
    """
    ...
    **kwargs
    ...
    """
    return read_table(
        source, columns=columns, memory_map=memory_map, use_threads=use_threads
    ).to_pandas(use_threads=use_threads, **kwargs)
...
def read_table(source, columns=None, memory_map=False, use_threads=True):
    ...
    if not isinstance(columns, Sequence):
        raise TypeError(
            "Columns must be a sequence but, got {}".format(type(columns).__name__)
        )
