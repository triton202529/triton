...
def _get_legacy_format_default(options):
    ...
    return IpcWriteOptions(
        use_legacy_format=use_legacy_format, metadata_version=metadata_version
    )
...
def new_stream(sink, schema, *, options=None):
    return RecordBatchStreamWriter(sink, schema, options=options)
...
def open_stream(source, *, options=None, memory_pool=None):
    ...
    """
    return RecordBatchStreamReader(source, options=options, memory_pool=memory_pool)
