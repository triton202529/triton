    ...
    def list_flights(self, context, criteria):
        if criteria == self.CRITERIA:
            yield flight.FlightInfo(
                pa.schema([]), flight.FlightDescriptor.for_path("/foo"), []
            )
...
class MetadataFlightServer(FlightServerBase):
    ...
    def do_get(self, context, ticket):
        ...
        return flight.GeneratorStream(
            table.schema, self.number_batches(table), options=self.options
        )
    ...
    def do_put(self, context, descriptor, reader, writer):
        ...
        for batch, buf in reader:
            assert batch.equals(
                pa.RecordBatch.from_arrays([pa.array([expected_data[counter]])], ["a"])
            )
...
class EchoStreamFlightServer(EchoFlightServer):
    ...
    def do_get(self, context, ticket):
        return flight.GeneratorStream(
            self.last_message.schema, self.last_message.to_batches(max_chunksize=1024)
        )
...
class SlowFlightServer(FlightServerBase):
    ...
    def do_get(self, context, ticket):
        return flight.GeneratorStream(
            pa.schema([("a", pa.int32())]), self.slow_stream()
        )
    ...
    def received_headers(self, headers):
        ...
        if auth_header:
            self.factory.set_call_credential(
                [b"authorization", auth_header[0].encode("utf-8")]
            )
...
def test_repr():
    ...
    assert (
        repr(flight.SchemaResult(pa.schema([("int", "int64")])))
    ...
    info = flight.FlightInfo(
        None, flight.FlightDescriptor.for_path(), [], 1, 42, True, b"test app metadata"
    )
    info_repr = (
        "<pyarrow.flight.FlightInfo "
        "schema=None "
...
def test_eq():
    items = [
        ...
        lambda: (flight.ActionType("foo", "bar"), flight.ActionType("foo", "baz")),
        lambda: (flight.BasicAuth("user", "pass"), flight.BasicAuth("user2", "pass")),
        lambda: (flight.BasicAuth("user", "pass"), flight.BasicAuth("user", "pass2")),
        lambda: (
            flight.FlightDescriptor.for_command("foo"),
            flight.FlightDescriptor.for_path("foo"),
        ),
        lambda: (flight.FlightEndpoint(b"foo", []), flight.FlightEndpoint(b"bar", [])),
        lambda: (
            flight.FlightEndpoint(
        ...
        lambda: (
            flight.FlightEndpoint(b"foo", [], app_metadata=b""),
            flight.FlightEndpoint(b"foo", [], app_metadata=b"meta"),
        ),
        lambda: (
            flight.FlightInfo(pa.schema([]), flight.FlightDescriptor.for_path(), []),
            flight.FlightInfo(
        ...
        lambda: (
            flight.FlightInfo(pa.schema([]), flight.FlightDescriptor.for_path(), []),
            flight.FlightInfo(
                pa.schema([]), flight.FlightDescriptor.for_command(b"foo"), []
            ),
        ),
        lambda: (
            flight.FlightInfo(
        ...
        lambda: (
            flight.FlightInfo(
                pa.schema([]), flight.FlightDescriptor.for_path(), [], total_records=-1
            ),
            flight.FlightInfo(
                pa.schema([]), flight.FlightDescriptor.for_path(), [], total_records=1
            ),
        ...
        lambda: (
            flight.FlightInfo(
                pa.schema([]), flight.FlightDescriptor.for_path(), [], total_bytes=-1
            ),
            flight.FlightInfo(
                pa.schema([]), flight.FlightDescriptor.for_path(), [], total_bytes=42
            ),
        ),
        lambda: (
            flight.FlightInfo(
                pa.schema([]), flight.FlightDescriptor.for_path(), [], ordered=False
            ),
            flight.FlightInfo(
                pa.schema([]), flight.FlightDescriptor.for_path(), [], ordered=True
            ),
        ),
        lambda: (
            flight.FlightInfo(
                pa.schema([]), flight.FlightDescriptor.for_path(), [], app_metadata=b""
            ),
            flight.FlightInfo(
                pa.schema([]),
                flight.FlightDescriptor.for_path(),
...
def test_flight_list_flights():
    ...
    with ConstantFlightServer() as server, flight.connect(
...
def test_flight_do_get_ints():
    ...
    with ConstantFlightServer(options=options) as server, flight.connect(
        ("localhost", server.port)
    ) as client:
        data = client.do_get(flight.Ticket(b"ints")).read_all()
        assert data.equals(table)
    ...
    with pytest.raises(
        flight.FlightServerError, match="expected IpcWriteOptions, got <class 'int'>"
    ):
        with ConstantFlightServer(options=42) as server, flight.connect(
            ("localhost", server.port)
        ) as client:
            data = client.do_get(flight.Ticket(b"ints")).read_all()
...
def test_flight_do_get_ticket():
    ...
    data1 = [pa.array([-10, -5, 0, 5, 10], type=pa.int32())]
    table = pa.Table.from_arrays(data1, names=["a"])
    with CheckTicketFlightServer(
        expected_ticket=b"the-ticket"
    ) as server, flight.connect(("localhost", server.port)) as client:
...
def test_flight_get_info():
    ...
    with GetInfoFlightServer() as server:
        ...
        assert info.endpoints[1].expiration_time == pa.scalar(
            "2023-04-05T12:34:56.789012345+00:00"
        ).cast(pa.timestamp("ns", "UTC"))
        assert info.endpoints[1].app_metadata == b"endpoint app metadata"
        assert info.endpoints[1].locations[0] == flight.Location.for_grpc_tcp(
            "localhost", 5005
        )
...
def test_flight_get_schema():
    ...
    with GetInfoFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
...
def test_list_actions():
    ...
    # ARROW-6392
    with ListActionsErrorFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
        with pytest.raises(
    ...
    with ListActionsFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
...
def test_do_action_result_convenience():
    with ConvenienceServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
...
def test_nicer_server_exceptions():
    with ConvenienceServer() as server, FlightClient(
        ...
        with pytest.raises(
            flight.FlightServerError, match="a bytes-like object is required"
        ):
...
def test_flight_large_message():
    ...
    with EchoFlightServer(expected_schema=data.schema) as server, FlightClient(
        ("localhost", server.port)
    ) as client:
...
def test_flight_generator_stream():
    ...
    with EchoStreamFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
        writer, _ = client.do_put(flight.FlightDescriptor.for_path("test"), data.schema)
        writer.write_table(data)
...
def test_flight_invalid_generator_stream():
    ...
    with InvalidStreamFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
...
def test_timeout_fires():
    ...
    with SlowFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
...
def test_timeout_passes():
    ...
    with ConstantFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
...
def test_read_options():
    ...
    with ConstantFlightServer() as server, FlightClient(
        ...
        response1 = client.do_get(flight.Ticket(b"multi"), options=options).read_all()
...
token_auth_handler = TokenServerAuthHandler(
    creds={
        b"test": b"p4ssw0rd",
    }
)
...
def test_http_basic_unauth():
    ...
    with EchoStreamFlightServer(
        auth_handler=basic_auth_handler
    ) as server, FlightClient(("localhost", server.port)) as client:
        action = flight.Action("who-am-i", b"")
        with pytest.raises(
            flight.FlightUnauthenticatedError, match=".*unauthenticated.*"
        ):
            list(client.do_action(action))


@pytest.mark.skipif(
    os.name == "nt", reason="ARROW-10013: gRPC on Windows corrupts peer()"
)
...
def test_http_basic_auth_invalid_password():
    ...
    with EchoStreamFlightServer(
        ...
        action = flight.Action("who-am-i", b"")
        with pytest.raises(
            flight.FlightUnauthenticatedError, match=".*wrong password.*"
        ):
...
def test_token_auth():
    ...
    with EchoStreamFlightServer(
        auth_handler=token_auth_handler
    ) as server, FlightClient(("localhost", server.port)) as client:
        action = flight.Action("who-am-i", b"")
...
def test_authenticate_basic_token_and_action():
    ...
    with HeaderAuthFlightServer(
        ...
        result = list(
            client.do_action(action=flight.Action("test-action", b""), options=options)
        )
...
def test_authenticate_basic_token_with_client_middleware():
    ...
    with HeaderAuthFlightServer(
        ...
        client = FlightClient(
            ("localhost", server.port), middleware=[client_auth_middleware]
        )
        encoded_credentials = base64.b64encode(b"test:password")
        options = flight.FlightCallOptions(
            headers=[(b"authorization", b"Basic " + encoded_credentials)]
        )
        result = list(
            client.do_action(action=flight.Action("test-action", b""), options=options)
        )
...
def test_arbitrary_headers_in_flight_call_options():
    ...
    with ArbitraryHeadersFlightServer(
        ...
        result = list(
            client.do_action(flight.Action("test-action", b""), options=options)
        )
...
def test_location_unknown_scheme():
    ...
    assert (
        flight.Location("https://example.com/bar.parquet").uri
...
def test_tls_fails():
    ...
    with ConstantFlightServer(
        tls_certificates=certs["certificates"]
    ) as s, FlightClient("grpc+tls://localhost:" + str(s.port)) as client:
        # gRPC error messages change based on version, so don't look
        # for a particular error
...
def test_tls_do_get():
    ...
    with ConstantFlightServer(
        ...
        ("localhost", s.port), tls_root_certs=certs["root_cert"]
    ) as client:
        data = client.do_get(flight.Ticket(b"ints")).read_all()
        assert data.equals(table)
...
def test_tls_disable_server_verification():
    ...
    with ConstantFlightServer(tls_certificates=certs["certificates"]) as s:
        try:
            client = FlightClient(
                ("localhost", s.port), disable_server_verification=True
            )
...
def test_flight_do_get_metadata():
    ...
    batches = []
    with MetadataFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
...
def test_flight_do_get_metadata_v4():
    ...
    options = pa.ipc.IpcWriteOptions(metadata_version=pa.ipc.MetadataVersion.V4)
    with MetadataFlightServer(options=options) as server, FlightClient(
        ("localhost", server.port)
    ) as client:
        reader = client.do_get(flight.Ticket(b""))
        data = reader.read_all()
...
def test_flight_do_put_limit():
    ...
    with EchoFlightServer() as server, FlightClient(
        ("localhost", server.port), write_size_limit_bytes=4096
    ) as client:
        writer, metadata_reader = client.do_put(
            flight.FlightDescriptor.for_path(""), large_batch.schema
        )
        with writer:
            with pytest.raises(
                flight.FlightWriteSizeExceededError, match="exceeded soft limit"
            ) as excinfo:
...
def test_roundtrip_errors():
    ...
    with ErrorFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
        ...
        for command, exception in exceptions.items():
            ...
            with pytest.raises(exception, match=".*foo.*"):
                writer, reader = client.do_put(
                    flight.FlightDescriptor.for_command(command), table.schema
                )
...
def test_middleware_mapping():
    ...
    with FlightServerBase(
        middleware={"test": server_middleware}
    ) as server, FlightClient(
        ("localhost", server.port), middleware=[client_middleware]
    ) as client:
...
def test_doexchange_get():
    """Emulate DoGet with DoExchange."""
    expected = pa.Table.from_arrays([pa.array(range(0, 10 * 1024))], names=["a"])

    with ExchangeFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
...
def test_doexchange_echo_v4():
    ...
    options = pa.ipc.IpcWriteOptions(metadata_version=pa.ipc.MetadataVersion.V4)
    with ExchangeFlightServer(options=options) as server, FlightClient(
        ("localhost", server.port)
    ) as client:
        descriptor = flight.FlightDescriptor.for_command(b"echo")
        writer, reader = client.do_exchange(descriptor)
...
def test_interrupt():
    ...
    def test(read_all):
        ...
        e = exc_info.value
        assert isinstance(e, (pa.ArrowCancelled, KeyboardInterrupt)) or isinstance(
            e.__context__, (pa.ArrowCancelled, KeyboardInterrupt)
        )

    with CancelFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
...
def test_never_sends_data():
    ...
    with NeverSendsDataFlightServer() as server, flight.connect(
        ("localhost", server.port)
    ) as client:
...
def test_large_descriptor():
    ...
    large_descriptor = flight.FlightDescriptor.for_command(b" " * (2**31 + 1))
    with FlightServerBase() as server, flight.connect(
        ("localhost", server.port)
    ) as client:
        with pytest.raises(OSError, match="Failed to serialize Flight descriptor"):
            writer, _ = client.do_put(large_descriptor, pa.schema([]))
            writer.close()
        with pytest.raises(
            pa.ArrowException, match="Failed to serialize Flight descriptor"
        ):
...
def test_large_metadata_client():
    ...
    descriptor = flight.FlightDescriptor.for_command(b"")
    metadata = b" " * (2**31 + 1)
    with EchoFlightServer() as server, flight.connect(
        ("localhost", server.port)
    ) as client:
    ...
    del metadata
    with LargeMetadataFlightServer() as server, flight.connect(
        ("localhost", server.port)
    ) as client:
        with pytest.raises(
            flight.FlightServerError, match="app_metadata size overflow"
        ):
            reader = client.do_get(flight.Ticket(b""))
            reader.read_all()
...
def test_none_action_side_effect():
    ...
    with ActionNoneFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
    ...
    with TracingFlightServer(
        ...
        options = flight.FlightCallOptions(
            headers=[
                ...
                (
                    b"traceparent",
                    b"00-000ff00f00f0ff000f0f00ff0f00fff0-" b"000f0000f0f00000-00",
                ),
                (b"tracestate", b""),
            ]
        )
...
def test_do_put_does_not_crash_when_schema_is_none():
    ...
    msg = (
        "Argument 'schema' has incorrect type "
...
def test_headers_trailers():
    ...
    with HeadersTrailersFlightServer() as server, FlightClient(
        ("localhost", server.port), middleware=[factory]
    ) as client:
