...
def test_env_var_io_thread_count():
    ...
    for v in ("-1", "z"):
        ...
        assert (
            "ARROW_IO_THREADS does not contain a valid number of threads" in err.strip()
        )
...
@pytest.mark.skipif(
    sys.platform == "win32",
    reason="Path to timezone database is not configurable " "on non-Windows platforms",
)
def test_set_timezone_db_path_non_windows():
    # set_timezone_db_path raises an error on non-Windows platforms
    with pytest.raises(
        ArrowInvalid,
        match="Arrow was set to use OS timezone " "database at compile time",
    ):
