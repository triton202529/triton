...
sapien. Quisque pretium vestibulum urna eu vehicula."""
    ...
    my_schema = pa.schema(
        [
            ...
            pa.field("bar", "string", True, metadata={"key3": "value3"}),
        ],
...
lorem: '{lorem[:65]}' + {len(lorem) - 65}"""
    ...
    result = pa.schema(
        [("f0", "int32")], metadata={"key": "value" + "x" * 62}
    ).to_string()
    assert (
        result
        == """\
...
xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'"""
    ...
    assert (
        my_schema.to_string(truncate_metadata=False)
        == f"""\
foo: int32 not null
  -- field metadata --
  key1: 'value1'
...
lorem: '{lorem[:92]} (... {len(lorem) - 91} chars omitted)"""
    ...
    assert (
        my_schema.to_string(
            truncate_metadata=False, show_field_metadata=False, element_size_limit=50
        )
        == f"""\
foo: int32 not null
bar: string
...
lorem: '{lorem[:50 - 8]} (... {len(lorem) - (50 - 9)} chars omitted)"""
    ...
    assert (
        my_schema.to_string(truncate_metadata=False, show_schema_metadata=False)
        == """\
foo: int32 not null
  -- field metadata --
  key1: 'value1'
...
def test_schema_equals_propagates_check_metadata():
    # ARROW-4088
    schema1 = pa.schema([pa.field("foo", pa.int32()), pa.field("bar", pa.string())])
    schema2 = pa.schema(
        [
            pa.field("foo", pa.int32()),
...
def test_type_schema_pickling(pickle_module):
    cases = [
        ...
        pa.map_(
            pa.field("key", pa.string(), nullable=False), pa.field("value", pa.int8())
        ),
    ]
...
def test_undecodable_metadata():
    ...
    schema = pa.schema(
        [pa.field("ints", pa.int16(), metadata={"key": data1})], metadata={"key": data2}
    )
