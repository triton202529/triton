...
def test_pandas_roundtrip_string_with_missing():
    ...
    arr = ["a", "", "c", None]
    table = pa.table(
        {"a": pa.array(arr), "a_large": pa.array(arr, type=pa.large_string())}
    )
...
@pytest.mark.pandas
@pytest.mark.parametrize("np_float_str", ["float32", "float64"])
...
def test_pyarrow_roundtrip(uint, int, float, np_float_str, unit, tz, offset, length):
    ...
    table = pa.table(
        {
            ...
            "b": pa.array(arr, type=int),
            "c": pa.array(
                np.array(arr, dtype=np.dtype(np_float_str)),
                type=float,
                from_pandas=True,
            ),
            "d": [True, False, True],
