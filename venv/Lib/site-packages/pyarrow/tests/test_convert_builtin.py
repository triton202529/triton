...
parametrize_with_iterable_types = pytest.mark.parametrize("seq", ITERABLE_TYPES)

parametrize_with_sequence_types = pytest.mark.parametrize("seq", SEQUENCE_TYPES)

parametrize_with_collections_types = pytest.mark.parametrize("seq", COLLECTIONS_TYPES)
...
@pytest.mark.parametrize(
    "factory", [pa.list_, pa.large_list, pa.list_view, pa.large_list_view]
)
...
@parametrize_with_sequence_types
@pytest.mark.parametrize("inner_seq", SEQUENCE_TYPES)
@pytest.mark.parametrize(
    "factory", [pa.list_, pa.large_list, pa.list_view, pa.large_list_view]
)
def test_nested_arrays(seq, inner_seq, factory):
    arr = pa.array(
        seq([inner_seq([]), inner_seq([1, 2]), None]), type=factory(pa.int64())
    )
    assert len(arr) == 3
    assert arr.null_count == 1
...
def test_sequence_integer(seq, np_scalar_pa_type):
    ...
    expected = [1, None, 3, None, np.iinfo(np_scalar).min, np.iinfo(np_scalar).max]
...
def test_sequence_numpy_integer_inferred(seq, np_scalar_pa_type):
    ...
    expected += [np_scalar(np.iinfo(np_scalar).min), np_scalar(np.iinfo(np_scalar).max)]
    arr = pa.array(seq(expected))
    assert len(arr) == 6
    assert arr.null_count == 2
    assert arr.type == pa_type
...
@pytest.mark.numpy
@pytest.mark.xfail(
    reason="Type inference for uint64 not implemented", raises=OverflowError
)
    ...
    for inner_seq in (np.array, list):
        ...
        if from_pandas:
            ...
            np.testing.assert_equal(
                arr.to_pylist(), [[1.0, 2.0], [1.0, 2.0, 3.0], [np.nan], None]
            )
...
@pytest.mark.numpy
@pytest.mark.xfail(
    reason=("Type inference for multidimensional ndarray " "not yet implemented"),
...
@pytest.mark.parametrize(
    "input", [(pa.date32(), [10957, None]), (pa.date64(), [10957 * 86400000, None])]
)
...
def test_sequence_timestamp_with_timezone(timezone, unit):
    ...
    data = [
        ...
        pytz.utc.localize(datetime.datetime(2008, 1, 5, 5, 0, 0, 1000)),
        None,
...
def test_sequence_timestamp_with_zoneinfo_timezone_inference():
    ...
    data = [
        ...
        None,
        datetime.datetime(
            2006, 1, 13, 12, 34, 56, 432539, tzinfo=zoneinfo.ZoneInfo(key="US/Eastern")
        ),
        datetime.datetime(
            2010, 8, 13, 5, 0, 0, 437699, tzinfo=zoneinfo.ZoneInfo(key="Europe/Moscow")
        ),
    ]
    expected = [
        pa.timestamp("us", tz=None),
...
def test_sequence_timestamp_from_mixed_builtin_and_pandas_datetimes():
    ...
    data = [
        pd.Timestamp(1184307814123456123, tz=pytz.timezone("US/Eastern"), unit="ns"),
        datetime.datetime(2007, 7, 13, 8, 23, 34, 123456),  # naive
        pytz.utc.localize(datetime.datetime(2008, 1, 5, 5, 0, 0, 1000)),
        None,
    ]
...
@pytest.mark.xfail(
    not _pandas_api.have_pandas, reason="pandas required for nanosecond conversion"
)
...
def test_sequence_duration_nested_lists_numpy():
    ...
    data = [[np.timedelta64(td1), None], [np.timedelta64(td1), np.timedelta64(td2)]]
    ...
    data = [
        np.array([np.timedelta64(td1), None], dtype="timedelta64[us]"),
...
def test_sequence_decimal_infer_mixed():
    ...
    cases = [
        ([decimal.Decimal("1.234"), decimal.Decimal("3.456")], pa.decimal128(4, 3)),
        ([decimal.Decimal("1.234"), decimal.Decimal("456.7")], pa.decimal128(6, 3)),
        ([decimal.Decimal("123.4"), decimal.Decimal("4.567")], pa.decimal128(6, 3)),
        ([decimal.Decimal("123e2"), decimal.Decimal("4567e3")], pa.decimal128(7, 0)),
        ([decimal.Decimal("123e4"), decimal.Decimal("4567e2")], pa.decimal128(7, 0)),
        ([decimal.Decimal("0.123"), decimal.Decimal("0.04567")], pa.decimal128(5, 5)),
        ([decimal.Decimal("0.001"), decimal.Decimal("1.01E5")], pa.decimal128(9, 3)),
    ]
...
def test_struct_from_list_of_pairs_errors():
    ...
    cases = [
        ...
        tuple(
            "unknown-key",
        ),  # not known field name
        "string",  # not a tuple
    ]
    for key_value_pair in cases:
        msg = re.escape(
            template.format(repr(key_value_pair), type(key_value_pair).__name__)
        )
    ...
    expected_type = pa.struct(
        [
            pa.field(
                "a",
                pa.struct(
                    [pa.field("aa", pa.list_(pa.int64())), pa.field("ab", pa.bool_())]
                ),
            ),
            pa.field("b", pa.string()),
        ]
...
def test_structarray_from_arrays_coerce():
    ...
    result = pa.StructArray.from_arrays(
        arrays, ["ints", "strs", "bools", "int_nonnull"]
    )
...
def test_map_from_dicts():
    data = [
        [{"key": b"a", "value": 1}, {"key": b"b", "value": 2}],
        [{"key": b"c", "value": 3}],
        [
            {"key": b"d", "value": 4},
            {"key": b"e", "value": 5},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [1, None, None],
            [pa.scalar(1), None, pa.scalar(None, pa.int64())],
            pa.int64(),
        ),
        ([None, None], [pa.scalar(None), pa.scalar(None)], pa.null()),
        ([1.0, 2.0, None], [pa.scalar(1.0), pa.scalar(2.0), None], pa.float64()),
        (
        ...
        (
            [[1, 2, 3]],
            [pa.scalar([1, 2, 3], type=pa.list_view(pa.int64()))],
            pa.list_view(pa.int64()),
        ),
        (
            [["a", "b"]],
            [pa.scalar(["a", "b"], type=pa.list_view(pa.string()))],
        ...
        (
            ...
            [
                pa.scalar(
                    [("a", 1), ("b", 2)],
                    type=pa.struct([("a", pa.int8()), ("b", pa.int8())]),
                )
            ],
        ...
        (
            ...
            [
                pa.scalar(
                    [("a", 1), ("b", "bar")],
                    type=pa.struct([("a", pa.int8()), ("b", pa.string())]),
                )
            ],
...
def test_array_accepts_pyarrow_scalar_errors(seq):
    ...
    with pytest.raises(
        pa.ArrowInvalid, match="cannot mix scalars with different types"
    ):
        pa.array(sequence)
    ...
    with pytest.raises(
        pa.ArrowInvalid,
        match="Cannot append scalar of type string " "to builder for type int32",
    ):
        pa.array([pa.scalar("a")], type=pa.int32())

    with pytest.raises(
        pa.ArrowInvalid,
        match="Cannot append scalar of type int64 " "to builder for type null",
    ):
