...
exported_functions = [
    func
    for (name, func) in sorted(pc.__dict__.items())
    if hasattr(func, "__arrow_compute_function__")
]

exported_option_classes = [
    cls
    for (name, cls) in sorted(pc.__dict__.items())
    if (
        isinstance(cls, type)
        and cls is not pc.FunctionOptions
...
def test_option_class_equality(request):
    options = [
        ...
        pc.RandomOptions(),
        pc.RankOptions(
            sort_keys="ascending", null_placement="at_start", tiebreaker="max"
        ),
        pc.RankQuantileOptions(sort_keys="ascending", null_placement="at_start"),
...
def test_call_function_with_memory_pool():
    ...
    result2 = pc.call_function(
        "take", [arr, indices], memory_pool=pa.default_memory_pool()
    )
...
def test_generated_signatures():
    ...
    assert str(sig) == (
        "(array, /, *, skip_nulls=True, min_count=1, " "options=None, memory_pool=None)"
    )
    # With positional options
    sig = inspect.signature(pc.quantile)
    assert str(sig) == (
        "(array, /, q=0.5, *, interpolation='linear', "
    ...
    assert str(sig) == (
        "(*strings, null_handling='emit_null', "
...
def find_new_unicode_codepoints():
    new = set()
    characters = [chr(c) for c in range(0x80, 0x11000) if not (0xD800 <= c < 0xE000)]
...
codepoints_ignore = {
    "is_alnum": numeric_info_missing | digit_info_missing | unknown_issue_is_alpha,
...
def test_utf8_zfill():
    ...
    arr = pa.array(["1", "-2", "+3"])
    result = pc.utf8_zero_fill(
        arr, options=pc.ZeroFillOptions(width=5, padding="💠")
    ).to_pylist()
    assert result == ["💠💠💠💠1", "-💠💠💠2", "+💠💠💠3"]
    ...
    arr = pa.array(["1", "-2", "+3"])
    result = pc.utf8_zero_fill(
        arr, options=pc.ZeroFillOptions(width=4, padding="x")
    ).to_pylist()
...
def test_replace_plain():
    ...
    ar = pc.replace_substring(
        data, pattern="foo", replacement="bar", max_replacements=1
    )
...
def test_binary_join_element_wise():
    ...
    assert (
        pc.binary_join_element_wise("a", null, "-", options=replace).as_py() == "a-spam"
    )
...
def test_drop_null_record_batch():
    ...
    batch = pa.record_batch(
        [pa.array(["a", None, "c", "d", None]), pa.array([None, None, "c", None, "e"])],
        names=["a'", "b'"],
    )
...
def test_drop_null_table():
    ...
    expected = pa.table(
        [pa.array(["a", "d"]), pa.array(["a", "d"]), pa.array(["a", "d"])],
        names=["a", "b", "c"],
    )
    result = table.drop_null()
    assert result.equals(expected)
...
def test_round():
    ...
    for ndigits, expected in ndigits_and_expected.items():
        ...
        assert pc.round(values, ndigits, round_mode="half_towards_infinity") == result
...
def test_fill_null():
    ...
    with pytest.raises(
        pa.ArrowInvalid, match="Array arguments must all be the same length"
    ):
...
def test_cast():
    ...
    assert pc.cast(arr, options=options) == pa.array([1, 2, 3, 4], type="int8")
    ...
    assert pc.cast(arr, options=allow_overflow_options) == pa.array([-1], type="int32")
    ...
    expected = pa.array([["1", "2"], ["3", "4", "5"]], type=pa.list_(pa.utf8()))
...
DecimalTypeTraits = namedtuple(
    "DecimalTypeTraits", ("name", "factory", "max_precision")
)

FloatToDecimalCase = namedtuple(
    "FloatToDecimalCase", ("precision", "scale", "float_val")
)

decimal_type_traits = [
    DecimalTypeTraits("decimal32", pa.decimal32, 9),
...
def check_cast_float_to_decimal(
    float_ty, float_val, decimal_ty, decimal_ctx, max_precision
):
...
def test_cast_float_to_decimal_random(float_ty, decimal_traits):
    ...
    with decimal.localcontext() as ctx:
        ...
        min_scale = max(
            -max_precision, precision + math.ceil(math.log10(2**float_exp_min))
        )
        ...
        for scale in range(min_scale, max_scale):
            ...
            float_exp = -mantissa_bits + math.floor(
                math.log2(10 ** (precision - scale))
            )
...
def test_strptime():
    ...
    expected = pa.array(
        [datetime.datetime(2020, 1, 5), None, None], type=pa.timestamp("s")
    )
    got = pc.strptime(arr, format="%d/%m/%Y", unit="s", error_is_null=True)
    assert got == expected

    with pytest.raises(pa.ArrowInvalid, match="Failed to parse string: '5/1/2020'"):
...
def test_strftime():
    ...
    assert result.equals(expected)
    with pytest.raises(
        pa.ArrowInvalid, match="Timezone not present, cannot convert to string"
    ):
...
def test_assume_timezone():
    ...
    with pytest.raises(
        ValueError, match="Timestamp doesn't exist in " f"timezone '{timezone}'"
    ):
    ...
    with pytest.raises(
        ValueError, match="Timestamp is ambiguous in " f"timezone '{timezone}'"
    ):
...
def test_count():
    ...
    with pytest.raises(ValueError, match='"something else" is not a valid count mode'):
...
def test_partition_nth():
    ...
    with pytest.raises(
        ValueError, match="'partition_nth_indices' cannot be called without options"
    ):
...
def test_partition_nth_null_placement():
    ...
    for pivot in (0, 7, 13, 19):
        for null_placement in ("at_start", "at_end"):
            indices = pc.partition_nth_indices(
                data, pivot=pivot, null_placement=null_placement
            )
...
def test_select_k_array():
    ...
    result = pc.select_k_unstable(
        arr, options=pc.SelectKOptions(k=2, sort_keys=[("dummy", "descending")])
    )
    validate_select_k(result, arr, "descending")
...
def test_select_k_table():
    ...
    with pytest.raises(
        ValueError, match="'select_k_unstable' cannot be called without options"
    ):
        pc.select_k_unstable(table)

    with pytest.raises(
        ValueError, match="select_k_unstable requires a nonnegative `k`"
    ):
        pc.select_k_unstable(table, k=-1, sort_keys=[("a", "ascending")])

    with pytest.raises(
        ValueError, match="select_k_unstable requires a " "non-empty `sort_keys`"
    ):
    ...
    with pytest.raises(
        ValueError, match="Invalid sort key column: No match for.*unknown"
    ):
...
def test_sort_indices_array():
    ...
    result = pc.sort_indices(
        arr, sort_keys=[("dummy", "descending")], null_placement="at_start"
    )
    assert result.to_pylist() == [2, 1, 0, 3]
    ...
    result = pc.sort_indices(
        ...
        options=pc.SortOptions(
            sort_keys=[("dummy", "descending")], null_placement="at_start"
        ),
    )
    assert result.to_pylist() == [2, 1, 0, 3]
...
def test_sort_indices_table():
    ...
    assert result.to_pylist() == [3, 0, 1, 2]
    result = pc.sort_indices(
        table, sort_keys=[(pc.field("a"), "ascending")], null_placement="at_start"
    )
    ...
    result = pc.sort_indices(
        table,
        sort_keys=[("a", "descending"), ("b", "ascending")],
        null_placement="at_start",
    )
    assert result.to_pylist() == [2, 1, 0, 3]
    # Positional `sort_keys`
    result = pc.sort_indices(
        table, [("a", "descending"), ("b", "ascending")], null_placement="at_start"
    )
    assert result.to_pylist() == [2, 1, 0, 3]
...
def test_index_in():
    ...
    result = pc.index_in(arr, value_set=pa.array([1, 3, None]), skip_nulls=True)
...
def test_cumulative_sum(start, skip_nulls):
    ...
    starts = [
        None,
        start_int,
        pa.scalar(start_int, type=pa.int8()),
    ...
    for strt in starts:
        ...
        expected_arrays = [
            ...
            (
                ...
                if skip_nulls
                else pa.array([0, None, None, None])
            ),
            (
                pa.chunked_array([[0, None, 20, 50]])
                if skip_nulls
                else pa.chunked_array([[0, None, None, None]])
            ),
        ]
    ...
    starts = [
        None,
        start,
        pa.scalar(start, type=pa.float32()),
    ...
    for strt in starts:
        ...
        expected_arrays = [
            ...
            (
                ...
                if skip_nulls
                else np.array([1, np.nan, None, None, None, None])
            ),
        ]
        for i, arr in enumerate(arrays):
            result = pc.cumulative_sum(arr, start=strt, skip_nulls=skip_nulls)
    ...
    for strt in starts:
        ...
        expected_arrays = [
            ...
            (
                ...
                if skip_nulls
                else pa.array([1, None, None, None])
            ),
            (
                pa.chunked_array([[1, None, 20, 100]])
                if skip_nulls
                else pa.chunked_array([[1, None, None, None]])
            ),
        ]
        for i, arr in enumerate(arrays):
    ...
    for strt in starts:
        ...
        expected_arrays = [
            ...
            (
                ...
                if skip_nulls
                else pa.array([2, 2, None, None, None, None])
            ),
            (
                pa.chunked_array([[2, 2, None, 5, 5, None]])
                if skip_nulls
                else pa.chunked_array([[2, 2, None, None, None, None]])
            ),
        ]
        for i, arr in enumerate(arrays):
            ...
            expected = pc.max_element_wise(
                expected_arrays[i],
                strt if strt is not None else int(-1e9),
                skip_nulls=False,
            )
            assert result.equals(expected)
    ...
    for strt in starts:
        ...
        expected_arrays = [
            ...
            (
                ...
                if skip_nulls
                else np.array([2.5, 2.5, None, None, None, None])
            ),
        ]
        for i, arr in enumerate(arrays):
            result = pc.cumulative_max(arr, start=strt, skip_nulls=skip_nulls)
    ...
    for strt in starts:
        ...
        expected_arrays = [
            ...
            (
                ...
                if skip_nulls
                else pa.array([5, 5, None, None, None, None])
            ),
            (
                pa.chunked_array([[5, 5, None, 2, 2, None]])
                if skip_nulls
                else pa.chunked_array([[5, 5, None, None, None, None]])
            ),
        ]
        for i, arr in enumerate(arrays):
    ...
    for strt in starts:
        ...
        expected_arrays = [
            ...
            (
                ...
                if skip_nulls
                else np.array([5.5, 5.5, None, None, None, None])
            ),
        ]
        for i, arr in enumerate(arrays):
            result = pc.cumulative_min(arr, start=strt, skip_nulls=skip_nulls)
...
def test_make_struct():
    ...
    assert pc.make_struct([1, 2, 3], "a b c".split()) == pa.StructArray.from_arrays(
        [[1, 2, 3], "a b c".split()], names="0 1".split()
    )
...
def test_map_lookup():
    ...
    arr = pa.array(
        [[("one", 1), ("two", 2)], [("none", 3)], [], [("one", 5), ("one", 7)], None],
        type=ty,
    )
...
def test_case_when():
    assert pc.case_when(
        pc.make_struct([True, False, None], [False, True, None]),
        [1, 2, 3],
        [11, 12, 13],
    ) == pa.array([1, 12, None])
...
def test_utf8_normalize():
    ...
    with pytest.raises(
        ValueError, match='"NFZ" is not a valid Unicode normalization form'
    ):
...
def test_random():
    ...
    for initializer in ["system", 42, -42, b"abcdef"]:
        assert pc.random(0, initializer=initializer) == pa.array([], type=pa.float64())
    ...
    arrays = [
        tuple(pc.random(100, initializer=i % 7).to_pylist()) for i in range(0, 100)
    ]
...
def test_rank_options_tiebreaker(tiebreaker, expected_values):
    ...
    rank_options = pc.RankOptions(
        sort_keys="ascending", null_placement="at_end", tiebreaker=tiebreaker
    )
...
def test_rank_options():
    ...
    with pytest.raises(ValueError, match=r'"NonExisting" is not a valid tiebreaker'):
        pc.RankOptions(
            sort_keys="descending", null_placement="at_end", tiebreaker="NonExisting"
        )
...
def test_rank_quantile_options():
    ...
    # Ensure sort_keys tuple usage
    result = pc.rank_quantile(
        arr, options=pc.RankQuantileOptions(sort_keys=[("b", "ascending")])
    )
    assert result.equals(expected)
...
def test_rank_normal_options():
    ...
    assert result.to_pylist() == expected
    result = pc.rank_normal(
        arr,
        options=pc.RankQuantileOptions(
            null_placement="at_start", sort_keys="descending"
        ),
    )
    assert result.to_pylist() == expected
...
def create_sample_expressions():
    ...
    schema = pa.schema(
        [
            ...
            pa.field("foo", pa.struct([pa.field("bar", pa.string())])),
        ]
    )
...
def test_list_slice_output_fixed(start, stop, step, expected, value_type, list_type):
    if list_type == "fixed":
        arr = pa.array(
            [[1, 2, 3], [4, 5, None], [6, None, None], None], pa.list_(pa.int8(), 3)
        ).cast(pa.list_(value_type(), 3))
    else:
        arr = pa.array([[1, 2, 3], [4, 5], [6], None], pa.list_(pa.int8())).cast(
    ...
    if stop is None and list_type != "fixed":
        msg = (
            "Unable to produce FixedSizeListArray from "
...
def test_pivot_wider():
    ...
    result = pc.pivot_wider(["height", "width", "depth"], [10, None, 11], key_names)
    assert result.as_py() == {"width": None, "height": 10}
    # check key order
    assert list(result.as_py()) == ["width", "height"]

    result = pc.pivot_wider(
        ["height", "width", "depth"], [10, None, 11], key_names=key_names
    )
    assert result.as_py() == {"width": None, "height": 10}
    ...
    with pytest.raises(ValueError, match="Encountered more than one non-null value"):
        result = pc.pivot_wider(
            ["height", "width", "height"], [10, None, 11], key_names=key_names
        )
