...
scipy_type_pairs = [pair for pair in tensor_type_pairs if pair[0] != "f2"]
...
def test_sparse_csf_tensor_numpy_roundtrip(dtype_str, arrow_type):
    ...
    sparse_tensor = pa.SparseCSFTensor.from_numpy(
        data, indptr, indices, shape, axis_order, dim_names
    )
    repr(sparse_tensor)
    result_data, result_indptr, result_indices = sparse_tensor.to_numpy()
    assert sparse_tensor.type == arrow_type
...
def test_dense_to_sparse_tensor(dtype_str, arrow_type, sparse_tensor_type):
    dtype = np.dtype(dtype_str)
    array = np.array([[4, 0, 9, 0], [0, 7, 0, 0], [0, 0, 0, 0], [0, 0, 0, 5]]).astype(
        dtype
    )
    dim_names = ("x", "y")
...
def test_sparse_coo_tensor_scipy_roundtrip(dtype_str, arrow_type, sparse_object):
    ...
    sparse_tensor = pa.SparseCOOTensor.from_scipy(scipy_matrix, dim_names=dim_names)
...
@pytest.mark.parametrize("dtype_str,arrow_type", scipy_type_pairs)
def test_sparse_csr_matrix_scipy_roundtrip(dtype_str, arrow_type, sparse_object):
    shape = (4, 6)
    dim_names = ("x", "y")
    dtype = np.dtype(dtype_str)
...
def test_pydata_sparse_sparse_coo_tensor_roundtrip(dtype_str, arrow_type):
    ...
    sparse_tensor = pa.SparseCOOTensor.from_pydata_sparse(
        sparse_array, dim_names=dim_names
    )
