...
def test_total_bytes_allocated():
    ...
    """
    res = subprocess.run(
        [sys.executable, "-c", code], universal_newlines=True, stderr=subprocess.PIPE
    )
    if res.returncode != 0:
...
def test_binary_total_values_length():
    ...
    large_arr = pa.array(
        [b"0000", None, b"11111", b"222222", b"3333333"], type="large_binary"
    )
...
@pytest.mark.parametrize("ty", [None, pa.null(), pa.int8(), pa.string()])
def test_nulls(ty):
...
def test_array_from_scalar():
    ...
    cases = [
        ...
        (
            ...
            2,
            pa.array(
                [now_utc] * 2, type=pa.timestamp("us", tz=pytz.timezone("US/Eastern"))
            ),
        ),
...
def test_string_view_from_buffers():
    array = pa.array(
        ["String longer than 12 characters", None, "short", "Length is 12"],
        type=pa.string_view(),
    )

    buffers = array.buffers()
    copied = pa.StringViewArray.from_buffers(pa.string_view(), len(array), buffers)
    ...
    with pytest.raises(ValueError, match=match):
        pa.StringViewArray.from_buffers(pa.string_view(), len(array), buffers[0:1])


@pytest.mark.parametrize(
    "list_type_factory", [pa.list_, pa.large_list, pa.list_view, pa.large_list_view]
)
def test_list_from_buffers(list_type_factory):
    ...
    with pytest.raises(ValueError):
        # too many children
        pa.Array.from_buffers(ty, 4, buffers[: ty.num_buffers], children=[child, child])
...
def test_struct_from_arrays():
    ...
    with pytest.raises(ValueError, match="not contain nulls"):
        pa.StructArray.from_arrays(arrays, fields, mask=pa.array([True, False, None]))

    with pytest.raises(TypeError, match="Mask must be"):
        pa.StructArray.from_arrays(arrays, fields, mask=pa.chunked_array([mask]))
...
def test_dictionary_from_buffers(offset):
    ...
    b = pa.DictionaryArray.from_buffers(
        a.type, len(a) - offset, a.indices.buffers(), a.dictionary, offset=offset
    )
    assert a[offset:] == b
...
def test_dictionary_to_numpy():
    ...
    a = pa.DictionaryArray.from_arrays(
        pa.array([0, 1, None, 0]), pa.array(["foo", "bar"])
    )
    ...
    anonulls = pa.DictionaryArray.from_arrays(
        pa.array([0, 1, 1, 0]), pa.array(["foo", "bar"])
    )
    ...
    afloat = pa.DictionaryArray.from_arrays(
        pa.array([0, 1, 1, 0]), pa.array([13.7, 11.0])
    )
    ...
    afloat2 = pa.DictionaryArray.from_arrays(
        pa.array([0, 1, None, 0]), pa.array([13.7, 11.0])
    )
    expected = pa.array([13.7, 11.0, None, 13.7]).to_numpy(zero_copy_only=False)
    np.testing.assert_allclose(
        afloat2.to_numpy(zero_copy_only=False), expected, equal_nan=True
    )
    ...
    np.testing.assert_allclose(
        aints.to_numpy(zero_copy_only=False), expected, equal_nan=True
    )
...
def test_map_from_dict():
    # ARROW-17832
    tup_arr = pa.array(
        [[("a", 1), ("b", 2)], [("c", 3)]], pa.map_(pa.string(), pa.int64())
    )
...
def test_map_from_arrays():
    ...
    result = pa.MapArray.from_arrays(
        offsets, keys, items, pa.map_(keys.type, items.type)
    )
    assert result.equals(expected)
    ...
    result = pa.MapArray.from_arrays(
        [0, 2, 2, 6], keys, items, mask=pa.array([False, True, False], type=pa.bool_())
    )
    assert result.equals(expected)
...
def test_fixed_size_list_from_arrays():
    ...
    result = pa.FixedSizeListArray.from_arrays(
        values, type=typ, mask=pa.array([False, True, False])
    )
    assert result.to_pylist() == [[0, 1, 2, 3], None, [8, 9, 10, 11]]

    result = pa.FixedSizeListArray.from_arrays(
        values, list_size=4, mask=pa.array([False, True, False])
    )
    assert result.to_pylist() == [[0, 1, 2, 3], None, [8, 9, 10, 11]]
...
def test_union_from_dense():
    ...
    def check_result(
        result, expected_field_names, expected_type_codes, expected_type_code_values
    ):
        result.validate(full=True)
        actual_field_names = [
            result.type[i].name for i in range(result.type.num_fields)
        ]
    ...
    check_result(
        pa.UnionArray.from_dense(
            logical_types, value_offsets, [binary, int64], ["bin", "int"], [11, 13]
        ),
        expected_field_names=["bin", "int"],
        expected_type_codes=[11, 13],
        expected_type_code_values=logical_types,
    ...
    # Bad type ids
    arr = pa.UnionArray.from_dense(logical_types, value_offsets, [binary, int64])
    with pytest.raises(pa.ArrowInvalid):
...
def test_union_from_sparse():
    ...
    # with type codes
    check_result(
        pa.UnionArray.from_sparse(logical_types, [binary, int64], type_codes=[11, 13]),
        expected_field_names=["0", "1"],
        expected_type_codes=[11, 13],
        expected_type_code_values=logical_types,
    )
...
def test_union_array_to_pylist_with_nulls():
    ...
    arr = pa.UnionArray.from_dense(
        ...
        [pa.array([0.0, 1.1, None, 3.3]), pa.array([True, None, False])],
    )
...
def test_cast_integers_unsafe():
    ...
    unsafe_cases = [
        (
            ...
            "int32",
            np.array([50000]).astype(dtype="i2"),
            pa.int16(),
        ),
        (
            np.array([70000], dtype="i4"),
            "int32",
            np.array([70000]).astype(dtype="u2"),
            pa.uint16(),
        ),
        (
            np.array([-1], dtype="i4"),
            "int32",
            np.array([-1]).astype(dtype="u2"),
            pa.uint16(),
        ),
        (
...
def test_decimal_to_int_non_integer():
    non_integer_cases = [
        (
            [decimal.Decimal("123456.21"), None, decimal.Decimal("-912345.13")],
            pa.decimal128(32, 5),
            [123456, None, -912345],
            pa.int32(),
        ),
...
def test_decimal_to_decimal():
    ...
    with pytest.raises(
        pa.ArrowInvalid, match="Decimal value does not fit in precision"
    ):
...
def test_cast_string_to_number_roundtrip():
    cases = [
        (pa.array(["1", "127", "-128"]), pa.array([1, 127, -128], type=pa.int8())),
        (
...
def test_cast_dictionary():
    ...
    for key_type in [pa.int8(), pa.int16(), pa.int32()]:
        ...
        expected = pa.array(
            ["foo", "bar", None], type=pa.dictionary(key_type, pa.string())
        )
    ...
    for arr, expected_values, expected_counts in cases:
        for arr_in in (arr, pa.chunked_array([arr])):
            ...
            assert result.type.equals(
                pa.struct(
                    [pa.field("values", arr.type), pa.field("counts", pa.int64())]
                )
            )
...
def test_unique_value_counts_dictionary_type():
    ...
    expected = pa.DictionaryArray.from_arrays(
        pa.array([], type="int64"), pa.array([], type="utf8")
    )
    assert unique_result.equals(expected)
...
def test_dictionary_encode_sliced():
    cases = [
        ...
        (
            pa.array([None, "foo", "bar", "foo", "xyzzy"], type=pa.large_string())[
                1:-1
            ],
            pa.DictionaryArray.from_arrays(
...
def test_array_conversions_no_sentinel_values():
    ...
    for ty in ["float16", "float32", "float64"]:
        arr3 = pa.array(np.array([1, np.nan, 2, 3, np.nan, 4], dtype=ty), type=ty)
        assert arr3.type == ty
...
def test_time32_time64_from_integer():
    ...
    expected = pa.array(
        [datetime.time(second=1), datetime.time(second=2), None], type=pa.time32("s")
    )
    assert result.equals(expected)

    result = pa.array([1, 2, None], type=pa.time32("ms"))
    expected = pa.array(
        [datetime.time(microsecond=1000), datetime.time(microsecond=2000), None],
        type=pa.time32("ms"),
    )
    assert result.equals(expected)
...
@pytest.mark.numpy
@pytest.mark.parametrize(
    "binary_type", [None, pa.binary(), pa.large_binary(), pa.binary_view()]
)
def test_array_from_numpy_ascii(binary_type):
    ...
    expected = pa.array(
        ["abcde", "", None, "abcde", "", None, "abcde", ""], type=expected_type
    )
    assert arrow_arr.equals(expected)
    ...
    expected_list = [
        ...
        pa.MonthDayNano(
            [
                ...
                (
                    datetime.timedelta(
                        seconds=1, microseconds=1, milliseconds=1, minutes=1, hours=1
                    )
    ...
    expected_list = [
        ...
        pa.MonthDayNano(
            [
                ...
                8,
                (
                    datetime.timedelta(seconds=1, microseconds=1, minutes=1, hours=1)
                    // datetime.timedelta(microseconds=1)
                )
                * 1000,
            ]
...
@pytest.mark.numpy
@pytest.mark.parametrize(
    "string_type", [None, pa.utf8(), pa.large_utf8(), pa.string_view()]
)
...
def test_nbytes_size():
    a = pa.chunked_array(
        [pa.array([1, None, 3], type=pa.int16()), pa.array([4, 5, 6], type=pa.int16())]
    )
    assert a.nbytes == 13
...
def test_map_array_values_offsets():
    ...
    ty_values = pa.struct(
        [pa.field("key", pa.utf8(), nullable=False), pa.field("value", pa.int32())]
    )
...
def test_struct_array_flattened_field():
    ty = pa.struct([pa.field("x", pa.int16()), pa.field("y", pa.float32())])
    a = pa.array(
        [(1, 2.5), (3, 4.5), (5, 6.5)], type=ty, mask=pa.array([False, True, False])
    )
...
def test_numpy_binary_overflow_to_chunked(large_types):
    ...
    for case, ex_type in [(values, binary_type), (unicode_values, string_type)]:
        arr = np.array(case)
...
def test_array_supported_masks():
    ...
    with pytest.raises(pa.ArrowTypeError):
        arr = pa.array(
            [4, None, 4, 3],
            mask=pa.array(
                [False, True, False, True], mask=pa.array([True, True, True, True])
            ),
        )
    ...
    with pytest.raises(TypeError):
        arr = pa.array(
            np.array([4, None, 4, 3.0]), mask=pa.array([True, False, True, False])
        )
...
def test_array_supported_pandas_masks():
    ...
    arr = pa.array(
        pandas.Series([0, 1], name="a", dtype="int64"),
...
def test_binary_array_masked():
    ...
    masked_nulls = pa.array(
        np.array([b"\x05"]), type=pa.binary(1), mask=np.array([True])
    )
    ...
    masked_nulls = pa.array(
        np.array([b"\x05"]), type=pa.binary(), mask=np.array([True])
    )
    assert [None] == masked_nulls.to_pylist()
    ...
    arrow_array = pa.array(
        npa, type=pa.binary(3), mask=np.array([False, False, False] * 10)
    )
    npa[npa == b"bbb"] = b"XXX"
...
def test_array_invalid_mask_raises():
    ...
    cases = [
        (
            [1, 2],
            np.array([False, False], dtype="O"),
            TypeError,
            "must be boolean dtype",
        ),
        ([1, 2], np.array([[False], [False]]), pa.ArrowInvalid, "must be 1D array"),
        ([1, 2, 3], np.array([False, False]), pa.ArrowInvalid, "different length"),
        (
            np.array([1, 2]),
            np.array([False, False], dtype="O"),
            TypeError,
        ...
        (np.array([1, 2, 3]), np.array([False, False]), ValueError, "different length"),
    ]
...
def check_run_end_encoded(
    ree_array, run_ends, values, logical_length, physical_length, physical_offset
):
...
def test_run_end_encoded_from_buffers():
    ...
    ree_array = pa.RunEndEncodedArray.from_buffers(
        ree_type, length, buffers, null_count, offset, children
    )
    check_run_end_encoded(ree_array, run_ends, values, 19, 4, 0)
    # buffers = []
    ree_array = pa.RunEndEncodedArray.from_buffers(
        ree_type, length, [], null_count, offset, children
    )
    check_run_end_encoded(ree_array, run_ends, values, 19, 4, 0)
    # null_count = -1
    ree_array = pa.RunEndEncodedArray.from_buffers(
        ree_type, length, buffers, -1, offset, children
    )
    check_run_end_encoded(ree_array, run_ends, values, 19, 4, 0)
    # offset = 4
    ree_array = pa.RunEndEncodedArray.from_buffers(
        ree_type, length - 4, buffers, null_count, 4, children
    )
    check_run_end_encoded(ree_array, run_ends, values, length - 4, 3, 1)
    # buffers = [None, None]
    with pytest.raises(ValueError):
        pa.RunEndEncodedArray.from_buffers(
            ree_type, length, [None, None], null_count, offset, children
        )
    # children = None
    with pytest.raises(ValueError):
        pa.RunEndEncodedArray.from_buffers(
            ree_type, length, buffers, null_count, offset, None
        )
    # len(children) == 1
    with pytest.raises(ValueError):
        pa.RunEndEncodedArray.from_buffers(
            ree_type, length, buffers, null_count, offset, [run_ends]
        )
    # null_count = 1
    with pytest.raises(ValueError):
        pa.RunEndEncodedArray.from_buffers(
...
def test_run_end_encoded_from_array_with_type():
    ...
    expected_2 = pa.RunEndEncodedArray.from_arrays(run_ends, values, ree_type_2)
...
@pytest.mark.parametrize(
    ("list_array_type", "list_type_factory"),
    [(pa.ListViewArray, pa.list_view), (pa.LargeListViewArray, pa.large_list_view)],
)
...
def test_list_view_from_arrays_fails(list_array_type, list_type_factory):
    ...
    with pytest.raises(pa.lib.ArrowInvalid):
        list_array_type.from_arrays(
            ...
            array_slice.sizes,
            array_slice.values,
            mask=array_slice.is_null(),
        )


@pytest.mark.parametrize(
    ("list_array_type", "list_type_factory", "offset_type"),
    [
...
def test_list_view_slice(list_view_type):
    ...
    array = list_view_type.from_arrays(
        offsets=[0, 3, 4], sizes=[3, 1, 2], values=[1, 2, 3, 4, 5, 6]
    )
