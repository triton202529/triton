...
def test_run_serialized_query(tmpdir, use_threads):
    ...
    query = tobytes(
        substrait_query.replace("FILENAME_PLACEHOLDER", pathlib.Path(path).as_uri())
    )
...
def test_udf_via_substrait(unary_func_fixture, use_threads):
  ...
  "relations": [
    ...
    expected_tb = test_table.add_column(
        1, "y", function(mock_udf_context(10), test_table["x"])
    )
...
def test_scalar_aggregate_udf_basic(varargs_agg_func_fixture):
    ...
    test_table = pa.Table.from_pydict(
        {"k": [1, 1, 2, 2], "v1": [1, 2, 3, 4], "v2": [1.0, 1.0, 1.0, 1.0]}
    )
...
"""
    ...
    expected_tb = pa.Table.from_pydict({"k": [1, 2], "v_avg": [2.5, 4.5]})
...
"""
    ...
    expected_tb = pa.Table.from_pydict(
        {"t": [1, 1, 2, 2], "k": [1, 0, 0, 1], "v_avg": [3.5, 3.5, 9.0, 11.0]}
    )
...
def test_serializing_schema():
    substrait_schema = b"\n\x01x\n\x01y\x12\x0c\n\x04*\x02\x10\x01\n\x04b\x02\x10\x01"
    expected_schema = pa.schema([pa.field("x", pa.int32()), pa.field("y", pa.string())])
