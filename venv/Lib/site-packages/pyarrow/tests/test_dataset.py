...
from pyarrow.tests.util import (
    FSProtocolClass,
    ProxyHandler,
...
def multisourcefs(request):
    ...
    for i, chunk in enumerate(
        [df_a.iloc[i : i + n // 10] for i in range(0, n, n // 10)]
    ):
    ...
    class reader:
        ...
        def _patch_kwargs(self, kwargs):
            if "use_threads" in kwargs:
                raise Exception(
                    ("Invalid use of dataset_reader, do not specify" " use_threads")
                )
...
def test_dataset(dataset, dataset_reader):
    ...
    assert result["f64"] == [0.0, 0.0, 1.0, 1.0, 2.0, 2.0, 3.0, 3.0, 4.0, 4.0]
    assert result["new"] == [
        False,
        False,
        True,
...
def test_scanner(dataset, dataset_reader):
    scanner = dataset_reader.scanner(dataset, memory_pool=pa.default_memory_pool())
    ...
    assert sorted_table["__filename"].to_pylist() == (
        ["subdir/1/xxx/file0.parquet"] * 5 + ["subdir/2/yyy/file1.parquet"] * 5
    )
...
def test_head(dataset, dataset_reader):
    ...
    result = dataset_reader.head(
        dataset, 2, columns=["i64"], filter=ds.field("i64") > 1
    ).to_pydict()
...
def test_take(dataset, dataset_reader):
    ...
    for indices in [[1, 7], pa.array([1, 7])]:
        assert dataset_reader.take(dataset, indices) == dataset_reader.to_table(
            dataset
        ).take(indices)
    with pytest.raises(IndexError):
...
def test_partitioning():
    ...
    table = pa.table(
        [
            pa.array(range(20)),
            pa.array(random.random() for _ in range(20)),
            pa.array(["a"] * 10 + ["b"] * 10),
        ],
        names=["f1", "f2", "part"],
    )
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        load_back = None
        with pytest.raises(
            ValueError, match="Expected Partitioning or PartitioningFactory"
        ):
...
def test_parquet_file_format_read_options():
    ...
    assert pff4.read_options == ds.ParquetReadOptions(binary_type=pa.binary_view())
    assert pff5.read_options == ds.ParquetReadOptions(list_type=pa.LargeListType)
...
def test_file_format_pickling(pickle_module):
    formats = [
        ...
        ds.CsvFileFormat(
            read_options=pa.csv.ReadOptions(skip_rows=3, column_names=["foo"])
        ),
        ds.CsvFileFormat(
            read_options=pa.csv.ReadOptions(skip_rows=3, block_size=2**20)
        ),
        ds.JsonFileFormat(),
        ds.JsonFileFormat(
            parse_options=pa.json.ParseOptions(
                newlines_in_values=True, unexpected_field_behavior="ignore"
            )
        ),
        ds.JsonFileFormat(
            read_options=pa.json.ReadOptions(use_threads=False, block_size=14)
        ),
    ]
    try:
...
def test_fragment_scan_options_pickling(pickle_module):
    options = [
        ...
        ds.CsvFragmentScanOptions(read_options=pa.csv.ReadOptions(block_size=2**16)),
        ds.JsonFragmentScanOptions(),
        ds.JsonFragmentScanOptions(
            pa.json.ParseOptions(
                newlines_in_values=False, unexpected_field_behavior="error"
            )
        ),
        ds.JsonFragmentScanOptions(
    ...
    assert factory.inspect().equals(
        pa.schema(
            [
                ...
                pa.field("struct", pa.struct({"a": pa.int64(), "b": pa.string()})),
...
def test_make_fragment_with_size(s3_example_simple):
    ...
    fragments_with_size = [
        file_format.make_fragment(path, fs, file_size=size)
...
def test_make_parquet_fragment_from_buffer(dataset_reader, pickle_module):
    ...
    dictionary_format = ds.ParquetFileFormat(
        read_options=ds.ParquetReadOptions(dictionary_columns=["alpha", "animal"]),
        use_buffered_stream=True,
        buffer_size=4096,
    )
...
def test_fragments_reconstruct(tempdir, dataset_reader, pickle_module):
    ...
    assert dataset_reader.to_table(pickled_fragment) == dataset_reader.to_table(
        fragment
    )
...
def test_fragments_parquet_row_groups(tempdir, dataset_reader):
    ...
    result = dataset_reader.to_table(row_group_fragments[0], schema=dataset.schema)
    assert result.column_names == ["f1", "f2", "part"]
    assert len(result) == 2
    ...
    result = dataset_reader.to_table(row_group_fragments[0], filter=ds.field("f1") < 1)
    assert len(result) == 1
...
def test_fragments_repr(tempdir, dataset):
    ...
    fragment = list(dataset.get_fragments())[0]
    assert (
        repr(fragment) == "<pyarrow.dataset.ParquetFileFragment path="
    ...
    fragment = list(dataset.get_fragments())[0]
    assert (
        repr(fragment) == "<pyarrow.dataset.FileFragment type=ipc path="
        f"{dataset.filesystem.normalize_path(str(path))}>"
    )
...
@pytest.mark.parametrize("pickled", [lambda x, m: x, lambda x, m: m.loads(m.dumps(x))])
def test_partitioning_factory_dictionary(
    mockfs, infer_dictionary, pickled, pickle_module
):
    paths_or_selector = fs.FileSelector("subdir", recursive=True)
    format = ds.ParquetFileFormat()
    options = ds.FileSystemFactoryOptions("subdir")
...
def test_partitioning_factory_segment_encoding(pickled, pickle_module):
    ...
    actual = factory.finish().to_table(
        columns={
    ...
    assert fragments[0].partition_expression.equals(
        (ds.field("date") == "2021-05-04 00%3A00%3A00") & (ds.field("string") == "%24")
    )
    ...
    factory = ds.FileSystemDatasetFactory(mockfs, selector, format, options)
    with pytest.raises(
        pa.ArrowInvalid, match="Could not cast segments for partition field"
    ):
        inferred_schema = factory.inspect()
...
def test_partitioning_factory_hive_segment_encoding_key_encoded(pickled, pickle_module):
    ...
    string_partition_schema_en = pa.schema(
        [("test%27%3B%20date", pa.string()), ("test%27%3B%5B%20string%27", pa.string())]
    )

    directory = (
        "hive/test%27%3B%20date=2021-05-04 00%3A00%3A00/"
...
def test_read_partition_keys_only(tempdir):
    ...
    table = pa.table(
        {"key": pa.repeat(0, BATCH_SIZE + 1), "value": np.arange(BATCH_SIZE + 1)}
    )
    pq.write_to_dataset(table[:BATCH_SIZE], tempdir / "one", partition_cols=["key"])
    pq.write_to_dataset(
        table[: BATCH_SIZE + 1], tempdir / "two", partition_cols=["key"]
    )
...
def test_hive_partitioning_nulls(tempdir):
    ...
    part = ds.HivePartitioning(
        pa.schema([pa.field("a", pa.string()), pa.field("b", pa.string())]), None, "xyz"
    )
    ds.write_dataset(table, tempdir, format="ipc", partitioning=part)
...
def test_construct_from_single_file(tempdir, dataset_reader, pickle_module):
    ...
    assert (
        dataset_reader.to_table(d1)
        == dataset_reader.to_table(d2)
        == dataset_reader.to_table(d3)
        == dataset_reader.to_table(d4)
    )
...
def test_construct_dataset_with_invalid_schema():
    empty = ds.dataset(
        [], format="ipc", schema=pa.schema([("a", pa.int64()), ("a", pa.string())])
    )
...
def test_scan_iterator(use_threads):
    ...
    for factory, schema in (
        (lambda: pa.RecordBatchReader.from_batches(batch.schema, [batch]), None),
...
def test_open_dataset_partitioned_directory(tempdir, dataset_reader, pickle_module):
    ...
    with change_cwd(tempdir):
        dataset = ds.dataset(
            "dataset-partitioned/", partitioning=ds.partitioning(flavor="hive")
        )
        assert dataset.schema.equals(full_table.schema)
...
def test_partition_discovery(
    ...
    ).append(pa.field("part2", expected_type(part_keys2[0])))
    assert dataset.schema.equals(expected_schema)
...
def test_specified_schema(tempdir, dataset_reader):
    ...
    assert dataset.schema.equals(schema)
    with pytest.raises(
        NotImplementedError, match="Unsupported cast from int64 to list"
    ):
        dataset_reader.to_table(dataset)
...
def test_incompatible_schema_hang(tempdir, dataset_reader):
    ...
    with pytest.raises(
        NotImplementedError, match="Unsupported cast from int64 to null"
    ):
...
def test_orc_writer_not_implemented_for_dataset():
    with pytest.raises(
        ...
        ds.write_dataset(pa.table({"a": range(10)}), format="orc", base_dir="/tmp")
...
def test_csv_format_options(tempdir, dataset_reader):
    ...
    dataset = ds.dataset(
        path, format=ds.CsvFileFormat(read_options=pa.csv.ReadOptions(skip_rows=1))
    )
    result = dataset_reader.to_table(dataset)
    ...
    dataset = ds.dataset(
        path,
        format=ds.CsvFileFormat(read_options=pa.csv.ReadOptions(column_names=["foo"])),
    )
    result = dataset_reader.to_table(dataset)
...
def test_csv_fragment_options(tempdir, dataset_reader):
    ...
    convert_options = pyarrow.csv.ConvertOptions(
        null_values=["MYNULL"], strings_can_be_null=True
    )
...
def test_json_format_options(tempdir, dataset_reader):
    ...
    with pytest.raises(ValueError, match="try to increase block size"):
        dataset = ds.dataset(
            path,
            format=ds.JsonFileFormat(read_options=pa.json.ReadOptions(block_size=4)),
        )

    dataset = ds.dataset(
        path, format=ds.JsonFileFormat(read_options=pa.json.ReadOptions(block_size=64))
    )
    result = dataset_reader.to_table(dataset)
    assert result.equals(table)
...
def test_encoding(tempdir, dataset_reader):
    ...
    for encoding, input_rows in [
        ...
        expected_table = pa.table(
            {"a": ["un"], "b": ["éléphant"]}, schema=expected_schema
        )
...
@pytest.mark.pandas
@pytest.mark.parametrize("compression", ["lz4", "zstd", "brotli"])  # not supported
def test_feather_format_compressed(tempdir, compression, dataset_reader):
    table = pa.table(
        {
            "a": pa.array([0] * 300, type="int8"),
            "b": pa.array([0.1, 0.2, 0.3] * 100, type="float64"),
        }
    )
...
@pytest.mark.pandas  # write_to_dataset currently requires pandas
@pytest.mark.skipif(
    sys.platform == "win32", reason="Results in FileNotFoundError on Windows"
)
...
def test_write_to_dataset_given_null_just_works(tempdir):
    ...
    table = pa.table(
        {"part": [None, None, "a", "a"], "col": list(range(4))}, schema=schema
    )
...
def _check_dataset_roundtrip(
    dataset, base_dir, expected_files, sort_col, base_dir_path=None, partitioning=None
):
    base_dir_path = base_dir_path or base_dir

    ds.write_dataset(
        dataset, base_dir, format="arrow", partitioning=partitioning, use_threads=False
    )
...
def test_write_dataset_partitioned(tempdir):
    ...
    expected_paths = [
        target / "part=a",
        target / "part=a" / "part-0.arrow",
        target / "part=b",
        target / "part=b" / "part-0.arrow",
    ]
    ...
    expected_paths = [
        target / "a",
        target / "a" / "part-0.arrow",
        target / "b",
        target / "b" / "part-0.arrow",
    ]
...
def test_write_dataset_with_field_names_hive(tempdir):
    ...
    ds.write_dataset(
        table, tempdir, format="ipc", partitioning=["b"], partitioning_flavor="hive"
    )
...
def test_write_dataset_with_scanner(tempdir):
    table = pa.table({"a": ["x", "y", None], "b": ["x", "y", "z"], "c": [1, 2, 3]})
    ...
    with tempfile.TemporaryDirectory() as tempdir2:
        ...
        load_back_table = load_back.to_table()
        assert dict(load_back_table.to_pydict()) == table.drop_columns("a").to_pydict()
...
def test_write_dataset_existing_data(tempdir):
    ...
    partitioning = ds.partitioning(
        schema=pa.schema([pa.field("c", pa.int64())]), flavor="hive"
    )
...
def _generate_data_and_columns(num_of_columns, num_of_records):
    ...
    for i in range(num_of_columns):
        data.append(
            _generate_random_int_array(size=num_of_records, min=1, max=num_of_records)
        )
...
def test_write_dataset_max_rows_per_file(tempdir):
    ...
    assert all(
        file_rowcount <= max_rows_per_file for file_rowcount in result_row_combination
    )
...
def test_write_dataset_min_rows_per_group(tempdir):
    ...
    record_batches = [
        _generate_data_and_columns(num_of_columns, num_of_records)
    ...
    for _, f_file in enumerate(files_in_dir):
        ...
        for id, batch in enumerate(batches):
            ...
            if id < len(batches) - 1:
                assert (
                    rows_per_batch >= min_rows_per_group
                    and rows_per_batch <= max_rows_per_group
                )
            else:
...
def test_write_dataset_max_open_files(tempdir):
    ...
    record_batch_1 = pa.record_batch(
        data=[[1, 2, 3, 4, 0, 10], ["a", "b", "c", "d", "e", "a"]], names=column_names
    )
    record_batch_2 = pa.record_batch(
        data=[[5, 6, 7, 8, 0, 1], ["a", "b", "c", "d", "e", "c"]], names=column_names
    )
    record_batch_3 = pa.record_batch(
        data=[[9, 10, 11, 12, 0, 1], ["a", "b", "c", "d", "e", "d"]], names=column_names
    )
    record_batch_4 = pa.record_batch(
        data=[[13, 14, 15, 16, 0, 1], ["a", "b", "c", "d", "e", "b"]],
        names=column_names,
    )

    table = pa.Table.from_batches(
        [record_batch_1, record_batch_2, record_batch_3, record_batch_4]
    )

    partitioning = ds.partitioning(
        pa.schema([(column_names[partition_column_id], pa.string())]), flavor="hive"
    )
    ...
    num_of_files_generated, number_of_partitions = _get_compare_pair(
        data_source_1, record_batch_1, file_format, partition_column_id
    )
    ...
    num_of_files_generated, number_of_partitions = _get_compare_pair(
        data_source_2, record_batch_1, file_format, partition_column_id
    )
    assert num_of_files_generated > number_of_partitions
...
def test_write_dataset_partitioned_dict(tempdir):
    ...
    # the formatting step)
    _check_dataset_roundtrip(
        dataset, str(target), expected_paths, "f1", target, partitioning=partitioning
    )
...
def test_write_dataset_use_threads_preserve_order(tempdir):
    ...
    ds.write_dataset(
        batches, tempdir, format="parquet", use_threads=True, preserve_order=True
    )
...
def test_write_table_multiple_fragments(tempdir):
    table = pa.table(
        [
            pa.array(range(10)),
            pa.array(random.random() for _ in range(10)),
            pa.array(["a"] * 5 + ["b"] * 5),
        ],
        names=["f1", "f2", "part"],
    )
...
def test_write_iterable(tempdir):
    ...
    ds.write_dataset(
        (batch for batch in table.to_batches()),
        base_dir,
        schema=table.schema,
        basename_template="dat_{i}.arrow",
        format="ipc",
...
def test_write_scanner(tempdir, dataset_reader):
    ...
    ds.write_dataset(
        dataset_reader.scanner(dataset, columns=["f1"]), base_dir, format="feather"
    )
    result = dataset_reader.to_table(ds.dataset(base_dir, format="ipc"))
    ...
    with pytest.raises(ValueError, match="Cannot specify a schema"):
        ds.write_dataset(
            dataset_reader.scanner(dataset),
            base_dir,
            schema=table.schema,
            format="feather",
        )
...
def test_write_dataset_csv(tempdir):
    ...
    # using custom options
    format = ds.CsvFileFormat(
        read_options=pyarrow.csv.ReadOptions(column_names=table.schema.names)
    )
...
def test_partition_dataset_parquet_file_visitor(tempdir):
    ...
    table = pa.table({"f1": f1_vals, "f2": f2_vals, "part": ["a"] * 20 + ["b"] * 20})
...
def test_write_dataset_s3_put_only(s3_server):
    ...
    _configure_s3_limited_user(
        s3_server, _minio_put_only_policy, "test_dataset_limited_user", "limited123"
    )
    fs = S3FileSystem(
...
def test_dataset_join(tempdir):
    ...
    assert result.to_table() == pa.table(
        {"colA": [1, 2, 6], "col2": ["a", "b", "f"], "col3": ["A", "B", None]}
    )
...
def test_dataset_join_collisions(tempdir):
    ...
    t2 = pa.table(
        {"colA": [99, 2, 1], "colB": [99, 20, 10], "colVals": ["Z", "B", "A"]}
    )
    ds.write_dataset(t2, tempdir / "t2", format="ipc")
    ds2 = ds.dataset(tempdir / "t2", format="ipc")
...
def test_dataset_join_asof(tempdir):
    t1 = pa.Table.from_pydict(
        {"colA": [1, 1, 5, 6, 7], "col2": ["a", "b", "a", "b", "f"]}
    )
    ds.write_dataset(t1, tempdir / "t1", format="ipc")
    ds1 = ds.dataset(tempdir / "t1", format="ipc")

    t2 = pa.Table.from_pydict(
        {"colB": [2, 9, 15], "col3": ["a", "b", "g"], "colC": [1.0, 3.0, 5.0]}
    )
    ds.write_dataset(t2, tempdir / "t2", format="ipc")
    ds2 = ds.dataset(tempdir / "t2", format="ipc")
...
def test_dataset_join_asof_multiple_by(tempdir):
    ...
    result = ds1.join_asof(ds2, on="on", by=["colA", "colB"], tolerance=1)
    assert result.to_table().sort_by("colA") == pa.table(
        {
            "colA": [1, 2, 6],
            "colB": [10, 20, 60],
...
def test_dataset_join_asof_empty_by(tempdir):
    ...
    result = ds1.join_asof(ds2, on="on", by=[], tolerance=1)
    assert result.to_table() == pa.table(
        {
            "on": [1, 2, 3],
            "colVals": ["Z", "Z", "B"],
...
def test_dataset_filter(tempdir, dstype):
    ...
    r2 = (
        ds1.filter(pc.field("colA") < 8)
        .filter(pc.field("colA") > 1)
        .scanner(filter=pc.field("colA") != 6)
    )
    ...
    joined = result.join(
        ds.dataset(pa.table({"colB": [10, 20], "col2": ["a", "b"]})),
        keys="col2",
        join_type="right outer",
    )
    assert joined.to_table().sort_by("colB") == pa.table(
        {"colA": [1, None], "colB": [10, 20], "col2": ["a", "b"]}
    )
...
def test_union_dataset_filter(tempdir, dstype):
    ...
    assert filtered_union_ds.to_table() == pa.table(
        {"colA": [1, 2, 9], "col2": ["a", "b", "h"]}
    )

    joined = filtered_union_ds.join(
        ds.dataset(pa.table({"colB": [10, 20], "col2": ["a", "b"]})),
        keys="col2",
        join_type="left outer",
    )
    assert joined.to_table().sort_by("colA") == pa.table(
        {"colA": [1, 2, 9], "col2": ["a", "b", "h"], "colB": [10, 20, None]}
    )
    ...
    assert table.to_pylist() == [
        {
            ...
            "type": None,
            "values": [1, 2],
            "structs": [{"fizz": None, "foo": "bar"}, None],
            "a.dotted.field": 1,
        },
        {
            "user_id": "qrs456",
...
def test_dataset_partition_with_slash(tmpdir):
    ...
    read_table = (
        ds.dataset(
            ...
            partitioning="hive",
            schema=pa.schema(
                [pa.field("exp_id", pa.int32()), pa.field("exp_meta", pa.utf8())]
            ),
        )
        .to_table()
        .combine_chunks()
...
def test_dataset_sort_by(tempdir, dstype):
    ...
    assert dt.filter((pc.field("values") < 4)).sort_by(
        "values"
    ).to_table().to_pydict() == {"keys": ["a", "a", "b"], "values": [1, 2, 3]}
...
def test_checksum_write_dataset_read_dataset_to_table(tempdir):
    ...
    table_corrupt = ds.dataset(
        corrupted_dir_path, format=pq_read_format_no_crc
    ).to_table()
...
def test_make_write_options_error():
    ...
    msg_1 = (
        "make_write_options() should be called on an " "instance of ParquetFileFormat"
    )
