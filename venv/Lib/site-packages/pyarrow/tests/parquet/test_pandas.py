...
def test_pandas_parquet_custom_metadata(tempdir):
    ...
    assert js["index_columns"] == [
        {"kind": "range", "name": None, "start": 0, "stop": 10000, "step": 1}
    ]
...
def test_pandas_parquet_column_multiindex(tempdir):
    ...
    df.columns = pd.MultiIndex.from_tuples(
        list(zip(df.columns, df.columns[::-1])), names=["level_1", "level_2"]
    )
...
def test_pandas_parquet_configuration_options(tempdir):
    ...
    for use_dictionary in [True, False]:
        _write_table(
            arrow_table, filename, version="2.6", use_dictionary=use_dictionary
        )
    ...
    for write_statistics in [True, False]:
        _write_table(
            arrow_table, filename, version="2.6", write_statistics=write_statistics
        )
        table_read = _read_table(filename)
        df_read = table_read.to_pandas()
        tm.assert_frame_equal(df, df_read)
...
def test_index_column_name_duplicate(tempdir):
    data = {
        ...
        "time": {
            pd.Timestamp("2017-06-30 01:31:00"): pd.Timestamp("2017-06-30 01:31:00"),
            pd.Timestamp("2017-06-30 01:32:00"): pd.Timestamp("2017-06-30 01:32:00"),
        },
    }
...
def test_backwards_compatible_column_metadata_handling(datadir):
    ...
    expected.index = pd.MultiIndex.from_arrays(
        [["a", "b", "c"], pd.date_range("2017-01-01", periods=3, tz="Europe/Brussels")],
        names=["index", None],
    )
...
def test_pandas_categorical_roundtrip():
    ...
    df = pd.DataFrame({"x": pd.Categorical.from_codes(codes, categories=categories)})
...
def test_categories_with_string_pyarrow_dtype(tempdir):
    ...
    assert (
        pa.array(df1["x"].cat.categories.values).to_pylist()
        == pa.array(df2["x"].cat.categories.values).to_pylist()
    )
...
def test_read_pandas_passthrough_keywords(tempdir):
    ...
    result = pq.read_pandas(
        "data.parquet", filesystem=SubTreeFileSystem(str(tempdir), LocalFileSystem())
    )
