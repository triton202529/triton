...
try:
    ...
    from pyarrow.tests.pandas_examples import (
...
def test_direct_read_dictionary_subfield():
    ...
    result = pq.read_table(
        pa.BufferReader(contents), read_dictionary=["f0.list.element"]
    )
    ...
    new_values = pa.DictionaryArray.from_arrays(
        inner_indices, values_as_dict.dictionary
    )
...
def test_single_pylist_column_roundtrip(
    tempdir,
    dtype,
):
...
def test_nested_list_nonnullable_roundtrip_bug():
    ...
    t = pa.table(
        [
            pa.array(
                ([[0] * ((i + 5) % 10) for i in range(0, 10)] * (num_rows // 10)),
                type=typ,
            )
        ],
        ["a"],
...
def test_large_list_records():
    ...
    list_values = [
        list(map(int, [random.randint(0, 100) for _ in range(x)])) if i % 8 else None
        ...
        for out_type, out_factory in list_types:
            for store_schema in (True, False):
                ...
                result = _roundtrip_table(
                    table,
                    write_table_kwargs=dict(store_schema=store_schema),
...
def test_large_binary_overflow():
    ...
    for use_dictionary in [False, True]:
        ...
        with pytest.raises(
            pa.ArrowInvalid, match="Parquet cannot store strings with size 2GB or more"
        ):
            _write_table(table, writer, use_dictionary=use_dictionary)


@pytest.mark.parametrize("storage_type", (pa.string(), pa.large_string()))
...
def test_uuid_extension_type():
    ...
    _check_roundtrip(
        table, table, {"arrow_extensions_enabled": True}, store_schema=False
    )
