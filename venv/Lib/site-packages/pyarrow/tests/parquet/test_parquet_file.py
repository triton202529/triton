...
def test_read_single_row_group():
    ...
    buf = io.BytesIO()
    _write_table(
        a_table, buf, row_group_size=N / K, compression="snappy", version="2.6"
    )
...
def test_read_column_invalid_index():
    table = pa.table(
        [pa.array([4, 5]), pa.array(["foo", "bar"])], names=["ints", "strs"]
    )
...
def test_iter_batches_reader(tempdir, chunk_size):
    ...
    for i in range(file_.num_row_groups):
        ...
        tm.assert_frame_equal(
            ...
            file_.read_row_groups([i]).to_pandas().iloc[900:].reset_index(drop=True),
        )
