...
def _check_roundtrip(
    table, expected=None, read_table_kwargs=None, **write_table_kwargs
):
    if expected is None:
        expected = table
...
def _roundtrip_pandas_dataframe(df, write_kwargs):
    ...
    result = _roundtrip_table(table, write_table_kwargs=write_kwargs)
...
def alltypes_sample(size=10000, seed=0, categorical=False):
    ...
    arrays = {
        ...
        "float64": np.arange(size, dtype=np.float64),
        "bool": np.random.randn(size) > 0,
        "datetime_ms": np.arange(
            "2016-01-01T00:00:00.001", size, dtype="datetime64[ms]"
        ),
        "datetime_us": np.arange(
            "2016-01-01T00:00:00.000001", size, dtype="datetime64[us]"
        ),
        "datetime_ns": np.arange(
            "2016-01-01T00:00:00.000000001", size, dtype="datetime64[ns]"
        ),
