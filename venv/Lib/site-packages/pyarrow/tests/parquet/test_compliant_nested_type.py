...
try:
    ...
    from pyarrow.tests.parquet.common import _read_table, _check_roundtrip
except ImportError:
    pq = None
...
_test_data_complex = [
    {"items": [{"name": "elem1", "value": "1"}, {"name": "elem2", "value": "2"}]},
...
def test_write_compliant_nested_type_disable(tempdir, test_data):
    ...
    with pq.ParquetWriter(
        path, table.schema, version="2.6", use_compliant_nested_type=False
    ) as writer:
        writer.write_table(table)
    new_table = _read_table(path)
    ...
    # Verify that the new table can be read/written correctly
    _check_roundtrip(new_table, use_compliant_nested_type=False)
