...
def test_filesystem_uri(tempdir):
    ...
    # filesystem object
    result = pq.read_table(path, filesystem=LocalFileSystem())
...
def test_filters_equivalency(tempdir):
    ...
    filters = [
        [("integer", "=", 1), ("string", "!=", "b"), ("boolean", "==", "True")],
        [("integer", "=", 0), ("boolean", "==", "False")],
    ]
    dataset = pq.ParquetDataset(base_path, filesystem=local, filters=filters)
    table = dataset.read()
    result_df = table.to_pandas().reset_index(drop=True)
    ...
    for filters in [[[("string", "==", b"1\0a")]], [[("string", "==", "1\0a")]]]:
        dataset = pq.ParquetDataset(base_path, filesystem=local, filters=filters)
...
def test_filters_cutoff_exclusive_datetime(tempdir):
    ...
    dataset = pq.ParquetDataset(
        base_path,
        filesystem=local,
        filters=[("dates", "<", "2018-04-12"), ("dates", ">", "2018-04-10")],
    )
    table = dataset.read()
    result_df = table.to_pandas().sort_values(by="index").reset_index(drop=True)
...
def test_filters_inclusive_datetime(tempdir):
    ...
    pd.DataFrame(
        {"dates": pd.date_range("2020-01-01", periods=10, freq="D"), "id": range(10)}
    ).to_parquet(path, use_deprecated_int96_timestamps=True)

    table = pq.read_table(
        path, filters=[("dates", "<=", datetime.datetime(2020, 1, 5))]
    )
...
def test_filters_invalid_pred_op(tempdir):
    ...
    dataset = pq.ParquetDataset(
        base_path, filesystem=local, filters=[("integers", "!=", {3})]
    )
...
def _generate_partition_directories(fs, base_dir, partition_spec, df):
    ...
    def _visit_level(base_dir, level, part_keys):
        ...
        for value in values:
            ...
            level_dir = pathsep.join([str(base_dir), f"{name}={value}"])
...
def test_read_multiple_files(tempdir):
    ...
    expected = pa.Table.from_arrays(
        [result.column(i) for i in to_read],
        names=col_names,
...
def _test_write_to_dataset_with_partitions(
    base_path, filesystem=None, schema=None, index_name=None
):
    import pandas as pd
    import pandas.testing as tm
    ...
    output_table = pa.Table.from_pandas(
        output_df, schema=schema, safe=False, preserve_index=False
    )
...
def _test_write_to_dataset_no_partitions(base_path, filesystem=None):
    ...
    selector = FileSelector(str(base_path), allow_not_found=False, recursive=True)
...
def test_write_to_dataset_with_partitions_and_schema(tempdir):
    schema = pa.schema(
        [
            pa.field("group1", type=pa.string()),
...
def test_write_to_dataset_with_partitions_and_index_name(tempdir):
    _test_write_to_dataset_with_partitions(str(tempdir), index_name="index_name")
...
@pytest.mark.skipif(
    sys.platform == "win32", reason="test fails because of unsupported characters"
)
def test_write_to_dataset_with_partitions_s3fs(s3_example_s3fs):
    fs, path = s3_example_s3fs

    _test_write_to_dataset_with_partitions(path, filesystem=fs)
...
def _make_dataset_for_pickling(tempdir, N=100):
    ...
    dataset = pq.ParquetDataset(tempdir, filesystem=local)
...
def test_read_table_duplicate_column_selection(tempdir):
    ...
    table = pa.table(
        {"a": pa.array([1, 2, 3], pa.int32()), "b": pa.array([1, 2, 3], pa.uint8())}
    )
