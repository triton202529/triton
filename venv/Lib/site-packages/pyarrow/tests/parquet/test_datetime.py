...
def test_pandas_parquet_datetime_tz():
    ...
    df = pd.DataFrame(
        {"tz_aware": s, "tz_eastern": s.dt.tz_convert("US/Eastern")}, index=s
    )
...
def test_coerce_timestamps_truncated(tempdir):
    ...
    explicitly allowed.
    """
    dt_us = datetime.datetime(
        year=2017, month=1, day=1, hour=1, minute=1, second=1, microsecond=1
    )
...
def test_list_of_datetime_time_roundtrip():
    ...
    times = pd.to_datetime(
        ["09:00", "09:30", "10:00", "10:30", "11:00", "11:30", "12:00"], format="%H:%M"
    )
...
def test_parquet_version_timestamp_differences():
    ...
    for ver in all_versions:
        _check_roundtrip(
            table, expected, version=ver, use_deprecated_int96_timestamps=True
        )
...
def test_noncoerced_nanoseconds_written_without_exception(tempdir):
    ...
    n = 9
    df = pd.DataFrame(
        {"x": range(n)}, index=pd.date_range("2017-01-01", freq="ns", periods=n)
    )
