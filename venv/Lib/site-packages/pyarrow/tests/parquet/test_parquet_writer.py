...
def test_validate_schema_write_table(tempdir):
    # ARROW-2926
    simple_fields = [pa.field("POS", pa.uint32()), pa.field("desc", pa.string())]
    ...
    with pq.ParquetWriter(
        path, simple_schema, version="2.6", compression="snappy", flavor="spark"
    ) as w:
...
def test_parquet_writer_chunk_size(tempdir):
    ...
    def check_chunk_size(data_size, chunk_size, expect_num_chunks):
        ...
        if remainder == 0:
            assert (
                metadata.row_group(expect_num_chunks - 1).num_rows == latched_chunk_size
            )
        else:
...
def test_parquet_content_defined_chunking(tempdir):
    ...
    pq.write_table(
        table,
        tempdir / "chunked-default.parquet",
        use_dictionary=False,
        column_encoding="PLAIN",
    ...
    pq.write_table(
        table,
        tempdir / "chunked-custom.parquet",
        use_dictionary=False,
        column_encoding="PLAIN",
