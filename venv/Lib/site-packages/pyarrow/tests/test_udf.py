...
def wrong_output_datatype_func_fixture():
    ...
    pc.register_scalar_function(
        wrong_output_datatype, func_name, doc, in_types, out_type
    )
...
def unary_vector_func_fixture():
    ...
    pc.register_vector_function(
        pct_rank, func_name, doc, {"x": pa.float64()}, pa.float64()
    )
...
def check_scalar_function(
    func_fixture, inputs, *, run_in_dataset=True, batch_length=None
):
...
def test_udf_array_binary(binary_func_fixture):
    check_scalar_function(
        binary_func_fixture,
        [pa.array([10, 20], pa.int64()), pa.array([2, 4], pa.int64())],
    )
...
def test_registration_errors():
    ...
    with pytest.raises(TypeError, match=expected_expr):
        pc.register_scalar_function(
            test_reg_function, "test_output_function", doc, in_types, None
        )
    ...
    with pytest.raises(TypeError, match=expected_expr):
        pc.register_scalar_function(
            test_reg_function, "test_input_function", doc, None, out_type
        )

    # register an already registered function
    # first registration
    pc.register_scalar_function(
        test_reg_function, "test_reg_function", doc, {}, out_type
    )
    # second registration
    expected_expr = (
        "Already have a function registered with name:" + " test_reg_function"
    )
...
def test_function_doc_validation():
    ...
    with pytest.raises(ValueError, match="Function doc must contain a summary"):
        pc.register_scalar_function(
            add_const, "test_no_summary", func_doc, in_types, out_type
        )
    ...
    with pytest.raises(ValueError, match="Function doc must contain a description"):
        pc.register_scalar_function(
            add_const, "test_no_desc", func_doc, in_types, out_type
        )
...
def test_nullary_function(nullary_func_fixture):
    ...
    check_scalar_function(
        nullary_func_fixture, [], run_in_dataset=False, batch_length=1
    )
...
def test_wrong_output_datatype(wrong_output_datatype_func_fixture):
    ...
    expected_expr = (
        "Expected output datatype int16, " "but function returned datatype int64"
    )
...
def test_wrong_signature(wrong_signature_func_fixture):
    ...
    expected_expr = (
        r"wrong_signature\(\) takes 0 positional arguments " "but 1 was given"
    )
...
def test_scalar_udf_context(unary_func_fixture):
    ...
    res = pc.call_function(
        func_name, [pa.array([1] * 1000, type=pa.int64())], memory_pool=proxy_pool
    )
...
def test_hash_agg_basic(unary_agg_func_fixture):
    ...
    expected = (
        ...
        .rename_columns(["id", "value_mean_udf"])
    )
...
def test_hash_agg_random(sum_agg_func_fixture):
    ...
    expected = (
        table.group_by("id")
        .aggregate([("value", "sum")])
        .rename_columns(["id", "value_sum_udf"])
    )
