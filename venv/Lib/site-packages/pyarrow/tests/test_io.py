...
def test_buffer_invalid():
    with pytest.raises(TypeError, match="(bytes-like object|buffer interface)"):
        pa.py_buffer(None)
...
def test_cache_options():
    ...
    opts4 = pa.CacheOptions(
        hole_size_limit=4096, range_size_limit=8192, prefetch_limit=5
    )
    opts5 = pa.CacheOptions(hole_size_limit=4096, range_size_limit=8192, lazy=False)
    opts6 = pa.CacheOptions.from_network_metrics(
        time_to_first_byte_millis=100,
...
def test_cache_options_pickling(pickle_module):
    options = [
        pa.CacheOptions(),
        pa.CacheOptions(
            hole_size_limit=4096, range_size_limit=8192, lazy=True, prefetch_limit=5
        ),
    ]
...
def test_compression_level(compression):
    ...
    for compression_level in range(min_level, max_level + 1):
        ...
        decompressed_bytes = codec.decompress(
            compressed_bytes, INPUT_SIZE, asbytes=True
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "zstd",
        pytest.param(
            "snappy", marks=pytest.mark.xfail(raises=pa.lib.ArrowNotImplementedError)
        ),
    ],
)
...
def check_transcoding(data, src_encoding, dest_encoding, chunk_sizes):
    ...
    stream = pa.transcoding_input_stream(
        pa.BufferReader(data.encode(src_encoding)), src_encoding, dest_encoding
    )
    out = []
    while True:
...
def test_transcoding_input_stream(src_encoding, dest_encoding):
    # All at once
    check_transcoding(
        unicode_transcoding_example, src_encoding, dest_encoding, [1000, 0]
    )
    # Incremental
    check_transcoding(
        unicode_transcoding_example,
...
@pytest.mark.parametrize(
    "src_encoding, dest_encoding", [("utf-8", "utf-8"), ("utf-8", "UTF8")]
)
...
def test_transcoding_encoding_error(src_encoding, dest_encoding):
    ...
    stream = pa.transcoding_input_stream(
        pa.BufferReader("\u0100".encode(src_encoding)), src_encoding, dest_encoding
    )
...
def test_transcoding_decoding_error(src_encoding, dest_encoding):
    ...
    stream = pa.transcoding_input_stream(
        pa.BufferReader(b"\xff\xff\xff\xff"), src_encoding, dest_encoding
    )
