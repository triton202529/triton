...
def get_many_types():
    ...
    return (
        ...
        pa.map_(
            pa.field("key", pa.int32(), nullable=False), pa.field("value", pa.int32())
        ),
        pa.struct(
            [
                pa.field("a", pa.int32()),
        ...
        pa.union(
            [pa.field("a", pa.binary(10), nullable=False), pa.field("b", pa.string())],
            mode=pa.lib.UnionMode_SPARSE,
        ),
...
def test_is_map():
    ...
    fields = pa.map_(
        pa.field("key_name", pa.utf8(), nullable=False),
        pa.field("value_name", pa.int32()),
    )
    assert types.is_map(fields)

    entries_type = pa.struct([pa.field("key", pa.int8()), pa.field("value", pa.int8())])
...
def test_convert_custom_tzinfo_objects_to_string():
    ...
    msg = (
        r"Object returned by tzinfo.utcoffset\(None\) is not an instance "
...
def test_map_type():
    ...
    # field names don't matter by default
    ty_named = pa.map_(
        pa.field("x", pa.utf8(), nullable=False), pa.field("y", pa.int32())
    )
    assert ty == ty_named
    assert not ty.equals(ty_named, check_metadata=True)
    ...
    # metadata doesn't matter by default
    ty_metadata = pa.map_(
        pa.utf8(), pa.field("value", pa.int32(), metadata={"hello": "world"})
    )
    assert ty == ty_metadata
    assert not ty.equals(ty_metadata, check_metadata=True)
    ...
    for keys_sorted in [True, False]:
        assert (
            pa.map_(pa.utf8(), pa.int32(), keys_sorted=keys_sorted).keys_sorted
            == keys_sorted
        )
...
@h.given(
    st.lists(past.all_types) | st.lists(past.all_fields) | st.lists(past.all_schemas)
)
