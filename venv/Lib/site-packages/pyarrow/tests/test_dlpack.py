...
def test_dlpack_not_supported():
    ...
    with pytest.raises(
        TypeError, match="Can only use DLPack " "on arrays with no nulls."
    ):
        np.from_dlpack(arr)
    ...
    with pytest.raises(
        TypeError, match="Bit-packed boolean data type " "not supported by DLPack."
    ):
        np.from_dlpack(arr)
...
def test_dlpack_cuda_not_supported():
    ...
    with pytest.raises(
        NotImplementedError,
        match="DLPack support is implemented " "only for buffers on CPU device.",
    ):
        np.from_dlpack(carr)
