...
@pytest.mark.parametrize(
    ...
    [
        ...
        (pa.MonthDayNano([1, -1, -10100]), None, pa.MonthDayNanoIntervalScalar),
...
def test_invalid_scalar():
    ...
    s.validate()
    with pytest.raises(ValueError, match="string scalar contains invalid UTF8 data"):
        s.validate(full=True)
...
def test_timestamp():
    ...
    for i, unit in enumerate(units):
        ...
        expected = pd.Timestamp("2000-01-01 12:34:56").tz_localize("utc").tz_convert(tz)
...
def test_duration():
    ...
    for i, unit in enumerate(units):
        ...
        assert arrow_arr[1].value * 1000 ** (i + 1) == expected.total_seconds() * 1e9
...
def test_map(pickle_module):
    ...
    assert s.values.to_pylist() == [{"key": "a", "value": 1}, {"key": "b", "value": 2}]
    ...
    assert s.as_py() == v
    assert s[1] == (pa.scalar("b", type=pa.string()), pa.scalar(2, type=pa.int8()))
    assert s[-1] == s[1]
    assert s[-2] == s[0]
