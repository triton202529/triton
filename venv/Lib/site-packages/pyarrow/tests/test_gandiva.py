...
def test_filter():
    ...
    table = pa.Table.from_arrays([pa.array([1.0 * i for i in range(10000)])], ["a"])
...
@pytest.mark.skip(
    reason="Gandiva C++ did not have *real* binary, " "time and date support."
)
...
def test_boolean():
    ...
    table = pa.Table.from_arrays(
        [
            pa.array([1.0, 31.0, 46.0, 3.0, 57.0, 44.0, 22.0]),
            pa.array([5.0, 45.0, 36.0, 73.0, 83.0, 23.0, 76.0]),
        ],
        ["a", "b"],
    )
...
def test_filter_project():
    ...
    greater_than_function = builder.make_function(
        "greater_than", [node_a, node_b], pa.bool_()
    )
...
def test_to_string():
    ...
    assert str(builder.make_literal(2.0, pa.float64())).startswith(
        "(const double) 2 raw("
    )
