...
def test_filter(table_source):
    ...
    decl = Declaration.from_sequence(
        [table_source, Declaration("filter", options=FilterNodeOptions(field("c") > 1))]
    )
...
@pytest.mark.parametrize(
    "source", [pa.record_batch({"number": [1, 2, 3]}), pa.table({"number": [1, 2, 3]})]
)
...
def test_project(table_source):
    ...
    decl = Declaration.from_sequence(
        [
            table_source,
            Declaration(
                "project", ProjectNodeOptions([pc.multiply(field("a"), 2)], ["a2"])
            ),
        ]
    )
    result = decl.to_table()
    ...
    decl = Declaration.from_sequence(
        [table_source, Declaration("project", ProjectNodeOptions([pc.sum(field("a"))]))]
    )
...
def test_aggregate_scalar(table_source):
    decl = Declaration.from_sequence(
        [
            table_source,
            Declaration(
                "aggregate", AggregateNodeOptions([("a", "sum", None, "a_sum")])
            ),
        ]
    )
    result = decl.to_table()
...
def test_hash_join():
    ...
    expected = pa.table(
        [[2, 3], [5, 6], [2, 3], [4, 5]], names=["key", "a", "key", "b"]
    )
    assert result.equals(expected)
    ...
    # left join
    join_opts = HashJoinNodeOptions("left outer", left_keys="key", right_keys="key")
    joined = Declaration(
        "hashjoin", options=join_opts, inputs=[left_source, right_source]
    )
    result = joined.to_table()
    ...
    join_opts = HashJoinNodeOptions(
        ...
        right_keys="key",
        left_output=["key", "a"],
        right_output=[field("b")],
    )
    joined = Declaration(
        "hashjoin", options=join_opts, inputs=[left_source, right_source]
    )
    result = joined.to_table()
    expected = pa.table([[1, 2, 3], [4, 5, 6], [None, 4, 5]], names=["key", "a", "b"])
    assert result.sort_by("a").equals(expected)
...
def test_hash_join_with_residual_filter():
    ...
    result = joined.to_table()
    expected = pa.table([[2], [5], [2], [4]], names=["key", "a", "key", "b"])
    assert result.equals(expected)
    ...
    join_opts = HashJoinNodeOptions(
        "inner", left_keys="key", right_keys="key", filter_expression=always_true
    )
    joined = Declaration(
        "hashjoin", options=join_opts, inputs=[left_source, right_source]
    )
    ...
    join_opts = HashJoinNodeOptions(
        "inner", left_keys="key", right_keys="key", filter_expression=always_false
    )
    joined = Declaration(
        "hashjoin", options=join_opts, inputs=[left_source, right_source]
    )
