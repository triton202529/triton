...
def make_random_csv(num_cols=2, num_rows=10, linesep="\r\n", write_names=True):
    ...
    columns = [
        pa.array(row, type=pa.int64()) for row in split_columns(arr, num_cols, num_rows)
    ]
...
def test_split_rows_and_columns_utility():
    ...
    assert columns == [[1, 6], [2, 7], [3, 8], [4, 9], [5, 10]]
...
def test_read_options(pickle_module):
    ...
    match = (
        "ReadOptions: autogenerate_column_names cannot be true when"
...
class BaseTestCSV(abc.ABC):
    ...
    def test_skip_rows_after_names(self):
        ...
        assert (
            table.num_rows
            == expected.num_rows - opts.skip_rows - opts.skip_rows_after_names
        )
        table_dict = table.to_pydict()
        for name, values in expected.to_pydict().items():
            assert (
                values[opts.skip_rows + opts.skip_rows_after_names :]
                == table_dict[name]
            )
    ...
    def test_row_number_offset_in_errors(self):
        ...
        message_value = format_msg(
            "In CSV column #0: {}" "CSV conversion error to int32: invalid value 'a'",
            102,
            csv,
        )
        with pytest.raises(pa.ArrowInvalid, match=message_value):
            self.read_bytes(
                csv_bad_type, read_options=read_options, convert_options=convert_options
            )

        long_row = (b"this is a long row" * 15) + b",3\r\n"
        csv_bad_columns_long = csv + long_row
        message_long = format_msg(
            "{}Expected 4 columns, got 2: {} ...", 102, long_row[0:96].decode("utf-8")
        )
        ...
        csv_bad_columns_long = csv + long_row
        message_long = format_msg(
            "{}Expected 4 columns, got 2: {} ...", 101, long_row[0:96].decode("utf-8")
        )
        with pytest.raises(pa.ArrowInvalid, match=message_long):
            self.read_bytes(
                csv_bad_columns_long,
    ...
    def test_chunker_out_of_sync(self):
        ...
        for block_size in range(8, 11):
            with pytest.raises(ValueError, match="cell values spanning multiple lines"):
                self.read_bytes(rows, read_options=ReadOptions(block_size=block_size))
...
class BaseCSVTableRead(BaseTestCSV):
    ...
    def test_include_columns(self):
        ...
        with pytest.raises(
            KeyError,
            match="Column 'xx' in include_columns " "does not exist in CSV file",
        ):
    ...
    def test_decimal_point(self):
        ...
        table = self.read_bytes(
            rows, parse_options=parse_options, convert_options=convert_options
        )
    ...
    def test_simple_timestamps(self):
        ...
        table = self.read_bytes(rows)
        schema = pa.schema(
            [("a", pa.int64()), ("b", pa.timestamp("s")), ("c", pa.timestamp("ns"))]
        )
        assert table.schema == schema
        assert table.to_pydict() == {
            "a": [1970, 1989],
...
class BaseStreamingCSVRead(BaseTestCSV):
    ...
    def test_header(self):
        ...
        expected_schema = pa.schema(
            [("abc", pa.null()), ("def", pa.null()), ("gh", pa.null())]
        )
    ...
    def test_inference(self):
        ...
        self.check_reader(
            reader,
            expected_schema,
            [{"a": ["123", "abc", "gh"], "b": [b"456", b"de\xff", b"ij"]}],
        )
    ...
    def test_no_ending_newline(self):
        ...
        self.check_reader(
            reader, expected_schema, [{"a": [1, 4], "b": [2, 5], "c": [3, 6]}]
        )
    ...
    def test_column_options(self):
        ...
        reader = self.open_bytes(rows, read_options=read_options)
        expected_schema = pa.schema(
            [("d", pa.int64()), ("e", pa.int64()), ("f", pa.int64())]
        )
        self.check_reader(
            reader, expected_schema, [{"d": [1, 4], "e": [2, 5], "f": [3, 6]}]
        )
        ...
        with pytest.raises(
            KeyError, match="Column 'g' in include_columns does not exist"
        ):
            reader = self.open_bytes(
                rows, read_options=read_options, convert_options=convert_options
            )
        ...
        expected_schema = pa.schema(
            [("g", pa.null()), ("f", pa.int64()), ("e", pa.string())]
        )
        self.check_reader(
            reader, expected_schema, [{"g": [None, None], "e": ["2", "5"], "f": [3, 6]}]
        )
        ...
        expected_schema = pa.schema(
            [("g", pa.float64()), ("f", pa.int64()), ("e", pa.string())]
        )
        self.check_reader(
            reader, expected_schema, [{"g": [None, None], "e": ["2", "5"], "f": [3, 6]}]
        )
    ...
    def test_encoding(self):
        ...
        expected_schema = pa.schema([("a", pa.string()), ("b", pa.binary())])
        self.check_reader(
            reader, expected_schema, [{"a": ["un"], "b": [b"\xe9l\xe9phant"]}]
        )
...
def test_write_read_round_trip():
    ...
    for read_options, parse_options, write_options in [
        ...
        buf.seek(0)
        assert t == read_csv(
            buf, read_options=read_options, parse_options=parse_options
        )
        buf = io.BytesIO()
        with CSVWriter(buf, t.schema, write_options=write_options) as writer:
