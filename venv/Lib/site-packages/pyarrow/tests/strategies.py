...
fixed_size_binary_type = st.builds(pa.binary, st.integers(min_value=0, max_value=16))
...
numeric_types = st.one_of(
    integer_types, floating_types, decimal128_type, decimal256_type
)
...
timestamp_types = st.builds(
    pa.timestamp, unit=st.sampled_from(["s", "ms", "us", "ns"]), tz=timezones
)
duration_types = st.builds(pa.duration, st.sampled_from(["s", "ms", "us", "ns"]))
interval_types = st.just(pa.month_day_nano_interval())
temporal_types = st.one_of(
    date_types, time_types, timestamp_types, duration_types, interval_types
)

primitive_types = st.one_of(
    null_type, bool_type, numeric_types, temporal_types, binary_like_types
)
...
def list_types(item_strategy=primitive_types):
    return (
        ...
        | st.builds(pa.list_, item_strategy, st.integers(min_value=0, max_value=16))
...
def arrays(draw, type, size=None, nullable=True):
    ...
    if pa.types.is_null(ty):
        ...
        value = st.datetimes(
            timezones=st.just(tz), min_value=min_datetime, max_value=max_datetime
        )
        ...
        value = _pymap(
            ty.key_type, ty.item_type, size=_default_array_sizes, nullable=nullable
        )
...
pandas_compatible_types = st.deferred(
    lambda: st.one_of(
        ...
        dictionary_types(value_strategy=pandas_compatible_dictionary_value_types),
