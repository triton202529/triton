...
def test_joins_corner_cases():
    t1 = pa.Table.from_pydict(
        {"colA": [1, 2, 3, 4, 5, 6], "col2": ["a", "b", "c", "d", "e", "f"]}
    )

    t2 = pa.Table.from_pydict(
        {"colB": [1, 2, 3, 4, 5], "col3": ["A", "B", "C", "D", "E"]}
    )
...
@pytest.mark.parametrize(
    "jointype,expected",
    [
        ("left semi", {"colA": [1, 2], "col2": ["a", "b"]}),
        ("right semi", {"colB": [1, 2], "col3": ["A", "B"]}),
        ("left anti", {"colA": [6], "col2": ["f"]}),
        ("right anti", {"colB": [99], "col3": ["Z"]}),
        (
            "inner",
            {"colA": [1, 2], "col2": ["a", "b"], "colB": [1, 2], "col3": ["A", "B"]},
        ),
        (
            "left outer",
            {
...
@pytest.mark.parametrize(
    "use_datasets", [False, pytest.param(True, marks=pytest.mark.dataset)]
)
...
def test_table_join_collisions():
    ...
    result = _perform_join("full outer", t1, ["colA", "colB"], t2, ["colA", "colB"])
    ...
    result = _perform_join(
        "full outer", t1, "colA", t2, "colA", right_suffix="_r", coalesce_keys=False
    )
    result = result.combine_chunks()
    ...
    result = _perform_join(
        "full outer", t1, "colA", t2, "colA", right_suffix="_r", coalesce_keys=True
    )
    result = result.combine_chunks()
    result = result.sort_by("colA")
    assert result == pa.table(
...
def test_filter_table():
    ...
    result = _filter_table(
        t,
        (pc.field("a") <= pc.scalar(3)) & (pc.field("b") == pc.scalar(20)),
    )
...
def test_complex_filter_table():
    ...
    result = _filter_table(
        t,
        (
            (pc.bit_wise_and(pc.field("a"), pc.scalar(1)) == pc.scalar(0))
