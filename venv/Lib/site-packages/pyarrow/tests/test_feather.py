...
from pyarrow.feather import read_feather, write_feather, read_table, FeatherDataset
...
def compression(request):
    if request.param in ["lz4", "zstd"] and not pa.Codec.is_available(request.param):
...
def test_dataset(version):
    ...
    data = {
        "col_" + str(i): np.random.randn(num_values[0]) for i in range(num_values[1])
    }
    table = pa.table(data)
...
def test_buffer_bounds_error(version):
    ...
    for i in range(16, 256):
        table = pa.Table.from_arrays(
            [pa.array([None] + list(range(i)), type=pa.float64())], names=["arr"]
        )
        _check_arrow_roundtrip(table)
...
def test_boolean_object_nulls(version):
    repeats = 100
    table = pa.Table.from_arrays(
        [np.array([False, None, True] * repeats, dtype=object)], names=["arr"]
    )
    _check_arrow_roundtrip(table)
...
def test_delete_partial_file_on_error(version):
    ...
    # strings will fail
    df = pd.DataFrame(
        {"numbers": range(5), "strings": [b"foo", None, "bar", CustomClass(), np.nan]},
...
def test_timestamp(version):
    ...
    df["with_tz"] = df.naive.dt.tz_localize("utc").dt.tz_convert("America/Los_Angeles")
...
def test_unicode_filename(version):
    ...
    _check_pandas_roundtrip(df, path=random_path(prefix=name), version=version)


@pytest.mark.pandas
def test_read_columns(version):
    df = pd.DataFrame({"foo": [1, 2, 3, 4], "boo": [5, 6, 7, 8], "woo": [1, 3, 5, 7]})
    expected = df[["boo", "woo"]]

    _check_pandas_roundtrip(df, expected, version=version, columns=["boo", "woo"])
...
def test_nested_types(compression):
    ...
    table = pa.table(
        {"col": pa.StructArray.from_arrays([[0, 1, 2], [1, 2, 3]], names=["f1", "f2"])}
    )
