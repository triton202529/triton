...
def test_read_options(pickle_module):
    ...
    check_options_class_pickling(
        cls, pickler=pickle_module, block_size=1234, use_threads=False
    )
        ...
        for data in [rows, rows + b"\n"]:
            for newlines_in_values in [False, True]:
                ...
                with pytest.raises(ValueError, match="try to increase block size"):
                    self.read_bytes(
                        data, read_options=read_options, parse_options=parse_options
                    )
...
class BaseTestStreamingJSONRead(BaseTestJSON):
    ...
    def test_bad_first_chunk(self):
        ...
        with pytest.raises(
            pa.ArrowInvalid, match="straddling object straddles two block boundaries*"
        ):
    ...
    def test_bad_middle_chunk(self):
        ...
        assert reader.schema == expected_schema
        assert reader.read_next_batch().to_pydict() == {"i": [0]}
        with pytest.raises(
            pa.ArrowInvalid, match="straddling object straddles two block boundaries*"
        ):
    ...
    def test_bad_first_parse(self):
        ...
        with pytest.raises(pa.ArrowInvalid, match="JSON parse error: Invalid value.*"):
    ...
    def test_bad_middle_parse(self):
        ...
        reader = self.open_bytes(bad_middle_chunk, read_options=read_options)
        assert reader.schema == expected_schema
        assert reader.read_next_batch().to_pydict() == {"n": [1000]}
        with pytest.raises(
            pa.ArrowInvalid,
 ...
 Missing a comma or '}' after an object member*",
    ...
    def test_non_linewise_chunker_first_block(self):
        ...
        reader = self.open_bytes(
            bad_middle_chunk, read_options=read_options, parse_options=parse_options
        )
        assert reader.schema == expected_schema
        assert reader.read_next_batch().to_pydict() == {"n": [0]}
    ...
    def test_non_linewise_chunker_bad_middle_block(self):
        ...
        assert reader.schema == expected_schema
        assert reader.read_next_batch().to_pydict() == {"n": [0]}
        assert reader.read_next_batch().to_pydict() == {"n": [1]}
    ...
    def test_ignore_leading_empty_blocks(self):
        ...
        explicit_schema = pa.schema([("b", pa.bool_()), ("s", pa.utf8())])
        read_options = ReadOptions(block_size=24)
        parse_options = ParseOptions(explicit_schema=explicit_schema)
        expected_data = {"b": [True], "s": ["foo"]}

        reader = self.open_bytes(
            leading_empty_chunk, read_options=read_options, parse_options=parse_options
        )
