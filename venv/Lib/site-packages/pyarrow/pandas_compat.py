...
def construct_metadata(
    ...
    num_serialized_index_levels = len(
        [descr for descr in index_descriptors if not isinstance(descr, dict)]
    )
    ...
    column_metadata = []
    for col, name, field_name, arrow_type in zip(
        columns_to_convert, column_names, column_field_names, df_types
    ):
        metadata = get_column_metadata(
            col, name=name, arrow_type=arrow_type, field_name=field_name
        )
    ...
    if preserve_index is not False:
        non_str_index_names = []
        for level, arrow_type, descriptor in zip(
            index_levels, index_types, index_descriptors
        ):
...
def _get_columns_to_convert(df, schema, preserve_index, columns):
    ...
    if not df.columns.is_unique:
        raise ValueError(f"Duplicate column names found: {list(df.columns)}")
    ...
    index_levels = (
        _get_index_level_values(df.index) if preserve_index is not False else []
    )
    ...
    for name in columns:
        ...
        if _pandas_api.is_sparse(col):
            raise TypeError(f"Sparse pandas data (column {name}) not supported.")
    ...
    for i, index_level in enumerate(index_levels):
        ...
        if (
            isinstance(index_level, _pandas_api.pd.RangeIndex)
...
def _resolve_columns_of_interest(df, schema, columns):
    if schema is not None and columns is not None:
        raise ValueError(
            "Schema and columns arguments are mutually "
...
def dataframe_to_types(df, preserve_index, columns=None):
    (
        ...
        columns_to_convert,
        _,
    ) = _get_columns_to_convert(df, None, preserve_index, columns)
...
def dataframe_to_arrays(
    df, schema, preserve_index, nthreads=1, columns=None, safe=True
):
    (
        all_names,
        column_names,
    ...
    def convert_column(col, field):
        ...
        if not field_nullable and result.null_count > 0:
            raise ValueError(
                f"Field {field} was non-nullable but pandas column "
    ...
    def _can_definitely_zero_copy(arr):
        return (
            ...
            and arr.flags.contiguous
            and issubclass(arr.dtype.type, np.integer)
        )

    if nthreads == 1:
        arrays = [
            convert_column(c, f) for c, f in zip(columns_to_convert, convert_fields)
        ]
    else:
        arrays = []
...
def _reconstruct_block(item, columns=None, extension_columns=None, return_block=True):
    ...
    if "dictionary" in item:
        ...
        if _pandas_api.is_ge_v21():
            arr = _pandas_api.pd.array(block_arr.view("int64"), dtype=dtype, copy=False)
        else:
            arr = block_arr
            if return_block:
        ...
        if not hasattr(pandas_dtype, "__from_arrow__"):
            raise ValueError(
                "This column does not support to be converted "
...
def table_to_dataframe(
    ...
    if not ignore_metadata and pandas_metadata is not None:
        ...
        table, index = _reconstruct_index(
            table, index_descriptors, all_columns, types_mapper
        )
    ...
    result = pa.lib.table_to_blocks(
        options, table, categories, list(ext_columns_dtypes.keys())
    )
...
def _reconstruct_index(table, index_descriptors, all_columns, types_mapper=None):
    ...
    for descr in index_descriptors:
        if isinstance(descr, str):
            ...
            index_level = _pandas_api.pd.RangeIndex(
...
def _extract_index_level(
    table, result_table, field_name, field_name_to_metadata, types_mapper=None
):
...
def _reconstruct_columns_from_metadata(columns, column_indexes):
    ...
    * Part of :func:`~pyarrow.pandas_compat.table_to_blockmanager`
    ...
    levels_dtypes = [
        (
            level,
            col_index.get("pandas_type", str(level.dtype)),
            col_index.get("numpy_type", None),
        )
        for level, col_index in zip_longest(levels, column_indexes, fillvalue={})
    ]
