...
class _PyArrowDataFrame:
    ...
    def get_column(self, i: int) -> _PyArrowColumn:
        ...
        """
        return _PyArrowColumn(self._df.column(i), allow_copy=self._allow_copy)

    def get_column_by_name(self, name: str) -> _PyArrowColumn:
        """
        Return the column whose name is the indicated name.
        """
        return _PyArrowColumn(self._df.column(name), allow_copy=self._allow_copy)
    ...
    def get_columns(self) -> Iterable[_PyArrowColumn]:
        ...
        """
        return [
            _PyArrowColumn(col, allow_copy=self._allow_copy) for col in self._df.columns
        ]
    ...
    def get_chunks(self, n_chunks: Optional[int] = None) -> Iterable[_PyArrowDataFrame]:
        ...
        iterator = [
            _PyArrowDataFrame(batch, self._nan_as_null, self._allow_copy)
