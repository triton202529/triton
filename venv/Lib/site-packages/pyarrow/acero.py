...
def _dataset_to_decl(dataset, use_threads=True, implicit_ordering=False):
    decl = Declaration(
        "scan",
        ScanNodeOptions(
            dataset, use_threads=use_threads, implicit_ordering=implicit_ordering
        ),
    )
...
def _perform_join(
    ...
    if join_type == "left semi" or join_type == "left anti":
        ...
        right_columns = [col for col in right_columns if col not in right_keys_order]
    elif join_type == "right outer":
        left_columns = [col for col in left_columns if col not in left_keys_order]
        ...
        for idx, col in enumerate(left_columns + right_columns):
            if idx < len(left_columns) and col in left_column_keys_indices:
                ...
                projections.append(
                    Expression._call(
                        ...
                        [
                            ...
                            Expression._field(right_operand_index + right_key_index),
                        ],
                    )
                )
                ...
                if (
                    ...
                    col += right_suffix
                projected_col_names.append(col)
                projections.append(Expression._field(idx))
...
def _perform_join_asof(
    ...
    right_columns = [
        col for col in right_operand.schema.names if col not in [right_on] + right_by
    ]
    ...
    if isinstance(left_operand, ds.Dataset):
        left_source = _dataset_to_decl(
            left_operand, use_threads=use_threads, implicit_ordering=True
        )
    else:
        left_source = Declaration(
            "table_source",
            TableSourceNodeOptions(left_operand),
        )
    if isinstance(right_operand, ds.Dataset):
        right_source = _dataset_to_decl(
            right_operand, use_threads=use_threads, implicit_ordering=True
        )
    else:
        right_source = Declaration(
            "table_source", TableSourceNodeOptions(right_operand)
