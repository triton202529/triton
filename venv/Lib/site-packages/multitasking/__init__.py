...
def createPool(
    name: str = "main", threads: Optional[int] = None, engine: Optional[str] = None
) -> None:
    ...
    try:
        threads = int(threads) if threads is not None else config["MAX_THREADS"]
...
def task(callee: Callable[..., Any]) -> Callable[..., Optional[Union[Thread, Process]]]:
    ...
    def async_method(*args: Any, **kwargs: Any) -> Optional[Union[Thread, Process]]:
        ...
        if not config["KILL_RECEIVED"]:
            ...
            try:
                ...
                single = engine_class(target=_run_via_pool, args=args, kwargs=kwargs)
...
def wait_for_tasks(sleep: float = 0) -> bool:
    ...
    try:
        ...
        while True:
            ...
            running_tasks = [
                task for task in config["TASKS"] if task is not None and task.is_alive()
            ]
