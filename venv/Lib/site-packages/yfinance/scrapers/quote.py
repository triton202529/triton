...
class InfoDictWrapper(MutableMapping):
    ...
    def __getitem__(self, k):
        if k in info_retired_keys_price:
            warnings.warn(
                f"Price data removed from info (key='{k}'). Use Ticker.fast_info or history() instead",
                DeprecationWarning,
            )
            return None
            ...
            warnings.warn(
                f"Market cap removed from info (key='{k}'). Use Ticker.fast_info instead",
                DeprecationWarning,
            )
            return None
        elif k in info_retired_keys_symbol:
            warnings.warn(
                f"Symbol removed from info (key='{k}'). You know this already",
                DeprecationWarning,
            )
            return None
    ...
    def _get_1y_prices(self, fullDaysOnly=False):
        if self._prices_1y is None:
            ...
            self._prices_1y = self._tkr.history(
                period="380d", auto_adjust=False, keepna=True, proxy=self.proxy
            )
...
class Quote:
    ...
    def _fetch_info(self, proxy):
        ...
        query1_info = next(
            (
                info
                for info in result.get("quoteSummary", {}).get("result", [])
