...
class TickerBase:
    ...
    def history(
        ...
        if start or period is None or period.lower() == "max":
            ...
            if start is None:
                if interval == "1m":
                    ...
                    max_start_datetime = pd.Timestamp.utcnow().floor(
                        "D"
                    ) - _datetime.timedelta(days=99 * 365)
        ...
        try:
            data = get_fn(url=url, params=params, proxy=proxy, timeout=timeout)
        ...
        logger.debug(
            f"{self.ticker}: yfinance received OHLC data: {quotes.index[0]} -> {quotes.index[-1]}"
        )
        ...
        quotes = utils.fix_Yahoo_returning_live_separate(
            quotes, params["interval"], tz_exchange
        )
        intraday = params["interval"][-1] in ("m", "h")
        ...
        logger.debug(
            f"{self.ticker}: OHLC after cleaning: {quotes.index[0]} -> {quotes.index[-1]}"
        )
        ...
        logger.debug(
            f"{self.ticker}: OHLC after combining events: {quotes.index[0]} -> {quotes.index[-1]}"
        )
        ...
        logger.debug(
            f"{self.ticker}: yfinance returning OHLC: {df.index[0]} -> {df.index[-1]}"
        )
    ...
    def _reconstruct_intervals_batch(self, df, interval, prepost, tag=-1):
        ...
        if interval in nexts:
            ...
            logger.warning(
                f"Have not implemented price repair for '{interval}' interval. Contact developers"
            )
        ...
        if (
            ...
            logger.debug(
                f"{self.ticker}: Price repair has hit max depth of 2 ('%s'->'%s'->'%s')",
        ...
        for g in dts_groups:
            ...
            if len(common_index) == 0:
                ...
                logger.info(
                    f"Can't calibrate {interval} block starting {start_d} so aborting repair"
                )
                continue
    ...
    def _fix_unit_random_mixups(self, df, interval, tz_exchange, prepost):
        ...
        if df.shape[0] == 1:
            ...
            logger.info(
                "price-repair-100x: Cannot check single-row table for 100x price errors"
            )
            if "Repaired?" not in df.columns:
                df["Repaired?"] = False
            return df
        ...
        if df2.shape[0] <= 1:
            logger.info(
                "price-repair-100x: Insufficient good data for detecting 100x price errors"
            )
            if "Repaired?" not in df.columns:
                df["Repaired?"] = False
            return df
        ...
        if n_fixed > 0:
            report_msg = (
                f"{self.ticker}: fixed {n_fixed}/{n_before} currency unit mixups "
            )
    ...
    def _fix_zeroes(self, df, interval, tz_exchange, prepost):
        ...
        if f_prices_bad.sum() == len(price_cols) * len(df2):
            ...
            logger.info(
                "price-repair-missing: No good data for calibration so cannot fix price=0 bad data"
            )
            if "Repaired?" not in df.columns:
                df["Repaired?"] = False
            return df
    ...
    def _fix_missing_div_adjust(self, df, interval, tz_exchange):
        ...
        if last_div_idx == 0:
            ...
            logger.debug(
                "div-adjust-repair: Insufficient data to recalculate div-adjustment"
            )
            return df
        ...
        if Yahoo_failed:
            ...
            if interval == "1d":
                # exclusive
                df2.loc[
                    : last_div_dt - _datetime.timedelta(seconds=1), "Adj Close"
                ] *= adj
            else:
                # inclusive
    ...
    def _fix_bad_stock_split(self, df, interval, tz_exchange):
        ...
        logger.debug(
            f"price-repair-split: Most recent split = {split:.4f} @ {most_recent_split_day.date()}"
        )

        return self._fix_prices_sudden_change(
            df, interval, tz_exchange, split, correct_volume=True
        )

    @utils.log_indent_decorator
    def _fix_prices_sudden_change(
        self, df, interval, tz_exchange, change, correct_volume=False
    ):
        if df.empty:
            return df
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            df_debug = df_debug.drop(
                ["Adj Close", "Volume", "Dividends", "Repaired?"],
                axis=1,
                errors="ignore",
            )
            debug_cols = ["Low", "High"]
        ...
        sd_pct = sd / avg
        logger.debug(
            f"price-repair-split: Estimation of true 1D change stats: mean = {avg:.2f}, StdDev = {sd:.4f} ({sd_pct*100.0:.1f}% of mean)"
        )
        ...
        if max(split, split_rcp) < 1.0 + largest_change_pct:
            logger.info(
                "price-repair-split: Split ratio too close to normal price volatility. Won't repair"
            )
            logger.debug(
        ...
        split_max = max(split, split_rcp)
        logger.debug(
            f"price-repair-split: split_max={split_max:.3f} largest_change_pct={largest_change_pct:.4f}"
        )
        ...
        if change in [100.0, 0.01] and f_splits.any():
            ...
            if f_pos.any():
                ...
                if gap_td < threshold:
                    logger.info(
                        "price-repair-split: 100x changes are too soon after stock split events, aborting"
                    )
                    return df
        ...
        if idx_latest_active is not None:
            ...
            logger.debug(
                f"price-repair-split: idx_latest_active={idx_latest_active}, idx_rev_latest_active={idx_rev_latest_active}"
            )
        if correct_columns_individually:
                ...
                if start_min is not None:
                    ...
                    for i in range(len(ranges) - 1, -1, -1):
                        ...
                        if df.index[r[0]].date() < start_min:
                            logger.debug(
                                f"price-repair-split: Pruning {c} range {df.index[r[0]]}->{df.index[r[1]-1]} because too old."
                            )
                            del ranges[i]
            ...
            if count == 0:
                ...
                col = OHLC[idx]
                logger.debug(
                    f"price-repair-split: Potential {fix_type} detected only in column {col}, so treating as false positive (ignore)"
                )
            else:
                ...
                for j in range(len(OHLC)):
                    ...
                    for r in ranges:
                        ...
                        if interday:
                            logger.info(
                                f"price-repair-split: Corrected {fix_type} on col={c} range=[{df2.index[r[1]-1].date()}:{df2.index[r[0]].date()}] m={m:.4f}"
                            )
                        else:
                            logger.info(
                                f"price-repair-split: Corrected {fix_type} on col={c} range=[{df2.index[r[1]-1]}:{df2.index[r[0]]}] m={m:.4f}"
                            )
            ...
            if start_min is not None:
                ...
                for i in range(len(ranges) - 1, -1, -1):
                    ...
                    if df.index[r[0]].date() < start_min:
                        logger.debug(
                            f"price-repair-split: Pruning range {df.index[r[0]]}->{df.index[r[1]-1]} because too old."
                        )
                        del ranges[i]
            for r in ranges:
                if r[2] == "split":
    ...
    def basic_info(self):
        warnings.warn(
            "'Ticker.basic_info' is renamed to 'Ticker.fast_info', hopefully purpose is clearer",
            DeprecationWarning,
        )
    ...
    def get_actions(self, proxy=None) -> pd.Series:
        ...
        if (
            ...
            and "Dividends" in self._history
            and "Stock Splits" in self._history
        ):
