...
def __dtype_from_pep3118(stream, is_subdtype):
    field_spec = {"names": [], "formats": [], "offsets": [], "itemsize": 0}
    offset = 0
    common_alignment = 1
    is_padding = False
    ...
    while stream:
        ...
        if stream.consume("T{"):
            ...
            raise ValueError(f"Unknown PEP 3118 data type specifier {stream.s!r}")
...
def _gcd(a, b):
    ...
    if not (math.isfinite(a) and math.isfinite(b)):
        raise ValueError(
            "Can only find greatest common divisor of "
...
def array_ufunc_errmsg_formatter(dummy, ufunc, method, *inputs, **kwargs):
    ...
    args_string = ", ".join(
        [f"{arg!r}" for arg in inputs] + [f"{k}={v!r}" for k, v in kwargs.items()]
    )
