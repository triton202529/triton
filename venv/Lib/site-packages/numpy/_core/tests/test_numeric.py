...
class TestResize:
    ...
    def test_unsigned_resize(self):
        ...
        for dt_pair in [(np.int32, np.uint32), (np.int64, np.uint64)]:
            ...
            assert_array_equal(
                np.resize(arr, dt_pair[0](1)), np.resize(arr, dt_pair[1](1))
            )
...
class TestTypes:
    ...
    def test_coercion(self):
        ...
        for a in [np.array([True, False]), np.array([-3, 12], dtype=np.int8)]:
            ...
            assert_equal(b.dtype, np.dtype(np.longdouble), f"array type {a.dtype}")
            ...
            assert_equal(b.dtype, np.dtype(np.clongdouble), f"array type {a.dtype}")
...
class TestNonzero:
    ...
    def test_nonzero_byteorder(self):
        values = [0.0, -0.0, 1, float("nan"), 0, 1, np.float16(0), np.float16(12.3)]
        ...
        for i in b:
            for j in b:
                ...
                with warnings.catch_warnings(record=True) as w:
                    ...
                    assert (
                        f"One of rtol or atol is not valid, atol: {i}, rtol: {j}"
                        in str(w[-1].message)
                    )
