...
groupbegins90 = (
    groupbegins77 + r"|module(?!\s*procedure)|python\s*module|(abstract|)\s*interface|"
    r"type(?!\s*\()"
)
beginpattern90 = (
...
def analyzeline(m, case, line):
    ...
    if case in ["begin", "call", "callfun"]:
            ...
            if bindcline:
                ...
                if bindcdat:
                    ...
                    if bindcdat.group("lang_name"):
                        groupcache[groupcounter]["bindlang"][name]["name"] = (
                ...
                if _intentcallbackpattern.match(ap):
                    if k not in groupcache[groupcounter]["args"]:
                        if groupcounter > 1:
                            ...
                            errmess(f"analyzeline: intent(callback) {k} is ignored\n")
                    else:
                        errmess(
        ...
        for l in ll:
            ...
            if l[0].startswith("("):
                outmess(
                    f'analyzeline: implied-DO list "{l[0]}" is not supported. Skipping.\n'
                )
                continue
            for idx, v in enumerate(
                ...
                if v.startswith("("):
                    outmess(
                        f'analyzeline: implied-DO list "{v}" is not supported. Skipping.\n'
                    )
                ...
                if "!" in l[1]:
                    ...
                    outmess(
                        f'Comment line in declaration "{l[1]}" is not supported. Skipping.\n'
                    )
                    continue
                vars.setdefault(v, {})
                ...
                if current_val and (current_val != new_val):
                    outmess(
                        f'analyzeline: changing init expression of "{v}" ("{current_val}") to "{new_val}"\n'
                    )
                vars[v]["="] = new_val
                last_name = v
            ...
            if m:
                ...
                if n in block["vars"]:
                    if "attrspec" in block["vars"][n]:
                        ...
                        block["vars"][n]["attrspec"] = [f"dimension({','.join(dims)})"]
                elif dims:
                    block["vars"][n] = {"attrspec": [f"dimension({','.join(dims)})"]}
                else:
                    block["vars"][n] = {}
                    ...
                    if len(dl) == 2:
                        ...
                        for v in block["vars"]:
                            ...
                            if dsize.contains(s):
                                ...
                                v_deps = {s.data for s in all_symbols if s.data in vars}
    ...
    if "commonvars" in block:
        for a in block["commonvars"]:
            if a in vars:
                ...
                errmess(f'vars2fortran: Confused?!: "{a}" is not defined in vars.\n')
    ...
    if isinstance(obj, list):
        ...
        for index, value in enumerate(obj):
            new_index, new_item = traverse(
                (index, value), visit, parents + [parent], result, *args, **kwargs
            )
        ...
        for key, value in obj.items():
            new_key, new_value = traverse(
                (key, value), visit, parents + [parent], result, *args, **kwargs
            )
