...
class _DataStream:
    ...
    def unsubscribe_updated_bars(self, *symbols):
        if self._running:
            asyncio.run_coroutine_threadsafe(
                self._unsubscribe(updated_bars=symbols), self._loop
            ).result()
        for symbol in symbols:
...
class DataStream(_DataStream):
                ...
                result = CancelErrorV2(
                    {
                        ...
                        for k, v in msg.items()
                        if k in cancel_error_mapping_v2
                    }
                )
            elif msg_type == "c":
                result = CorrectionV2(
    ...
    def unsubscribe_statuses(self, *symbols):
        if self._running:
            asyncio.run_coroutine_threadsafe(
                self._unsubscribe(statuses=symbols), self._loop
            ).result()
        for symbol in symbols:
    ...
    def unsubscribe_lulds(self, *symbols):
        if self._running:
            asyncio.run_coroutine_threadsafe(
                self._unsubscribe(lulds=symbols), self._loop
            ).result()
        for symbol in symbols:
...
class CryptoDataStream(_DataStream):
    ...
    def unsubscribe_orderbooks(self, *symbols):
        if self._running:
            asyncio.run_coroutine_threadsafe(
                self._unsubscribe(orderbooks=symbols), self._loop
            ).result()
        for symbol in symbols:
...
class Stream:
    def __init__(
        ...
        self._crypto_ws = CryptoDataStream(
            self._key_id,
            self._secret_key,
            self._data_stream_url,
            raw_data,
    ...
    def subscribe_trades(
        self, handler, *symbols, handler_cancel_errors=None, handler_corrections=None
    ):
        self._data_ws.subscribe_trades(handler, *symbols)
        self._data_ws.register_handler("cancelErrors", handler_cancel_errors, *symbols)
        self._data_ws.register_handler("corrections", handler_corrections, *symbols)
    ...
    def is_open(self):
        ...
        """
        open_ws = (
            self._trading_ws._ws
            or self._data_ws._ws
