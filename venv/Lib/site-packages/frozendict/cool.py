...
def isIterableNotString(o):
    ...
    return (
        isinstance(o, abc.Iterable)
        and not isinstance(o, memoryview)
        and not hasattr(o, "isalpha")
    )
...
def register(to_convert, converter, *, inverse=False):
    ...
    if not issubclass(type(to_convert), type):
        raise ValueError(f"`to_convert` parameter must be a type, {to_convert} found")
    ...
    if to_convert in freeze_conversion_map:
        ...
        warnings.warn(
            f"{to_convert.__name__} is already in the conversion map", FreezeWarning
        )
...
_freeze_types = [x for x in _freeze_conversion_map] + [
...
def deepfreeze(o, custom_converters=None, custom_inverse_converters=None):
    ...
    for type_i, converter in custom_inverse_converters.items():
        if not issubclass(type(type_i), type):
            raise ValueError(
                f"{type_i} in `custom_inverse_converters` parameter " + "is not a type"
            )
