...
def patchOrUnpatchJson(*, patch, warn=True):  # pragma: no cover
    ...
    if patch:
        ...
        if OldJsonEncoder is None:
            raise ValueError(
                "Old json encoder is None " + "(maybe you already unpatched json?)"
            )
...
def patchOrUnpatchOrjson(*, patch, warn=True):  # pragma: no cover
    ...
    if self._oldOrjsonDumps is None:
        if not patch:
            raise ValueError(
                "Old orjson encoder is None " + "(maybe you already unpatched orjson?)"
            )
...
def patchOrUnpatchMutableMappingSubclasshook(*, patch, warn=True):  # pragma: no cover
    warn_c = True
