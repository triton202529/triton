...
class ComplianceAuditEngine:
    ...
    def _check_disclosures(self, trade: Dict) -> Dict:
        ...
        for field in required:
            if field not in trade or not trade[field]:
                warnings.append({
                    'field': field,
                    'message': f"Missing required disclosure: {field}"
                })
    ...
    def generate_regulatory_report(self, 
        ...
        report = {
            ...
            'summary': {
                ...
                'violations': len([t for t in trades if not t.get('compliance_checks', {}).get('passed', True)])
            },
            'trades': trades,
    ...
    def _summarize_compliance(self, trades: List[Dict]) -> Dict:
        ...
        return {
            ...
            'violation_types': pd.Series([v.get('rule', 'unknown') for v in violations]).value_counts().to_dict()
        }
