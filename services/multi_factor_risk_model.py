...
class MultiFactorRiskModel:
    ...
    def __init__(self, min_samples: int = 100, verbose: bool = False):
        self.factor_loadings = pd.DataFrame()     # DataFrame: rows = factor columns, cols = PCA factors
    ...
    def _calculate_fundamental_factors(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        volume = pd.to_numeric(factors.get('volume', pd.Series(np.repeat(1e6, len(factors)))), errors='coerce').fillna(1e6)
        ...
        factors['size_factor'] = np.log(size_proxy.replace({0: np.nan})).replace([np.inf, -np.inf], np.nan)
    ...
    def fit(self, universe_data: Dict[str, pd.DataFrame]) -> 'MultiFactorRiskModel':
        ...
        for ticker, df in universe_data.items():
            try:
                ...
                fund_cols = ['market_return', 'size_factor', 'value_factor', 'momentum_factor', 'volatility_factor', 'quality_factor']
                tech_cols = ['trend_5d', 'trend_20d', 'trend_50d', 'momentum_1d', 'momentum_5d',
                             'momentum_20d', 'vol_5d', 'vol_20d', 'vol_ratio', 'rsi', 'mean_reversion']

                fund_df = fundamental_factors[[c for c in fund_cols if c in fundamental_factors.columns]]
                tech_df = technical_factors[[c for c in tech_cols if c in technical_factors.columns]]
        ...
        if len(combined_factors) < self.min_samples:
            print(f"⚠️ Insufficient data for factor model: {len(combined_factors)} samples (min {self.min_samples})")
            return self
        ...
        try:
            ...
            try:
                ...
                if hasattr(self.scaler, "mean_") and hasattr(self.scaler, "scale_"):
                    x_scaled = (x_vec - self.scaler.mean_) / np.where(self.scaler.scale_ == 0, 1, self.scaler.scale_)
                else:
                    x_scaled = x_vec
            ...
            for i, factor_name in enumerate(self.factor_loadings.columns):
                ...
                loading_vec = self.factor_loadings[factor_name].reindex(index=fitted_cols).fillna(0).values
    ...
    def get_portfolio_risk_decomposition(self, portfolio_weights: Dict[str, float],
    ...
    def get_risk_adjusted_weights(self, signals: Dict[str, float],
