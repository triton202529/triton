...
def clamp_window(days_requested: int) -> int:
    if days_requested > MAX_LOOKBACK_DAYS:
        print(
            f"ℹ️ Window {days_requested}d > max {MAX_LOOKBACK_DAYS}d. Clamping to {MAX_LOOKBACK_DAYS} days."
        )
...
def fetch_top_headlines(
    page_size: int = 100, page: int = 1, country: str = "us", category: str = "business"
):
...
def aggregate_mode(
    ...
    for i in range(0, len(tickers), batch_size):
        ...
        if all_rows:
            pd.DataFrame(all_rows).drop_duplicates(
                subset=["ticker", "title", "publishedAt"]
    ...
    if not any_success and not all_rows:
        ...
        if r is None:
            print("🚫 No news articles found (fallback failed).")
            return
        items = r.json().get("articles", []) or []
        if not items:
            print("🚫 No news articles found in top-headlines.")
            return
        for a in items:
...
def by_ticker_mode(
    tickers: list[str], window: int, output_path: str, page_size: int, max_pages: int
):
    end_dt = datetime.now(timezone.utc)
    window = clamp_window(window)
    start_dt = end_dt - timedelta(days=window)
