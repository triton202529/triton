...
class AdaptiveRiskEngine:
    ...
    def process_signals(
        ...
        try:
            regime_adjusted_weights = self._apply_regime_adjustments(
        ...
        if self.config.get("enable_hedging", True):
            try:
                hedged_weights = self._apply_tail_risk_hedging(
                    regime_adjusted_weights, regime, universe_data
                )
            except Exception as e:
    ...
    def _apply_regime_adjustments(
        self, weights: Dict[str, float], regime: str, universe_data: Dict[str, pd.DataFrame]
    ) -> Dict[str, float]:
