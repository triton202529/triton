...
class AutomatedFeatureEngineer:
    ...
    def engineer_features(self, df: pd.DataFrame, ticker: str = None) -> pd.DataFrame:
        ...
        if 'volume' in features.columns:
            ...
            features['volume_price_trend'] = (features['close'] - features['close'].shift(1)) * features['volume']
    ...
    def select_features(self, X: pd.DataFrame, y: pd.Series, method: str = 'mutual_info'):
        ...
        if method == 'mutual_info':
            selector = SelectKBest(score_func=mutual_info_regression, k=min(self.max_features, len(X_numeric.columns)))
        else:
            selector = SelectKBest(score_func=f_regression, k=min(self.max_features, len(X_numeric.columns)))
