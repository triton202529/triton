...
class VaRCVaREngine:
    ...
    def parametric_var(self, returns: pd.Series, portfolio_value: float = 1000000) -> Tuple[float, float]:
        """
        ...
        cvar_pct = -(mean_return - std_return * phi_z / (1 - self.confidence_level)) * np.sqrt(self.time_horizon)
        cvar_dollar = cvar_pct * portfolio_value
        
        return var_dollar, cvar_dollar
    
    def historical_var(self, returns: pd.Series, portfolio_value: float = 1000000) -> Tuple[float, float]:
        """
    ...
    def monte_carlo_var(self, returns: pd.Series, portfolio_value: float = 1000000) -> Tuple[float, float]:
        """
    ...
    def _calculate_component_var(self,
        ...
        for ticker in portfolio_tickers:
            ...
            position_var = weight * portfolio_value * self.portfolio_var / portfolio_value if self.portfolio_var else 0
    ...
    def stress_test(self,
                   ...
                   portfolio_value: float = 1000000) -> Dict[str, Dict]:
        """
        ...
        for scenario_name, shocks in scenarios.items():
            ...
            weights_array = np.array([weights.get(t, 0) for t in portfolio_tickers])
            shocked_portfolio_returns = (shocked_returns[portfolio_tickers] * weights_array).sum(axis=1)
    ...
    def _check_risk_limits(self, metrics: Dict, portfolio_value: float) -> Dict:
        ...
        if metrics['var'] / portfolio_value > limits['var_limit']:
            breaches['var'] = f"VaR {metrics['var']/portfolio_value:.2%} exceeds limit {limits['var_limit']:.2%}"
        
        if metrics['cvar'] / portfolio_value > limits['cvar_limit']:
            breaches['cvar'] = f"CVaR {metrics['cvar']/portfolio_value:.2%} exceeds limit {limits['cvar_limit']:.2%}"
        
        if metrics['volatility'] > limits['volatility_limit']:
            breaches['volatility'] = f"Volatility {metrics['volatility']:.2%} exceeds limit {limits['volatility_limit']:.2%}"
        
        if metrics['max_drawdown'] < limits['max_drawdown_limit']:
            breaches['max_drawdown'] = f"Max drawdown {metrics['max_drawdown']:.2%} exceeds limit {limits['max_drawdown_limit']:.2%}"
...
def main():
    ...
    scenarios = {
        'Market Crash': {'AAPL': -0.20, 'MSFT': -0.18, 'GOOGL': -0.22, 'AMZN': -0.25, 'TSLA': -0.30},
        'Tech Selloff': {'AAPL': -0.15, 'MSFT': -0.15, 'GOOGL': -0.18, 'AMZN': -0.12, 'TSLA': -0.20},
        'Mild Correction': {'AAPL': -0.05, 'MSFT': -0.05, 'GOOGL': -0.06, 'AMZN': -0.05, 'TSLA': -0.08}
    }
    ...
    for scenario, result in stress_results.items():
        print(f"\n  {scenario}:")
        print(f"    Expected Loss: ${result['expected_loss']:,.2f} ({result['expected_loss_pct']:.2%})")
        print(f"    Worst Case: ${result['worst_case_loss']:,.2f} ({result['worst_case_loss_pct']:.2%})")
