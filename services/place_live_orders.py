...
def _build_open_duplicates_set(
    open_orders: List[Dict[str, Any]],
) -> Set[Tuple[str, str, int, str, Optional[float]]]:
            ...
            print(
                (
                    ...
                    % (
                        ...
                        f"{cur_val:.3f}",
                        f"{lp:.3f}",
                        (f"{action}:{qty}" if action else flag),
                    )
                )
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Place live orders (paper/live) with risk caps, ticks, notifications."
    )
    ...
    parser.add_argument(
        "--session",
        default=None,
        help="Optional session label; default is YYYYMMDD-HHMMSS.",
    )
    parser.add_argument(
        "--use-bracket",
        action="store_true",
        help="Attach TP/SL bracket orders (BUY-only by default).",
    )
    parser.add_argument(
        "--tp-pct",
        type=float,
        default=0.03,
        help="Take profit percent for brackets (default 0.03 = 3%).",
    )
    parser.add_argument(
        "--sl-pct",
        type=float,
        default=0.015,
        help="Stop loss percent for brackets (default 0.015 = 1.5%).",
    )
    parser.add_argument(
        "--reconcile",
        action="store_true",
        help="Reconcile intended qty vs current position.",
    )
    parser.add_argument(
        "--reconcile-mode",
        default="all",
        choices=["all", "sells_only", "buys_only", "none"],
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Default price tick size (overridden by config/ticks.csv).",
    )
    parser.add_argument(
        "--tif", default="day", choices=["day", "gtc"], help="time_in_force for orders."
    )
    parser.add_argument(
        "--allow-duplicates",
        action="store_true",
        help="Allow identical orders more than once today.",
    )
    parser.add_argument(
        "--no-bp-check", action="store_true", help="Disable buying power pre-check."
    )
    parser.add_argument(
        "--bp-buffer",
        type=float,
        default=None,
        help="Override absolute BP buffer (dollars).",
    )
    parser.add_argument(
        "--bp-buffer-pct",
        type=float,
        default=None,
        help="Override fractional BP buffer (5 or 0.05 both mean 5%).",
    )
    ...
    parser.add_argument(
        "--reduce-sell-to-available",
        dest="reduce_sell_to_available",
        action="store_true",
        default=None,
    ...
    parser.add_argument(
        "--no-reduce-sell-to-available",
        dest="no_reduce_sell_to_available",
        action="store_true",
    ...
    bp_abs = (
        ...
        else float(risk_cfg.get("bp_buffer", 0.0) or 0.0)
    )
    bp_pct_raw = (
        args.bp_buffer_pct
        if args.bp_buffer_pct is not None
        else float(risk_cfg.get("bp_buffer_pct", 0.0) or 0.0)
    )
    ...
    pos_map: Dict[str, int] = (
        _load_positions_map(broker)
        if args.reconcile and args.reconcile_mode != "none"
        else _load_positions_map(broker)
    )
    ...
    for _, row in orders_df.iterrows():
        ...
        if opp_open:
            if args.cancel_opposites:
                if args.verbose:
                    print(
                        f"Canceling {len(opp_open)} open {opp_side.upper()} orders for {symbol} to avoid wash-trade..."
                    )
                ...
                if args.verbose:
                    print(
                        f"Opposite-side open exists for {symbol} ({opp_side}); skipping. Use --cancel-opposites to auto-cancel."
                    )
                _append_log(
                    [
                        _now_iso(),
        ...
        if side == "sell" and reduce_sells:
            ...
            if qty > avail:
                if args.verbose:
                    print(
                        f"{symbol} SELL qty {qty} > available {avail} (pos={cur_pos_qty}, held_sells={held_sell}); reducing."
                    )
                qty = avail
            if qty <= 0:
                if args.verbose:
        ...
        if side == "sell" and args.max_sell_fraction < 1.0:
            ...
            if qty > cap:
                if args.verbose:
                    print(
                        f"{symbol} SELL qty {qty} > cap {cap} (max-sell-fraction={args.max_sell_fraction:.3f}); reducing."
                    )
                qty = cap
            if qty <= 0:
                if args.verbose:
        ...
        limit_price_input = (
            None
            if (limit_price_input is None or pd.isna(limit_price_input))
            else float(limit_price_input)
        )
        ...
        if est_notional < float(args.min_notional or 0):
            if args.verbose:
                print(
                    f"Min-notional skip: {symbol} {side.upper()} {qty} est_notional={est_notional:.2f} < {args.min_notional:.2f}"
                )
            _append_log(
                [
                    _now_iso(),
        ...
        if symbol in pos_map:
            ...
            if (
                args.reconcile
                and args.reconcile_mode in ("all", "buys_only")
                and side == "buy"
                and current > 0
            ):
            ...
            if (
                args.reconcile
                and args.reconcile_mode in ("all", "sells_only")
                and side == "sell"
                and current > 0
            ):
                qty = min(qty, current)
        ...
        if side == "sell":
            ...
            if not args.respect_input_types:
                ...
                if last and last > 0:
                    ...
                    offset = (
                        float(args.buy_limit_offset_large)
                        if is_large
                        else float(args.buy_limit_offset_small)
                    )
        ...
        est_price_for_checks = (
            q_limit if (side == "buy" and otype == "limit" and q_limit) else est_price
        )
        ...
        if side == "buy" and account_equity > 0:
            ...
            if est_notional > max_add_notional:
                ...
                if max_qty <= 0:
                    print(
                        f"Risk block: {symbol} projected position would exceed {float(risk_cfg.get('max_position_pct',0.10))*100:.1f}% of equity."
                    )
                ...
                if max_qty < qty:
                    ...
                    if est_notional < float(args.min_notional or 0):
                        if args.verbose:
                            print(
                                f"Post-risk min-notional skip: {symbol} {side.upper()} {qty} est_notional={est_notional:.2f} < {args.min_notional:.2f}"
                            )
                        _append_log(
                            [
                                _now_iso(),
        ...
        if not args.no_bp_check and side == "buy":
            if bp_remaining <= 0.0 or est_notional > bp_remaining:
                msg = (
                    f"BP block: {symbol} BUY {qty} est_notional={est_notional:.2f} "
        ...
        if side == "buy" and (
            used_notional + est_notional > float(risk_cfg.get("max_daily_notional", 0.0))
        ):
            print(
                f"Risk block: daily notional cap reached ({used_notional:.2f} + {est_notional:.2f} > "
                f"{float(risk_cfg.get('max_daily_notional',0.0)):.2f})."
            )
            notify(
                "error",
                f"Risk block daily cap: {used_notional:.2f} + {est_notional:.2f} > "
                f"{float(risk_cfg.get('max_daily_notional',0.0)):.2f}",
            )
            continue
        ...
        if dup_key in dup_open:
            msg = (
                f"Duplicate open order exists for {symbol} {side.upper()} {qty} {otype.upper()} "
        ...
        if not args.allow_duplicates and _has_same_order_today(
            broker, symbol, side, qty, otype, q_limit
        ):
            msg = (
                f"Duplicate guard: {symbol} {side.upper()} {qty} {otype.upper()} "
        ...
        if not args.live:
            print(
                f"DRY RUN: {symbol} {side.upper()} {qty} {otype.upper()} "
        ...
        try:
            ...
            print(
                f"Submitted {symbol} {side.upper()} {qty} {otype.upper()}. status={status} order_id={order_id}"
            )
            notify(
                "submit",
