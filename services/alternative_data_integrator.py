...
class AlternativeDataIntegrator:
    ...
    def get_social_sentiment(self, ticker: str) -> Dict[str, float]:
        ...
        aggregate = np.average(
            [twitter_score, reddit_score, stocktwits_score],
            weights=[0.4, 0.4, 0.2]
        )
    ...
    def aggregate_alternative_signals(self, ticker: str) -> Dict[str, float]:
        ...
        signal_strengths = {
            source: data.get('signal_strength', 0.5)
        ...
        aggregate_signal = sum(
            signal_strengths[source] * weights[source]
            for source in signal_strengths.keys()
        )
