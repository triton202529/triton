...
def compute_confidence_row(row):
    ...
    edge_sign_ok = (
        1.0
        if (edge >= 0 and row.get("signal") == "BUY") or (edge < 0 and row.get("signal") == "SELL")
        else 0.3
    )
    ...
    trend = (
        1.0
        if (row.get("signal") == "BUY" and sma20 > sma50)
        or (row.get("signal") == "SELL" and sma20 < sma50)
        else 0.35
    )
    rsi_conf = (
        0.7
        if (row.get("signal") == "BUY" and rsi < 40) or (row.get("signal") == "SELL" and rsi > 60)
        else 0.4
    )
...
available_parquets = {
    fn.replace(".parquet", "").upper() for fn in os.listdir(RESULTS_DIR) if fn.endswith(".parquet")
}
...
def train_one_ticker(ticker: str, raw_df: pd.DataFrame, min_rows: int) -> bool:
    try:
        ...
        conf_df = df_model.apply(
            lambda r: pd.Series(
                compute_confidence_row(r),
                index=["confidence", "position_size", "delta_pct"],
            ),
            axis=1,
        )
...
for ticker in sorted(universe):
    if ticker not in available_parquets:
        skipped_tickers.append(
            {
                "ticker": ticker,
                "reason": "No parquet data (data/results/{TICKER}.parquet)",
            }
        )
        continue
