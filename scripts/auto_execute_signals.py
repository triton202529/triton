...
def cancel_open_orders_for_symbol(client: TradingClient, symbol: str) -> int:
    ...
    try:
        ...
        try:
            ...
            orders = [
                o for o in client.get_orders(filter=req) if getattr(o, "symbol", None) == symbol
            ]
        except Exception:
            orders = []
    canceled = 0
...
def main():
    ...
    ap.add_argument(
        ...
        help=f"Signals CSV (default: {DEFAULT_SIGNALS})",
    )
    ap.add_argument(
        "--log",
        default=str(DEFAULT_LOG),
        help=f"Trade log CSV (default: {DEFAULT_LOG})",
    )
    ap.add_argument(
        "--trade-pct",
        type=float,
        default=DEFAULT_TRADE_PCT,
    ...
    print(
        f"🔌 Connected: status={acct.status} cash=${float(acct.cash):,.2f} equity=${float(acct.equity):,.2f} mode={'paper' if paper else 'live'}"
    )
        ...
        if args.dry_run:
            ...
            for p in getattr(positions, "__iter__", lambda: [])():
                ...
                close_position_safely(
                    client, p.symbol, p.symbol, ysym, logs, timestamp0, dry_run=True
                )
        else:
            try:
                positions = client.get_all_positions()
            ...
            for p in positions:
                ...
                close_position_safely(
                    client, p.symbol, p.symbol, ysym, logs, timestamp0, dry_run=False
                )
    ...
    for _, row in latest_signals.iterrows():
        ...
        disp = (
            yahoo_symbol
            if yahoo_symbol.upper() == alpaca_symbol
            else f"{yahoo_symbol} (Alpaca: {alpaca_symbol})"
        )
        ...
        try:
            ...
            if side_text == "BUY":
                ...
                base_alloc = (
                    args.dollars_per_trade
                    if args.dollars_per_trade > 0
                    else (trade_pct * buying_power_now)
                )
                ...
                if alloc < min_alloc:
                    ...
                    print(
                        f"❌ Allocation ${alloc:.2f} below --min-alloc ${min_alloc:.2f} for {disp}"
                    )
                    continue
                    ...
                    if args.dry_run:
                        ...
                        logs.append(
                            [
                                ...
                                price,
                                "EXECUTED",
                                f"Order ID: {resp.id} (notional DAY)",
                            ]
                        )
                    ...
                    if qty <= 0:
                        logs.append(
                            [
                                ...
                                price,
                                "SKIPPED",
                                "Insufficient buying power for 1 share",
                            ]
                        )
